...
def set_vigil_home_override(path: str | Path | None) -> Token:
...
def _get_platform_default_vigil_home() -> Path:
    """Return the platform-native default VIGIL home path."""
...
def get_vigil_home() -> Path:
    ...
    Reads VIGIL_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``VIGIL_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``VIGIL_HOME`` explicitly (see the systemd
    template in ``vigil_cli/gateway.py`` and the kanban dispatcher in
    ``vigil_cli/kanban_db.py``).
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_vigil_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[VIGIL_HOME fallback] VIGIL_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass VIGIL_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return _get_platform_default_vigil_home()


def get_default_vigil_root() -> Path:
    ...
    """
    native_home = _get_platform_default_vigil_home()
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when VIGIL is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_vigil_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_vigil_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        ...
        4. ``<VIGIL_HOME>/skills`` last-resort
    """
...
def get_vigil_dir(new_subpath: str, old_name: str) -> Path:
...
def iter_vigil_node_dirs(home: Path | None = None) -> list[Path]:
    """Return VIGIL-managed Node.js directories in preferred lookup order.
...
def find_vigil_node_executable(command: str) -> str | None:
    """Return a VIGIL-managed Node/npm executable path, if installed."""
    names = _candidate_node_command_names(command)
    for directory in iter_vigil_node_dirs():
        for name in names:
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for VIGIL-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring VIGIL-managed installs.

    This is for VIGIL-owned subprocesses that should not be broken by a bad,
    missing, or elevation-triggering system Node/npm on PATH.
    """
    return find_vigil_node_executable(command) or find_node_executable_on_path(command)


def with_vigil_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with VIGIL-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_vigil_node_dirs() if path.is_dir()]
...
def secure_parent_dir(path: Path) -> None:
    ...
    See https://github.com/Signmanal/VIGIL/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    ...
    vigil_home = get_vigil_home_override() or (env or {}).get("VIGIL_HOME") or os.getenv("VIGIL_HOME")
    if not vigil_home:
        return None
    profile_home = os.path.join(vigil_home, "home")
...
def get_config_path() -> Path:
    ...
    Replaces the ``get_vigil_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, vigil_logging.py, vigil_time.py, etc.).
    """
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_vigil_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._vigil_ipv4_patched = True  # type: ignore[attr-defined]
