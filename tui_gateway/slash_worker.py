"""Persistent slash-command worker — one VIGILCLI per TUI session.
...
def _run(cli: VIGILCLI, command: str) -> str:
...
def main():
    ...
    # Start before the (hundreds-of-ms) VIGILCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = VIGILCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
