...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("VIGIL_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent VIGILCLI subprocess for slash commands."""
...
def _config_model_target() -> tuple[str, str]:
    ...
    config.yaml wins over VIGIL_MODEL / VIGIL_INFERENCE_MODEL here, the
    reverse of `_resolve_model()`'s startup order. Those env vars are a
    provision-time seed (hosted instances set VIGIL_INFERENCE_MODEL in the
...
def _load_fallback_model():
    ...
    stays in parity with ``VIGILCLI.__init__`` and ``gateway/run.py``:
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the VIGIL/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not VIGIL Desktop. If it serves VIGIL/Desktop UI or another unrelated app, stop that process and report failure.",
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("VIGIL_API_KEY", "") or cfg.get("api_key", "")
