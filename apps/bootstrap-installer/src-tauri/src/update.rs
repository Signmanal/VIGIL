...
//!
//! Driven when the installer is launched as `VIGIL-Setup.exe --update` (see
...
//!      rebuild apps/desktop by design — see cmd_update in vigil_cli/main.py),
...
/// vigil_cli/main.py (sys.exit(2)). We surface a targeted message for this.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating VIGIL"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let vigil_home = crate::paths::vigil_home();
    let install_root = vigil_home.join("vigil-agent");
    ...
    let vigil = resolve_hermes(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the vigil CLI under {}. Is VIGIL installed? \
    ...
    // time `vigil update` runs there is no legitimate vigil.exe to protect,
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "VIGIL is still running. Close all VIGIL windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "vigil update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::vigil_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 2: vigil desktop --build-only ----------------------------
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `vigil desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch VIGIL manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_vigil_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for VIGIL to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend vigil.exe (or the desktop VIGIL.exe
            ...
            // force-kill every VIGIL.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] VIGIL still holding install files ({}); force-killing stragglers…",
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("VIGIL.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("VIGIL.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `vigil.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `vigil.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\vigil.exe update`. And a
...
/// vigil.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named vigil.exe.)
...
fn venv_hermes(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("vigil.exe")
    } else {
        install_root.join("venv").join("bin").join("vigil")
    }
}
...
fn resolve_hermes(install_root: &Path) -> Option<PathBuf> {
    ...
    let exe = if cfg!(target_os = "windows") { "vigil.exe" } else { "vigil" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        ...
        vigil_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        vigil_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_vigil_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no VIGIL.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.vigil-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.vigil-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::vigil_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_vigil_is_under_install_root() {
        let root = Path::new("/x/vigil-agent");
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/vigil-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".vigil-update-in-progress");
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/VIGIL.app"]),
            Some(PathBuf::from("/Applications/VIGIL.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("VIGIL.app");
        let tmp = base.join("VIGIL.app.vigil-update-new");
        let old = base.join("VIGIL.app.vigil-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("VIGIL.app");
        let tmp = base.join("VIGIL.app.vigil-update-new"); // intentionally absent
        let old = base.join("VIGIL.app.vigil-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("VIGIL.app");
        let tmp = base.join("VIGIL.app.vigil-update-new"); // absent
        let old = base.join("VIGIL.app.vigil-update-old");
        write_marker(&target, "OLD");
