...
#[tauri::command]
pub async fn launch_vigil_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_vigil_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built VIGIL desktop at {}. The desktop build step \
             may have been skipped or failed. Run `vigil desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching VIGIL desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/VIGIL
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_vigil_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "VIGIL.exe"),
            ("win-arm64-unpacked", "VIGIL.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/VIGIL.app/Contents/MacOS", "VIGIL"),
            ("mac-arm64/VIGIL.app/Contents/MacOS", "VIGIL"),
        ]
    } else {
        &[("linux-unpacked", "vigil")]
    };
...
pub(crate) fn resolve_vigil_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_vigil_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../VIGIL.app/Contents/MacOS/VIGIL -> .../VIGIL.app
...
pub(crate) fn vigil_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".vigil-bootstrap-complete").exists()
        && resolve_vigil_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_vigil_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built VIGIL desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_vigil_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::vigil_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // the repo INTO at $HermesHome\vigil-agent. Mirrors vigil_constants.
    let vigil_home = args
        .vigil_home
        .clone()
        .unwrap_or_else(|| crate::paths::vigil_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&vigil_home).join("vigil-agent");

    // Copy ourselves to VIGIL_HOME/vigil-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_vigil_home() {
...
async fn run_install_script(
    ...
    args: &[String],
    vigil_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, vigil_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("VIGIL"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../VIGIL.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("VIGIL.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("vigil");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/VIGIL.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_vigil_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_vigil_desktop_app(&root)
    ...
    #[test]
    fn resolve_vigil_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_vigil_desktop_app(&root).is_none(),
