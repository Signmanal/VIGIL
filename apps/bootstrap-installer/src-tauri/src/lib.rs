...
/// desktop app handing off via `VIGIL-Setup.exe --update`).
///
...
pub fn run() {
    ...
    tracing::info!(?mode, force_setup, "VIGIL installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // startup-grace handling used by launch_vigil_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::vigil_home().join("vigil-agent");
                if bootstrap::vigil_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_vigil_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "vigil already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            update::start_update,
            // Hand-off
            bootstrap::launch_vigil_desktop,
            // Diagnostics
            paths::get_log_path,
            paths::get_vigil_home,
            paths::open_log_dir,
        ])
        .run(tauri::generate_context!())
        .expect("error while running VIGIL Setup");
}
