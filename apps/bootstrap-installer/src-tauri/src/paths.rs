...
//!
//! Mirrors `vigil_constants.get_vigil_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\vigil
//!   macOS:   ~/.vigil
//!   Linux:   ~/.vigil  (override via $VIGIL_HOME)
//!
//! NOTE (macOS): Python's get_vigil_home(), scripts/install.sh, and the
...
pub fn vigil_home() -> PathBuf {
    ...
    {
        // %LOCALAPPDATA%\vigil — matches scripts/install.ps1's $HermesHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("vigil");
        }
    }

    // macOS + Linux + fallback: ~/.vigil (matches Python get_vigil_home(),
    // install.sh, and the Electron desktop's resolveHermesHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".vigil");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    vigil_home().join("bootstrap-cache")
}
...
/// vigil-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\vigil\vigil-setup.exe`; on other
...
pub fn update_in_progress_marker() -> PathBuf {
    vigil_home().join(".vigil-update-in-progress")
}
...
pub fn copy_self_to_vigil_home() -> std::io::Result<()> {
...
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".vigil-bootstrap-complete")
}
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[vigil-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
...
#[tauri::command]
pub fn get_vigil_home() -> String {
    vigil_home().to_string_lossy().into_owned()
}
