...
const isOpen = (gateway: VIGILGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<VIGILGateway | null>(null)
...
let primaryGateway: VIGILGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: VIGILGateway | null, profile = 'default'): void {
...
export function activeGateway(): VIGILGateway | null {
...
function createSecondary(profile: string): Secondary {
  const gateway = new VIGILGateway()
...
export async function ensureActiveGatewayOpen(): Promise<VIGILGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
