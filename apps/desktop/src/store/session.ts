...
function workspaceCwdKey(connection: VIGILConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.vigilDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.vigilDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<VIGILConnection | null>(null)
...
export const setConnection = (next: Updater<VIGILConnection | null>) => updateAtom($connection, next)
