...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'vigil:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.vigilDesktop?.saveImageFromUrl) {
        if (await window.vigilDesktop.saveImageFromUrl(src)) {
