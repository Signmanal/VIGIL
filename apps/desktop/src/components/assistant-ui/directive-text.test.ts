...
import { formatRefValue, vigilDirectiveFormatter } from './directive-text'
...
describe('vigilDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = vigilDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = vigilDirectiveFormatter.parse('@file:src/main.tsx the entry point')
