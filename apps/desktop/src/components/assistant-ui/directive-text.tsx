...
const VIGIL_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type VIGILRefType = (typeof VIGIL_REF_TYPES)[number]
...
const ICON_PATHS: Record<VIGILRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as VIGILRefType] ?? ICON_FALLBACK
...
const VIGIL_DIRECTIVE_RE = new RegExp(
...
export const vigilDirectiveFormatter: Unstable_DirectiveFormatter = {
  ...
  const matches = [
    ...
    ...Array.from(text.matchAll(VIGIL_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as VIGILRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: VIGILRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...vigilDirectiveFormatter.parse(text)]
  } catch {
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.vigilDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.vigilDesktop?.readFileDataUrl(id)
