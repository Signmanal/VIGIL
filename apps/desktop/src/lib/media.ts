...
export function mediaStreamUrl(path: string): string {
  return `vigil-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// expose GET /api/media (vigil_cli/web_server.py).
export async function gatewayMediaDataUrl(path: string): Promise<string> {
  const file = filePathFromMediaPath(path)

  const result = await window.vigilDesktop!.api<{ data_url: string }>({
