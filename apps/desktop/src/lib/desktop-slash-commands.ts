...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active XCLAW profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show XCLAW Agent version', surface: exec() },
...
function isKnownVIGILSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownVIGILSlashCommand(normalized)
}
