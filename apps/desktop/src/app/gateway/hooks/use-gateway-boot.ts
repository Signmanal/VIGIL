...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.vigilDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: VIGILGateway | null) => void
  refreshVIGILConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: VIGILConnection | null) => {
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        await callbacksRef.current.refreshVIGILConfig().catch(() => undefined)
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshVIGILConfig()
