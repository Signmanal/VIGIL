...
// bug that drives the REAL useGatewayBoot hook + REAL VIGILGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshVIGILConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { vigilDesktop?: unknown }).vigilDesktop = fakeDesktop()
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForVIGIL) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startVIGIL()'s remote branch awaits waitForVIGIL() for 45s before it
    ...
    ;(window as { vigilDesktop?: unknown }).vigilDesktop = desktop
