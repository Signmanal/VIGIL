...
import type { ConfigFieldSchema, VIGILConfigRecord } from '@/types/hermes'
...
export function voiceFieldVisible(key: string, config: VIGILConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [_defaults, setDefaults] = useState<VIGILConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getVIGILConfigRecord(), getVIGILConfigDefaults(), getVIGILConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: VIGILConfigRecord) => {
