...
import { getVIGILConfig, getVIGILConfigDefaults } from '@/hermes'
...
interface VIGILConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useVIGILConfig({ activeSessionIdRef, refreshProjectBranch }: VIGILConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshVIGILConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getVIGILConfig(), getVIGILConfigDefaults().catch(() => ({}))])
  ...
  return { refreshVIGILConfig, sttEnabled, voiceMaxRecordingSeconds }
}
