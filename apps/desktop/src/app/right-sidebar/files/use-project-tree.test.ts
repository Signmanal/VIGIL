...
import type { VIGILReadDirResult } from '@/global'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<VIGILReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { vigilDesktop: { readDir: typeof readDir } }).vigilDesktop = { readDir }
})
...
afterEach(() => {
  ...
  $connection.set(null)
  resetProjectTreeState()
  delete (window as unknown as { vigilDesktop?: unknown }).vigilDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): VIGILReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { vigilDesktop: unknown }).vigilDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: VIGILReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<VIGILReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { vigilDesktop: unknown }).vigilDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.vigilDesktop is missing', async () => {
    delete (window as unknown as { vigilDesktop?: unknown }).vigilDesktop
