...
import type { VIGILReadDirEntry, VIGILReadDirResult } from '@/global'

export type ProjectTreeEntry = VIGILReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: VIGILReadDirEntry) {
...
async function filterIgnored(entries: VIGILReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<VIGILReadDirResult> {
  if (!window.vigilDesktop) {
