...
import { getVIGILConfigRecord, type VIGILConfigRecord, saveVIGILConfig } from '@/hermes'
...
export interface I18nConfigClient {
  getConfig: () => Promise<VIGILConfigRecord>
  saveConfig: (config: VIGILConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.vigilDesktop?.api) {
      return Promise.resolve({})
    }

    return getVIGILConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.vigilDesktop?.api) {
...
export function getConfigDisplayLanguage(config: VIGILConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: VIGILConfigRecord, locale: Locale): VIGILConfigRecord {
