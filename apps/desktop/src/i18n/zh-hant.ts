...
export const zhHant = defineLocale({
  ...
  boot: {
    ready: 'XCLAW Desktop 已就緒',
    desktopBootFailedWithMessage: message => `桌面啟動失敗：${message}`,
    steps: {
      connectingGateway: '正在連線桌面閘道',
      loadingSettings: '正在載入 XCLAW 設定',
      loadingSessions: '正在載入最近工作階段',
      startingDesktopConnection: '正在啟動桌面連線',
      startingVIGILDesktop: '正在啟動 XCLAW Desktop…'
    },
    errors: {
      backgroundExited: 'XCLAW 背景程序已結束。',
      backgroundExitedDuringStartup: 'XCLAW 背景程序在啟動期間結束。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '您的 XCLAW 後端早於目前的桌面版本，可能無法正常運作。請更新以保持一致。',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面後端拒絕了該請求 (405 Method Not Allowed)。請嘗試重新啟動 XCLAW Desktop。',
  ...
  settings: {
    ...
    resetConfirm: '要將所有設定恢復為 XCLAW 預設值嗎？',
    exportFailed: '匯出失敗',
    resetFailed: '重設失敗',
    nav: {
      providers: '提供方',
    ...
    notifications: {
      ...
      focusedHint: '完成提醒僅在 XCLAW 位於背景時觸發。',
      kinds: {
        approval: {
          label: '需要核准',
        ...
        input: {
          label: '需要輸入',
          description: 'XCLAW 提出了問題，或需要密碼或密鑰。'
        },
        turnDone: {
          label: '回覆就緒',
          description: 'XCLAW 在背景時完成了一輪對話。'
        },
        turnError: {
          label: '本輪失敗',
          description: '本輪以錯誤結束。'
      ...
      completionSoundDesc: '專家回合結束時播放。可在此選擇預設並預覽。',
    ...
    appearance: {
      ...
      colorMode: '色彩模式',
      colorModeDesc: '選擇固定模式，或讓 XCLAW 跟隨系統設定。',
      ...
      pet: {
        title: '寵物',
        intro:
          '領養一隻懸浮在應用上的 petdex 動畫寵物，它會根據 XCLAW 的狀態做出反應——工具執行時奔跑、成功時歡呼、出錯時沮喪。',
        restartHint: '寵物功能需要重新啟動——目前執行的應用在此功能加入前啟動。請結束並重新開啟 XCLAW，然後回到此處。',
        scaleTitle: '大小',
    ...
    fieldLabels: defineFieldCopy({
      ...
      delegation: {
        model: '子專家模型',
        provider: '子專家提供方',
        maxIterations: '子專家輪次上限',
        maxConcurrentChildren: '平行子專家',
        childTimeoutSeconds: '子專家逾時',
        reasoningEffort: '子專家推理強度'
      },
      updates: {
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: 'XCLAW 需要本機時間上下文時使用。留空則使用系統時區。',
      agent: {
        imageInputMode: '控制圖片附件如何傳送給模型。',
        maxTurns: 'XCLAW 停止一次執行前的工具呼叫輪次上限。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'XCLAW 單次檔案讀取可讀取的最大字元數。',
      approvals: {
        mode: 'XCLAW 如何處理需要明確批准的指令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'XCLAW 從應用程式內更新自身時，保留本機原始碼變更（stash）或丟棄（discard）。終端機更新一律會詢問。'
      }
    }),
    about: {
      ...
      uninstall: {
        heading: '解除安裝與資料管理',
        summaryTitle: '本機元件清理',
        summaryDesc: '可選擇僅移除桌面應用程式，或同時移除 Agent 與本機資料。預設不會執行任何解除安裝操作。',
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'XCLAW Desktop 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 XCLAW 後端，請使用遠端閘道。在下方按設定檔指定各自的遠端主機。',
      appliesTo: '套用至',
      allProfiles: '全部設定檔',
      ...
      localTitle: '本機閘道',
      localDesc: '在 localhost 啟動私有 XCLAW 後端。這是預設方式，可離線使用。',
      remoteTitle: '遠端閘道',
      remoteDesc:
        '將此桌面殼層連線至遠端 XCLAW 後端。託管閘道使用 OAuth 或帳號密碼；自託管閘道也可使用工作階段 Token。',
      ...
      savedTitle: '閘道設定已儲存',
      restartingMessage: 'XCLAW Desktop 將使用已儲存的設定重新連線。',
      savedMessage: '已儲存，下次重新啟動後生效。',
      connectedTo: (baseUrl, version) => `已連線至 ${baseUrl}${version ? ` · XCLAW ${version}` : ''}`,
    ...
    model: {
      ...
      resetUnavailableAux: '重設不可用任務',
      auxiliaryProviderUnavailable: (count, tasks, provider) =>
        `${count} 個輔助任務（${tasks}）正在使用 ${provider}，但該提供方尚未連線或設定。`,
    ...
    providers: {
      ...
      intro: '使用訂閱登入，無需複製 API 金鑰。XCLAW 會在應用程式中為您完成瀏覽器登入。',
      connected: '已連線',
      collapse: '收合',
    ...
    toolsets: {
      ...
      ready: '就緒',
      nousIncluded: '包含在 Nous 訂閱中；登入 XCLAW Portal 即可啟用。',
  ...
  skills: {
    ...
    searchMarket: '搜尋 skills.sh 或貼上 owner/repo...',
    refresh: '重新整理技能',
    refreshing: '正在重新整理技能',
    loading: '正在載入功能…',
    noSkillsTitle: '找不到技能',
    ...
    marketTitle: '技能市場',
    marketDesc: '搜尋已設定的 Skill Hub，將社群技能安裝到目前設定檔，然後重新整理本機技能列表。',
    marketOpen: '開啟',
    marketSource: '來源',
    marketAllSources: '全部來源',
    marketLoading: '正在搜尋技能市場...',
    marketSourcesTitle: '已設定來源',
    marketSourceReady: '可用',
    marketSourceUnavailable: '不可用',
    marketSourceRateLimited: '已限流',
    marketFeaturedTitle: '目前來源技能',
    marketFeaturedDesc: '顯示所選來源返回的技能列表，可直接安裝到目前設定檔。',
    marketEmptyTitle: '搜尋市場',
    marketEmptyDesc: '輸入 browser、security、docs 等主題，或貼上 skills.sh 上的 owner/repo 標識。',
    marketNoResultsTitle: '沒有市場結果',
    marketNoResultsDesc: '嘗試更廣泛的搜尋、切換來源，或直接安裝 owner/repo 標識。',
    marketInstall: '安裝',
    marketInstalling: '安裝中...',
    marketInstalled: '已安裝',
    marketInstallStarted: identifier => `${identifier} 已開始安裝。背景 action 完成後重新整理技能列表。`,
    marketInstallFailed: identifier => `安裝 ${identifier} 失敗`,
    marketSearchFailed: '技能市場搜尋失敗',
    marketDirectInstall: identifier => `直接安裝 ${identifier}`,
    marketTrust: trust => `信任層級：${trust}`,
    marketSourceLabel: source => `來源：${source}`
  },

  agents: {
    close: '關閉專家',
    title: '專家工作台',
    subtitle: '目前回合的子專家即時活動。',
    currentExpertTitle: '目前活躍專家',
    currentExpertDesc: '這裡顯示目前對話正在使用的專家設定；當某個回合派發任務時，子專家會在下方繼續顯示。',
    activeBadge: '活躍',
    manageCurrentExpert: '編輯角色',
    currentExpertNoDescription: '該專家暫未填寫描述，可在角色管理中編輯 SOUL.md 或補充設定檔說明。',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '請重新啟動 XCLAW 以使用寵物功能。',
      empty: '沒有符合的寵物。',
      turnOff: '關閉',
      turnOn: '開啟',
      installed: '已安裝',
    ...
    generatePet: {
      ...
      staleBackend: '請更新 XCLAW 以生成寵物。',
      backgroundHint: '你可以關閉此視窗——完成後 XCLAW 會通知你。',
    ...
    hermesActiveSessions: (version, count) => `XCLAW ${version} · 活躍工作階段 ${count}`,
  ...
  profiles: {
    ...
    enabledSkills: (enabled, total) => `已啟用 ${enabled}/${total}`,
    env: 'env',
    defaultBadge: '預設',
    rename: '重新命名',
    copySetup: '複製安裝指令',
    setupCommandHint: '複製的是此設定檔的 CLI setup 指令，可直接貼到終端機執行；技能市場安裝請在市場卡片中點擊安裝。',
    copying: '複製中…',
    modelLabel: '模型',
    skillsLabel: '技能',
    notSet: '未設定',
    ...
    editProfile: '編輯專家設定檔',
    editProfileDesc: '名稱、起始技能、MCP 和 SOUL.md 會作為草稿統一儲存。',
    unsavedChanges: '有未儲存的變更',
    ...
    saveSoul: '儲存 SOUL.md',
    resetChanges: '重設變更',
    defaultNameLocked: 'default 是根設定檔；這裡儲存的是顯示名稱，內部識別、路徑和命令仍保持 default。',
    deleteTitle: '刪除設定檔？',
    deleteDescPrefix: '這將刪除 ',
    deleteDescMid: ' 並移除其 ',
    deleteDescSuffix: ' 目錄。此操作無法復原。',
    deleting: '刪除中…',
    createDesc: '設定檔是獨立的 XCLAW 環境：各自擁有獨立的設定、技能和 SOUL.md。',
    nameLabel: '名稱',
    cloneFrom: '複製來源',
    cloneFromNone: '無（空白）',
    cloneFromDesc: '從選取的來源設定檔複製設定、技能和 SOUL.md。',
    startingSkills: '起始技能',
    startingSkillsDesc: source => `選擇建立後保持啟用的技能。目前從 ${source} 載入；清空則建立最小化專家。`,
    profileSkillsDesc: '選擇此設定檔自己的 skills 目錄中啟用哪些技能；不會直接修改 default。',
    startingMcpDesc: source => `選擇從 ${source} 複製哪些 MCP 伺服器和工具。`,
    profileMcpDesc: '選擇此設定檔啟用哪些 MCP 伺服器和工具。',
    searchProfileSkills: '搜尋技能名稱、分類或描述…',
    searchProfileMcp: '搜尋 MCP 名稱、指令或 URL…',
    loadingSkills: '正在載入技能...',
    loadingMcp: '正在載入 MCP 伺服器...',
    noSkillsAvailable: '此來源設定檔沒有可用技能。',
    noSkillSearchResults: '沒有符合的技能。',
    noMcpAvailable: '此來源設定檔沒有 MCP 伺服器設定。',
    noProfileMcpAvailable: '此設定檔沒有 MCP 伺服器設定。',
    noMcpSearchResults: '沒有符合的 MCP 伺服器。',
    skillsSelected: (selected, total) => `已選 ${selected}/${total}`,
    selectAllSkills: '全選',
    clearSkills: '清空',
  ...
  cron: {
    ...
    newCron: '新自動化',
    emptyDescNew: '按 cron 表達式排程一個提示詞。XCLAW 會執行它，並將結果傳送至您選擇的目的地。',
    ...
    updated: '自動化已更新',
    failedLoad: '載入自動化失敗',
    failedUpdate: '更新自動化失敗',
    failedTrigger: '觸發自動化失敗',
    failedDelete: '刪除自動化失敗',
    failedSave: '儲存自動化失敗',
    editTitle: '編輯自動化',
    createTitle: '新自動化',
  ...
  artifacts: {
    ...
    statSessions: '來源工作階段',
    statRelated: '關聯輸出',
    retentionTitle: '刪除與保留策略',
    retentionPolicy: (days, limit) =>
      `目前顯示最近 ${days} 天且最多 ${limit} 筆產物；刪除只針對產物中心識別到的本機生成產物檔案。`,
    retentionScope: '外部連結、遠端路徑、引用檔案、技能原始碼不會刪除。',
    retentionPending: count => `${count} 個本機生成產物檔案可刪除`,
    retentionHidden: count => `已刪除並隱藏 ${count} 筆產物記錄`,
    retentionDaysLabel: '保留天數',
    retentionLimitLabel: '最多筆數',
    retentionSave: '儲存策略',
    retentionResetDefault: '恢復預設',
    retentionSaved: (days, limit) => `保留策略已更新為最近 ${days} 天、最多 ${limit} 筆。`,
    retentionCleanNow: '刪除舊產物檔案',
    retentionRestore: '恢復隱藏記錄',
    retentionDeleteConfirm: count => `確認將 ${count} 個本機生成產物檔案移到垃圾桶？`,
    retentionDeleteSucceeded: count => `已將 ${count} 個生成產物檔案移到垃圾桶。`,
    retentionDeletePartial: (deleted, failed) => `已刪除 ${deleted} 個生成產物檔案，${failed} 個刪除失敗。`,
    itemsReport: '份報告',
    itemsImage: '張圖片',
    itemsLink: '個連結',
    itemsFile: '個檔案',
  ...
  sidebar: {
    ...
    projects: '專案',
    addProject: '新增專案',
    addProjectPathPrompt: '請輸入專案目錄路徑',
    noProjects: '新增專案目錄後，可從左側直接在該專案中啟動工作階段。',
    sessions: '工作階段',
    cronJobs: '自動化',
  ...
  composer: {
    ...
    placeholderStarting: '正在啟動 XCLAW...',
    placeholderReconnecting: '正在重新連線至 XCLAW…',
    ...
    attachUrlDesc: 'XCLAW 將擷取該頁面並作為此回合的脈絡。',
  ...
  updates: {
    ...
    unsupportedMessage: '此版本的 XCLAW 無法在應用程式內自行更新。',
    connectionRetry: '請檢查網路連線後重試。',
    latestBody: '您正在執行最新版本。',
    latestBodyRelease: '您正在執行最新的桌面客戶端版本。',
    ...
    guiSkewBody:
      '後端已更新，但此桌面應用程式套件未變更。請更新或重新安裝 XCLAW 桌面應用程式（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '複製',
    copied: '已複製',
    done: '完成',
    applyingBody: 'XCLAW 更新程式會在自己的視窗中接管，並在完成後自動重新開啟 XCLAW。更新期間請勿自行重新開啟 XCLAW。',
    applyingBodyBackend: '遠端後端正在套用更新並將重新啟動。恢復後 XCLAW 會自動重新連線。',
    applyingClose: '此視窗會在更新期間關閉，隨後 XCLAW 會自動重新開啟。',
    errorTitle: '更新未完成',
  ...
  install: {
    ...
    failedTitle: '安裝失敗',
    settingUpTitle: '正在設定 XCLAW Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某個安裝步驟失敗。在 Windows 上，如果另一個 XCLAW CLI 或桌面執行個體正在執行，可能會出現這種情況。請停止正在執行的 XCLAW 執行個體後重試。可查看下方的詳細資訊或 desktop 記錄中的完整記錄。',
    activeDesc: '這是一次性設定。XCLAW 安裝程式正在下載相依套件並設定您的電腦。之後啟動會略過此步驟。',
  ...
  onboarding: {
    headerTitle: '開始設定 XCLAW Agent',
    headerDesc: '連線模型提供方即可開始聊天。大多數選項只需一次點擊。',
    preparingInstall: 'XCLAW 正在完成安裝。首次執行通常不到一分鐘。',
    starting: '正在啟動 XCLAW…',
    ...
    recommended: '建議',
    connected: '已連線',
    featuredPitch: '一個訂閱，300+ 前沿模型 — 執行 XCLAW 的建議方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自託管',
        description: '將 XCLAW 指向本機或自託管的 OpenAI 相容端點（vLLM、llama.cpp、Ollama 等）。'
      }
    },
    backToSignIn: '返回登入',
    getKey: '取得金鑰',
    ...
    flowSubtitles: {
      ...
      device_code: '在瀏覽器中開啟驗證頁面 — XCLAW 會自動連線',
      loopback: '開啟瀏覽器登入 — XCLAW 會自動連線',
    ...
    authorizeThere: '請在那裡授權 XCLAW。',
    copyAuthCode: '複製授權碼並貼到下方。',
    pasteAuthCode: '貼上授權碼',
    reopenAuthPage: '重新開啟授權頁面',
    autoBrowser: provider => `已在瀏覽器中開啟 ${provider}。請在那裡授權 XCLAW，連線會自動完成，無需複製或貼上。`,
  ...
  preview: {
    ...
    openInVsCode: '用 VS Code 開啟',
    openInCursor: '用 Cursor 開啟',
    openInDefaultApp: '用預設應用程式開啟',
    chooseLocalApp: '選擇本機應用程式...',
    revealFile: '顯示檔案位置',
    expandPreview: '展開預覽',
    collapsePreview: '收合預覽',
    webPreview: '網頁',
    webPreviewMode: '以網頁形式預覽',
    sourceMode: '顯示原始碼預覽',
    openFailed: '無法開啟預覽目標',
    ...
    largeTitle: '此檔案較大',
    largeBody: (label, size) => `${label} 大小為 ${size}。XCLAW 只會顯示前 512 KB。`,
    previewAnyway: '仍然預覽',
    ...
    web: {
      ...
      failedToLoad: '預覽載入失敗',
      tryAgain: '重試',
      restarting: 'XCLAW 正在重新啟動...',
      askRestart: '請 XCLAW 重新啟動伺服器',
      lookingRestart: taskId => `XCLAW 正在尋找要重新啟動的預覽伺服器 (${taskId})`,
      restartingTitle: '正在重新啟動預覽伺服器',
      restartingMessage: 'XCLAW 正在背景執行。可在預覽主控台查看進度。',
      ...
      finishedRestarting: message => `XCLAW 已完成預覽伺服器重新啟動${message ? `：${message}` : ''}`,
      failedRestarting: message => `伺服器重新啟動失敗：${message}`,
      unknownError: '未知錯誤',
      restartedTitle: '預覽伺服器已重新啟動',
      reloadingNow: '正在重新載入預覽。',
      restartFailedTitle: '預覽重新啟動失敗',
      restartFailedMessage: 'XCLAW 無法重新啟動伺服器。',
      stillWorking: 'XCLAW 仍在執行，但尚未收到重新啟動結果。伺服器指令可能正在前台執行。',
  ...
  assistant: {
    ...
    approval: {
      gatewayDisconnected: 'XCLAW 閘道未連線',
      sendFailed: '無法傳送核准回應',
      run: '執行',
      command: '指令',
      moreOptions: '更多核准選項',
      ...
      alwaysDescription: pattern =>
        `這會將「${pattern}」模式加入永久允許清單（~/.vigil/config.yaml）。XCLAW 對類似指令將不再詢問，包括目前工作階段和未來工作階段。`,
      alwaysAllow: '一律允許'
    },
    clarify: {
      notReady: '澄清請求尚未就緒',
  ...
  prompts: {
    ...
    sudoTitle: '管理員密碼',
    sudoDesc: 'XCLAW 需要您的 sudo 密碼來執行特權指令。它只會傳送給本機執行端。',
    sudoPlaceholder: 'sudo 密碼',
    secretTitle: '需要密鑰',
    secretDesc: 'XCLAW 需要一個憑證才能繼續。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重新啟動 XCLAW Desktop 後可使用儲存圖片。',
    restartToSaveImages: '重新啟動 XCLAW Desktop 以儲存圖片',
