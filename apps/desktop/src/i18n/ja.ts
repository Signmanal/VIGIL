...
export const ja = defineLocale({
  ...
  settings: {
    ...
    model: {
      ...
      resetUnavailableAux: '利用できないタスクをリセット',
      auxiliaryProviderUnavailable: (count, tasks, provider) =>
        `${count} 件の補助タスク（${tasks}）は ${provider} を使用していますが、このプロバイダーは未接続または未設定です。`,
      auxiliaryDesc:
  ...
  skills: {
    ...
    marketSourcesTitle: '設定済みソース',
    marketSourceReady: '利用可能',
    marketSourceUnavailable: '利用不可',
    marketSourceRateLimited: 'レート制限中',
    marketFeaturedTitle: 'このソースのスキル',
    marketFeaturedDesc: '選択したソースのスキル一覧です。現在のプロファイルに直接インストールできます。',
  ...
  agents: {
    ...
    currentExpertTitle: '現在のアクティブエキスパート',
    currentExpertDesc: '現在のチャットで使われているプロファイルです。委任されたサブエキスパートは下に表示されます。',
    activeBadge: 'アクティブ',
    manageCurrentExpert: 'ロールを編集',
    currentExpertNoDescription: '説明はまだありません。ロール管理で SOUL.md またはプロファイル説明を編集してください。',
    skillMetric: (enabled, total) => `${enabled}/${total} Skill`,
    toolMetric: count => `${count} ツール`,
  ...
  profiles: {
    ...
    enabledSkills: (enabled, total) => `${enabled}/${total} 有効`,
    env: 'env',
    defaultBadge: 'デフォルト',
    rename: '名前を変更',
    copySetup: 'セットアップをコピー',
    setupCommandHint:
      'このプロファイルの CLI setup コマンドをコピーします。ターミナルに貼り付けて実行できます。スキルはマーケットカードからインストールしてください。',
    copying: 'コピー中...',
    modelLabel: 'モデル',
    skillsLabel: 'スキル',
    notSet: '未設定',
    ...
    editProfile: '専門家プロファイルを編集',
    editProfileDesc: '名前、開始時のスキル、MCP、SOUL.md を 1 つの下書きとしてまとめて保存します。',
    unsavedChanges: '未保存の変更',
    ...
    profileSkillsDesc: 'このプロファイル自身の skills ディレクトリから有効にするスキルを選択します。',
    startingMcpDesc: source => `${source} からコピーする MCP サーバーとツールを選択します。`,
    profileMcpDesc: 'このプロファイルで有効にする MCP サーバーとツールを選択します。',
    searchProfileSkills: 'スキル名、カテゴリ、説明を検索...',
    searchProfileMcp: 'MCP 名、コマンド、URL を検索...',
    loadingSkills: 'スキルを読み込み中...',
    loadingMcp: 'MCP サーバーを読み込み中...',
    noSkillsAvailable: 'このソースプロファイルに利用できるスキルはありません。',
    noSkillSearchResults: '一致するスキルはありません。',
    noMcpAvailable: 'このソースプロファイルに MCP サーバーは設定されていません。',
    noProfileMcpAvailable: 'このプロファイルに MCP サーバーは設定されていません。',
    noMcpSearchResults: '一致する MCP サーバーはありません。',
  ...
  sidebar: {
    ...
    addProjectPathPrompt: 'プロジェクトフォルダのパスを入力',
    noProjects: 'プロジェクトフォルダを追加すると、ここからセッションを開始できます。',
    sessions: 'セッション',
    cronJobs: '自動化',
  ...
  updates: {
    ...
    latestBodyRelease: '最新のデスクトップアプリ版を実行しています。',
