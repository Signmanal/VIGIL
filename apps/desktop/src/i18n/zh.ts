...
export const zh: Translations = {
  ...
  boot: {
    ...
    steps: {
      ...
      loadingSettings: '正在加载 VIGIL 设置',
      loadingSessions: '正在加载最近会话',
      startingDesktopConnection: '正在启动桌面连接',
      startingVIGILDesktop: '正在启动 VIGIL 桌面版…'
    },
    errors: {
      backgroundExited: 'VIGIL 后台进程已退出。',
      backgroundExitedDuringStartup: 'VIGIL 后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 VIGIL 后端早于当前桌面构建，可能无法正常工作。请更新以保持一致。',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 VIGIL Desktop。',
    ...
    native: {
      ...
      rejectAction: '拒绝',
      inputTitle: '需要输入',
      inputBody: 'VIGIL 正在等待你的回应。',
      turnDoneTitle: 'VIGIL 已完成',
      turnDoneBody: '回复已就绪。',
      turnErrorTitle: '本轮失败',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 VIGIL 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      providers: '提供方',
    ...
    notifications: {
      ...
      focusedHint: '完成提醒仅在 VIGIL 处于后台时触发。',
      kinds: {
        approval: {
          label: '需要批准',
        ...
        input: {
          label: '需要输入',
          description: 'VIGIL 提出了问题，或需要密码或密钥。'
        },
        turnDone: {
          label: '回复就绪',
          description: 'VIGIL 在后台时完成了一轮对话。'
        },
        turnError: {
          label: '本轮失败',
          description: '本轮以错误结束。'
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 VIGIL 跟随系统设置。',
      ...
      pet: {
        title: '宠物',
        intro:
          '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 VIGIL 的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
        restartHint: '宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 VIGIL，然后回到此处。',
        scaleTitle: '大小',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: '当 VIGIL 需要本地时间上下文时使用。留空则使用系统时区。',
      agent: {
        imageInputMode: '控制图片附件如何发送给模型。',
        maxTurns: 'VIGIL 停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: 'VIGIL 单次文件读取可读取的最大字符数。',
      approvals: {
        mode: 'VIGIL 如何处理需要显式审批的命令。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'VIGIL 从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
      }
    }),
    about: {
      ...
      automaticUpdatesDesc: 'VIGIL 会在后台自动检查更新，并在有可用更新时通知你。',
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'VIGIL Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 VIGIL 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
      ...
      localTitle: '本地网关',
      localDesc: '在 localhost 启动私有 VIGIL 后端。这是默认方式，并且可离线工作。',
      remoteTitle: '远程网关',
      remoteDesc:
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'VIGIL Desktop 将使用已保存设置重新连接。',
      savedMessage: '已保存，下一次重启生效。',
      connectedTo: (baseUrl, version) => `已连接到 ${baseUrl}${version ? ` · VIGIL ${version}` : ''}`,
    ...
    providers: {
      ...
      intro: '使用订阅登录，无需复制 API 密钥。VIGIL 会在应用中为你完成浏览器登录。',
      connected: '已连接',
      collapse: '收起',
  ...
  skills: {
    ...
    marketFeaturedTitle: '推荐技能',
    marketFeaturedDesc: '来自已配置 Skill Hub 的推荐结果，可直接安装到当前配置档案。',
  ...
  agents: {
    close: '关闭代理',
    title: 'Agent 工作台',
    subtitle: '监控运行中的子代理，并管理会话可选择的智能体角色。',
    manageRoles: '管理智能体角色',
    roleHint: '可复用智能体使用配置档案承载：编辑 SOUL.md 后，在对话侧栏选择对应档案即可切换角色。',
    emptyTitle: '暂无活跃子代理',
    emptyDesc: '当某个回合派发任务时，子代理会在此实时显示进度。开始会话前可通过角色切换助手人格。',
    running: '运行中',
    failed: '失败',
    done: '完成',
    streaming: '流式传输',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '请重启 VIGIL 以使用宠物功能——当前后端版本过旧。',
      empty: '没有匹配的宠物。',
      turnOff: '关闭',
      turnOn: '开启',
      installed: '已安装',
    ...
    generatePet: {
      ...
      staleBackend: '请更新 VIGIL 以生成宠物。',
      backgroundHint: '你可以关闭此窗口——完成后 VIGIL 会通知你。',
    ...
    nav: {
      ...
      settings: { title: '设置', detail: '配置 VIGIL 桌面端' },
    ...
    hermesActiveSessions: (version, count) => `VIGIL ${version} · 活跃会话 ${count}`,
    restartGateway: '重启网关',
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接，然后把 VIGIL 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 VIGIL 自带的 WhatsApp 桥接，首次运行时扫描二维码，然后启用该平台。',
      bluebubbles:
        '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 VIGIL 指向该 URL。',
  ...
  profiles: {
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 VIGIL 环境：各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
    cloneFrom: '克隆来源',
    cloneFromNone: '无（空白）',
  ...
  cron: {
    ...
    newCron: '新建已安排任务',
    emptyDescNew: '按 cron 表达式排程一个提示词。VIGIL 会运行它，并把结果发送到你选择的目的地。',
    ...
    created: '已安排任务已创建',
    updated: '已安排任务已更新',
    failedLoad: '加载已安排任务失败',
    failedUpdate: '更新已安排任务失败',
    failedTrigger: '触发已安排任务失败',
    failedDelete: '删除已安排任务失败',
    failedSave: '保存已安排任务失败',
    editTitle: '编辑已安排任务',
    createTitle: '新建已安排任务',
  ...
  artifacts: {
    ...
    noArtifactsDesc: '对话和已安排任务生成或引用的报告、图片、文件、链接会集中显示在这里。',
    failedLoad: '产物加载失败',
    openFailed: '打开失败',
    preview: '预览',
    previewFailed: '预览失败',
  ...
  composer: {
    ...
    placeholderStarting: '正在启动 VIGIL…',
    placeholderReconnecting: '正在重新连接 VIGIL…',
    ...
    attachUrlDesc: 'VIGIL 将抓取该页面并作为本回合的上下文。',
  ...
  updates: {
    ...
    manualTitle: '从终端更新',
    manualBody: '你是从命令行安装的 VIGIL，因此更新也需要在那里运行。请将此命令粘贴到终端：',
    manualPickedUp: '下次启动 VIGIL 时会使用新版本。',
    guiSkewTitle: '请更新桌面应用',
    guiSkewBody:
      '后端已更新，但此桌面应用包未更改。请更新或重新安装 VIGIL 桌面应用（你的 AppImage / .deb / .rpm）以保持一致。',
    copy: '复制',
    copied: '已复制',
    done: '完成',
    applyingBody: 'VIGIL 更新器会在自己的窗口中接管，并在完成后自动重新打开 VIGIL。更新期间请不要自行重新打开 VIGIL。',
    applyingBodyBackend: '远程后端正在应用更新并将重启。恢复后 VIGIL 会自动重新连接。',
    applyingClose: '此窗口会在更新期间关闭，随后 VIGIL 会自动重新打开。',
    errorTitle: '更新未完成',
  ...
  install: {
    ...
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 VIGIL Agent',
    finishingTitle: '正在收尾',
    failedDesc:
      '某个安装步骤失败。在 Windows 上，如果另一个 VIGIL CLI 或桌面实例正在运行，可能会出现这种情况。请停止正在运行的 VIGIL 实例后重试。可查看下面的详情或 desktop 日志中的完整记录。',
    activeDesc: '这是一次性设置。VIGIL 安装器正在下载依赖并配置你的机器。之后启动会跳过此步骤。',
  ...
  onboarding: {
    headerTitle: '开始设置 VIGIL Agent',
    headerDesc: '连接模型提供方即可开始对话。大多数选项只需一次点击。',
    preparingInstall: 'VIGIL 正在完成安装。首次运行通常不到一分钟。',
    starting: '正在启动 VIGIL…',
    ...
    recommended: '推荐',
    connected: '已连接',
    featuredPitch: '一个订阅，300+ 前沿模型 — 运行 VIGIL 的推荐方式',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: '自托管',
        description: '将 VIGIL 指向本地或自托管的 OpenAI 兼容端点 (vLLM、llama.cpp、Ollama 等)。'
      }
    },
    backToSignIn: '返回登录',
    getKey: '获取密钥',
    ...
    flowSubtitles: {
      ...
      device_code: '在浏览器中打开验证页面 — VIGIL 会自动连接',
      loopback: '打开浏览器登录 — VIGIL 会自动连接',
    ...
    authorizeThere: '请在那里授权 VIGIL。',
    copyAuthCode: '复制授权码并粘贴到下面。',
    pasteAuthCode: '粘贴授权码',
    reopenAuthPage: '重新打开授权页面',
    autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权 VIGIL，连接会自动完成，无需复制或粘贴。`,
  ...
  preview: {
    ...
    largeTitle: '此文件较大',
    largeBody: (label, size) => `${label} 大小为 ${size}。VIGIL 只会显示前 512 KB。`,
    previewAnyway: '仍然预览',
    ...
    web: {
      ...
      failedToLoad: '预览加载失败',
      tryAgain: '重试',
      restarting: 'VIGIL 正在重启...',
      askRestart: '让 VIGIL 重启服务器',
      lookingRestart: taskId => `VIGIL 正在查找要重启的预览服务器 (${taskId})`,
      restartingTitle: '正在重启预览服务器',
      restartingMessage: 'VIGIL 正在后台工作。可在预览控制台查看进度。',
      ...
      finishedRestarting: message => `VIGIL 已完成预览服务器重启${message ? `: ${message}` : ''}`,
      failedRestarting: message => `服务器重启失败：${message}`,
      unknownError: '未知错误',
      restartedTitle: '预览服务器已重启',
      reloadingNow: '正在重新加载预览。',
      restartFailedTitle: '预览重启失败',
      restartFailedMessage: 'VIGIL 无法重启服务器。',
      stillWorking: 'VIGIL 仍在工作，但还没有收到重启结果。服务器命令可能正在前台运行。',
  ...
  assistant: {
    ...
    approval: {
      gatewayDisconnected: 'VIGIL 网关未连接',
      sendFailed: '无法发送审批响应',
      run: '运行',
      command: '命令',
      moreOptions: '更多审批选项',
      ...
      alwaysDescription: pattern =>
        `这会将“${pattern}”模式加入永久允许列表 (~/.vigil/config.yaml)。VIGIL 对类似命令将不再询问，包括当前会话和未来会话。`,
      alwaysAllow: '始终允许'
    },
    clarify: {
      notReady: '澄清请求尚未就绪',
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: 'VIGIL 需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
    sudoPlaceholder: 'sudo 密码',
    secretTitle: '需要密钥',
    secretDesc: 'VIGIL 需要一个凭据才能继续。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重启 VIGIL 桌面版后可使用保存图片。',
    restartToSaveImages: '重启 VIGIL 桌面版以保存图片',
