...
import { BUILTIN_THEME_LIST, BUILTIN_THEMES, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, sentinelTheme } from './presets'
...
const SKIN_KEY = 'vigil-desktop-theme-v2'
const MODE_KEY = 'vigil-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'vigil-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'vigil-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'vigil-desktop-active-profile-v1'
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? sentinelTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const warm = skinName === 'sentinel' ? '#F59E0B' : c.primary

  root.style.setProperty('color-scheme', rendered)
  root.dataset.vigilTheme = skinName
  root.dataset.vigilMode = rendered
  ...
  window.vigilDesktop?.setTitleBarTheme?.({
  ...
  try {
    window.localStorage.setItem('vigil-boot-background', c.background)
    window.localStorage.setItem('vigil-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.vigilThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.vigilDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active VIGIL backend theme on connect. */
