...
// vigil_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - vigil_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - vigil_session_rt: the OAuth refresh token. Long-lived (24h rotating,
...
const AT_COOKIE_VARIANTS = ['__Host-vigil_session_at', '__Secure-vigil_session_at', 'vigil_session_at']
const RT_COOKIE_VARIANTS = ['__Host-vigil_session_rt', '__Secure-vigil_session_rt', 'vigil_session_rt']
