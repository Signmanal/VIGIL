...
test('canImportVIGILCli returns false when path is falsy', () => {
  assert.equal(canImportVIGILCli(''), false)
  assert.equal(canImportVIGILCli(null), false)
  assert.equal(canImportVIGILCli(undefined), false)
})
...
test('canImportVIGILCli returns false when interpreter cannot run -c', () => {
  ...
  assert.equal(canImportVIGILCli(NODE_BIN), false)
})
...
test('verifyVIGILCli returns false when command is falsy', () => {
  assert.equal(verifyVIGILCli(''), false)
  assert.equal(verifyVIGILCli(null), false)
  assert.equal(verifyVIGILCli(undefined), false)
})
...
test('verifyVIGILCli returns true when --version exits 0', () => {
  ...
  try {
    ...
    assert.equal(verifyVIGILCli(NODE_BIN), true)
  } finally {
    try {
...
test('verifyVIGILCli swallows timeouts (does not throw)', () => {
  // We can't easily provoke a real 5s hang in CI without slowing the
  // suite, but we CAN confirm that an invocation that DOES throw
  // (because the binary is missing) returns false rather than
  // propagating. Same code path the timeout case takes.
  assert.equal(verifyVIGILCli('/definitely/not/a/real/binary/anywhere'), false)
})
