#!/usr/bin/env node
// set-exe-identity.cjs — stamp the XCLAW icon + version metadata onto the
// built XCLAW.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked XCLAW.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.cjs <path-to-XCLAW.exe>
//
...
// Stamp the XCLAW icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'XCLAW',
      FileDescription: 'XCLAW',
      CompanyName: 'Signmanal',
      LegalCopyright: 'Copyright (c) 2026 Signmanal'
    }
  })

  console.log('[set-exe-identity] done — XCLAW icon + identity stamped')
}
