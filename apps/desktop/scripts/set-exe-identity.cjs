#!/usr/bin/env node
// set-exe-identity.cjs — stamp the VIGIL icon + version metadata onto the
// built VIGIL.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked VIGIL.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.cjs <path-to-VIGIL.exe>
//
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'VIGIL',
      FileDescription: 'VIGIL',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — VIGIL icon + identity stamped')
}
