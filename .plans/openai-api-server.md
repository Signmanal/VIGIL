# OpenAI-Compatible API Server for VIGIL Agent
...
SSE streaming. By exposing this endpoint, vigil-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  vigil-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.vigil/.env`
2. Run `vigil gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with vigil-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns vigil-agent as a model) |
...
Authorization: Bearer vigil-api-key-here
...
But vigil-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "vigil-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured vigil-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of vigil-agent's core prompt
...
Once implemented, vigil-agent works as a drop-in backend for:
