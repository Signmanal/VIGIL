...
def vigil_home(tmp_path, monkeypatch):
    ...
    # Some modules cache get_vigil_home; clear if needed.
    import vigil_constants
    if hasattr(vigil_constants, "_VIGIL_HOME_CACHE"):
        vigil_constants._VIGIL_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(vigil_home, monkeypatch):
...
def test_install_bws_happy_path(vigil_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(vigil_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(vigil_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from vigil_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under vigil_home/cache/."""
    home = tmp_path / ".vigil"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
