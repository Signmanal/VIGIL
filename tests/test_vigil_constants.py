"""Tests for vigil_constants module."""
...
from vigil_constants import (
    ...
    find_vigil_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_vigil_root,
    get_vigil_home,
    iter_vigil_node_dirs,
    is_container,
    parse_reasoning_effort,
    secure_parent_dir,
    with_vigil_node_path,
)


class TestGetDefaultVIGILRoot:
    """Tests for get_default_vigil_root() — Docker/custom deployment awareness."""

    def test_no_vigil_home_returns_native(self, tmp_path, monkeypatch):
        """When VIGIL_HOME is not set, returns ~/.vigil."""
        monkeypatch.delenv("VIGIL_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_vigil_root() == tmp_path / ".vigil"

    def test_vigil_home_is_native(self, tmp_path, monkeypatch):
        """When VIGIL_HOME = ~/.vigil, returns ~/.vigil."""
        native = tmp_path / ".vigil"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("VIGIL_HOME", str(native))
        assert get_default_vigil_root() == native

    def test_vigil_home_is_profile(self, tmp_path, monkeypatch):
        """When VIGIL_HOME is a profile under ~/.vigil, returns ~/.vigil."""
        native = tmp_path / ".vigil"
    ...
    def test_vigil_home_is_docker(self, tmp_path, monkeypatch):
        """When VIGIL_HOME points outside ~/.vigil (Docker), returns VIGIL_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("VIGIL_HOME", str(docker_home))
        assert get_default_vigil_root() == docker_home

    def test_vigil_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any VIGIL_HOME outside ~/.vigil is treated as the root."""
        custom = tmp_path / "my-vigil-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("VIGIL_HOME", str(custom))
        assert get_default_vigil_root() == custom
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("VIGIL_HOME", str(profile))
        assert get_default_vigil_root() == docker_root

    def test_no_vigil_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\vigil, not ~/.vigil."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("VIGIL_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(vigil_constants.sys, "platform", "win32")

        assert get_default_vigil_root() == local_appdata / "vigil"

    def test_no_vigil_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/vigil without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("VIGIL_HOME", raising=False)
        ...
        assert get_default_vigil_root() == home / "AppData" / "Local" / "vigil"


class TestGetVIGILHome:
    """Tests for get_vigil_home() platform-aware fallback."""
    ...
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(vigil_constants.sys, "platform", "win32")
        monkeypatch.setattr(vigil_constants, "_profile_fallback_warned", False)

        assert get_vigil_home() == local_appdata / "vigil"


class TestVIGILManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_vigil_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        assert find_vigil_node_executable("npm") == str(npm_cmd)
    ...
    def test_with_vigil_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "vigil"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_vigil_node_path({"PATH": "system-node"})
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(vigil_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(vigil_constants, "_container_detected", True)
        assert is_container() is True
