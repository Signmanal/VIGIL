...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    vigil_state = ModuleType("vigil_state")
    vigil_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "vigil_state", vigil_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "VIGIL"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "VIGIL"
