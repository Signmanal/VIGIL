...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/vigil\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/vigil"')
    cat_idx = block.find('cat > "$command_link_dir/vigil" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/vigil before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    shim_path = command_link_dir / "vigil"
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/vigil was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/vigil must be replaced with a regular file, not "
