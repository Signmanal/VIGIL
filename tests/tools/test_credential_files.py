...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        vigil_home.mkdir()
        (vigil_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(vigil_home / "token.json")
    ...
    def test_dict_with_name_key_fallback(self, tmp_path):
        ...
        vigil_home = tmp_path / ".vigil"
        vigil_home.mkdir()
        (vigil_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        vigil_home = tmp_path / ".vigil"
        vigil_home.mkdir()
        (vigil_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        vigil_home = tmp_path / ".vigil"
        vigil_home.mkdir()
        (vigil_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    ...
    def test_custom_container_base(self, tmp_path):
        vigil_home = tmp_path / ".vigil"
        (vigil_home / "skills").mkdir(parents=True)
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file one level above vigil_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside vigil_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside VIGIL_HOME must still be registered."""
        vigil_home = tmp_path / ".vigil"
        vigil_home.mkdir()
        monkeypatch.setenv("VIGIL_HOME", str(vigil_home))
        (vigil_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_vigil_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of VIGIL_HOME must be allowed."""
        vigil_home = tmp_path / ".vigil"
        vigil_home.mkdir()
        subdir = vigil_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file outside vigil_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside vigil_home pointing outside
        symlink = vigil_home / "evil_link.json"
        try:
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, vigil_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(vigil_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(vigil_home, [str(sensitive)])
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        ...
        (vigil_home / "oauth.json").write_text("{}")
        self._write_config(vigil_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        vigil_home = tmp_path / ".vigil"
        vigil_home.mkdir()
        (vigil_home / "cache" / "documents").mkdir(parents=True)
        (vigil_home / "cache" / "audio").mkdir(parents=True)
        (vigil_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("VIGIL_HOME", str(vigil_home))
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.vigil/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        vigil_home = tmp_path / ".vigil"
        vigil_home.mkdir()
        # Use legacy dir name — get_vigil_dir prefers old if it exists
        (vigil_home / "document_cache").mkdir()
        (vigil_home / "image_cache").mkdir()
        monkeypatch.setenv("VIGIL_HOME", str(vigil_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(vigil_home / "document_cache") in host_paths
        assert str(vigil_home / "image_cache") in host_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        vigil_home = tmp_path / ".vigil"
        img_dir = vigil_home / "cache" / "images"
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.vigil")
            == "/home/agent/.vigil/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        vigil_home = tmp_path / ".vigil"
        (vigil_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("VIGIL_HOME", str(vigil_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(vigil_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        vigil_home = tmp_path / ".vigil"
        doc_dir = vigil_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        vigil_home = tmp_path / ".vigil"
        ss_dir = vigil_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.vigil/cache/screenshots/session_abc/screen1.png"
