...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV VIGIL_TUI_DIR=/opt/vigil/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R hermes:hermes /opt/vigil/ui-tui" not in text
    assert "chown -R hermes:hermes /opt/vigil/node_modules" not in text
