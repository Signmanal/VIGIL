...
VIGIL_GATEWAY_BOOTSTRAP_STATE on first boot, so a freshly-provisioned
...
VIGIL_GATEWAY_BOOTSTRAP_STATE=running; stage2-hook.sh (installed as
/etc/cont-init.d/01-vigil-setup, which runs lexicographically BEFORE
...
This mirrors the existing VIGIL_AUTH_JSON_BOOTSTRAP env-seed pattern: it seeds
...
def test_seed_block_present_and_guarded(stage2_text: str) -> None:
    ...
    assert '[ ! -f "$VIGIL_HOME/gateway_state.json" ]' in block, (
        "seed must be guarded by [ ! -f ] so persisted state wins on restart"
    )
    assert "VIGIL_GATEWAY_BOOTSTRAP_STATE" in block
...
def _run_seed(
    ...
    """Run the extracted seed block in a sandbox $VIGIL_HOME.

    ``env_value`` is the VIGIL_GATEWAY_BOOTSTRAP_STATE value (None = unset).
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        env_line = (
            f'export VIGIL_GATEWAY_BOOTSTRAP_STATE="{env_value}"\n'
            if env_value is not None
            else "unset VIGIL_GATEWAY_BOOTSTRAP_STATE\n"
        )
        script = (
            "set -e\n"
            f'VIGIL_HOME="{home}"\n'
...
def test_no_seed_when_env_unset(stage2_text: str) -> None:
    ...
    assert out is None, "seed must not run when VIGIL_GATEWAY_BOOTSTRAP_STATE is unset"
