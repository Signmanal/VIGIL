...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    ...
    schema = registry.get_definitions(set(resolve_toolset("vigil-cli")), quiet=True)
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("VIGIL_KANBAN_TASK", "t_fake")
    home = tmp_path / ".vigil"
    home.mkdir()
    monkeypatch.setenv("VIGIL_HOME", str(home))
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the VIGIL_KANBAN_TASK env var means it's a focused
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["vigil_cli/kanban.py"],
...
def test_complete_stamps_worker_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("VIGIL_SESSION_ID", "session-trusted")
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    under an authoritative-looking name like ``vigil-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "vigil-system",
    })
    ...
    try:
        ...
        # caller-supplied "vigil-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_create_inherits_worker_dir_workspace(monkeypatch, worker_env):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("VIGIL_KANBAN_TASK", self_tid)
...
def test_create_no_worker_task_stays_scratch(monkeypatch, worker_env):
    """Orchestrator/CLI callers (no VIGIL_KANBAN_TASK) still default to
...
def test_create_session_id_arg_overrides_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("VIGIL_SESSION_ID", "from-env")
    from tools import kanban_tools as kt
    from vigil_cli import kanban_db as kb
    out = kt._handle_create({
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no VIGIL_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (VIGIL_KANBAN_TASK set) MUST have the full
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("VIGIL_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("VIGIL_KANBAN_RUN_ID", str(run2.id))
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning VIGIL_KANBAN_TASK
    to a task that exists in the alt board)."""
    home = tmp_path / ".vigil"
    home.mkdir()
    monkeypatch.setenv("VIGIL_HOME", str(home))
    monkeypatch.setenv("VIGIL_PROFILE", "alt-worker")
    monkeypatch.delenv("VIGIL_KANBAN_DB", raising=False)
    monkeypatch.delenv("VIGIL_KANBAN_BOARD", raising=False)
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("VIGIL_SESSION_THREAD_ID", "thread-7")
    monkeypatch.setenv("VIGIL_SESSION_USER_ID", "user-9")
...
def test_create_subscribes_tui_session_via_session_key(monkeypatch, worker_env):
    ...
    local channel), but the parent process exports VIGIL_SESSION_KEY.
    ...
    monkeypatch.setenv("VIGIL_SESSION_KEY", "tui-session-abc")
    monkeypatch.delenv("VIGIL_SESSION_ID", raising=False)
...
def test_create_partial_session_context_no_subscribe(monkeypatch, worker_env):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("VIGIL_SESSION_PLATFORM", "slack")
    monkeypatch.delenv("VIGIL_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("VIGIL_SESSION_KEY", raising=False)
    monkeypatch.delenv("VIGIL_SESSION_ID", raising=False)
