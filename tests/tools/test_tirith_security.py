...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._vigil_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(vigil_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(vigil_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_vigil_bin(self):
        """After _INSTALL_FAILED, manual install in VIGIL_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        vigil_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(vigil_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(vigil_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._vigil_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == vigil_bin
            assert _tirith_mod._resolved_path == vigil_bin
            mock_clear.assert_called_once()
...
class TestVIGILHomeIsolation:
    def test_vigil_bin_dir_respects_vigil_home(self):
        """_vigil_bin_dir must use VIGIL_HOME, not hardcoded ~/.vigil."""
        from tools.tirith_security import _vigil_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"VIGIL_HOME": tmpdir}):
            result = _vigil_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_vigil_home(self):
        """_failure_marker_path must use VIGIL_HOME, not hardcoded ~/.vigil."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"VIGIL_HOME": "/custom/vigil"}):
            result = _failure_marker_path()
        assert result == "/custom/vigil/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets VIGIL_HOME; verify it's active."""
        vigil_home = os.getenv("VIGIL_HOME")
        assert vigil_home is not None, "VIGIL_HOME should be set by conftest"
        assert "vigil_test" in vigil_home, "Should point to test temp dir"

    def test_get_vigil_home_fallback(self):
        """Without VIGIL_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_vigil_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove VIGIL_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".vigil")
            result = _get_vigil_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._vigil_bin_dir",
