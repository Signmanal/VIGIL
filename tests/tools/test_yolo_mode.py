...
class TestYoloMode:
    ...
    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for VIGIL_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("VIGIL_YOLO_MODE", "")
        monkeypatch.setenv("VIGIL_INTERACTIVE", "1")
        monkeypatch.setenv("VIGIL_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("VIGIL_YOLO_MODE") returns ""
