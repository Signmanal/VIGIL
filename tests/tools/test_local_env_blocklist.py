...
Verifies that VIGIL-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed VIGIL secrets.

See: https://github.com/NousResearch/vigil-agent/issues/1002
See: https://github.com/NousResearch/vigil-agent/issues/1264
"""
...
class TestProviderEnvBlocklist:
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        VIGIL-managed; the rest belongs to the user.
        """
        general_chain = {
...
class TestBlocklistCoverage:
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their VIGIL-managed
...
class TestVIGILBinDirOnPath:
    ...
    def test_resolves_via_sys_executable_dir(self, monkeypatch, tmp_path):
        ...
        (venv_bin / "vigil").write_text("#!/bin/sh\n")
    ...
    def test_make_run_env_injects_vigil_bin_dir(self, monkeypatch):
        ...
        assert entries[0] == "/opt/vigil/bin"
