...
def _restore_tool_modules():
    original_vigil_home = os.environ.get("VIGIL_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_vigil_home is None:
            os.environ.pop("VIGIL_HOME", None)
        else:
            os.environ["VIGIL_HOME"] = original_vigil_home
        _reset_modules(("tools", "vigil_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    vigil_cli = types.ModuleType("vigil_cli")
    vigil_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["vigil_cli"] = vigil_cli
    vigil_home = tmp_path / "vigil-home"
    os.environ["VIGIL_HOME"] = str(vigil_home)
    sys.modules["vigil_cli.config"] = types.SimpleNamespace(
        get_vigil_home=lambda: vigil_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="vigil-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": vigil_home / "modal_snapshots.json",
