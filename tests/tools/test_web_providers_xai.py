...
class TestXAIProviderIsAvailable:
    ...
    def test_available_via_auth_store(self, monkeypatch, tmp_path):
        """Cheap probe should detect xai-oauth tokens in ~/.vigil/auth.json
...
class TestXAIProviderOAuthPath:
    ...
    runtime resolver (``vigil auth`` sign-in) rather than an env-var key.
    Patches at the ``vigil_cli.runtime_provider.resolve_runtime_provider``
