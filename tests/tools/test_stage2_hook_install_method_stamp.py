...
into the shared $VIGIL_HOME, and must heal a stale 'docker' stamp left there
by older images.

Background (shared-$VIGIL_HOME bug)
------------------------------------
$VIGIL_HOME (/opt/data) is a DATA volume that users commonly bind-mount from
...
``/opt/vigil/.install_method`` in the Dockerfile, read first by
``detect_install_method``). stage2 must therefore:

  * NOT write the 'docker' stamp into $VIGIL_HOME any more, and
  * proactively remove a stale 'docker' stamp from $VIGIL_HOME so homes
...
def test_stage2_does_not_write_install_method_into_home(stage2_text: str) -> None:
    ...
    assert not re.search(
        r"(tee|>)\s*\"?\$VIGIL_HOME/\.install_method", stage2_text
    ), (
        "stage2 must not stamp $VIGIL_HOME/.install_method — that data dir "
...
def test_stage2_heals_stale_docker_home_stamp(stage2_text: str) -> None:
    # It must remove a stale 'docker' stamp from $VIGIL_HOME so already
    # poisoned shared homes recover.
    assert 'rm -f "$VIGIL_HOME/.install_method"' in stage2_text, (
        "stage2 must remove a stale 'docker' stamp from $VIGIL_HOME to heal "
