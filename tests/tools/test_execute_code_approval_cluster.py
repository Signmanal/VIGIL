...
def test_env_scrub_vigil_allowlist_and_secret_blocks():
    ...
    for kept in ("VIGIL_HOME", "VIGIL_PROFILE", "VIGIL_CONFIG", "VIGIL_ENV", "PATH"):
...
def test_env_scrub_logs_dropped_vigil_vars(caplog):
    """Dropping a non-allowlisted, non-secret VIGIL_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset VIGIL_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "VIGIL_HOME": "/h",          # allowlisted → kept, not logged
        "VIGIL_BASE_URL": "https://x",   # dropped → logged
        "VIGIL_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "VIGIL_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "VIGIL_HOME" in out and "PATH" in out
    assert "VIGIL_BASE_URL" not in out and "VIGIL_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "VIGIL_BASE_URL" in msgs and "VIGIL_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "VIGIL_API_KEY" not in msgs
