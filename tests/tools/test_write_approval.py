...
and the shared slash-command handlers (vigil_cli/write_approval_commands.py).
...
@pytest.fixture
def vigil_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="vigil_wa_test_")
...
def _set_approval(subsystem, enabled):
    import vigil_cli.config as cfg
    c = cfg.load_config()
...
def test_default_gate_is_off(vigil_home):
...
def test_invalid_subsystem_is_off(vigil_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(vigil_home):
...
def test_memory_gate_on_no_interactive_stages(vigil_home):
...
def test_memory_gate_on_then_apply(vigil_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(vigil_home, capsys):
...
def test_load_on_disk_store_honors_configured_char_limits(vigil_home, monkeypatch):
    ...
    monkeypatch.setattr("vigil_cli.config.load_config", _boom)
...
def test_skill_gate_off_allows_create(vigil_home):
...
def test_skill_gate_on_always_stages(vigil_home):
...
def test_skill_gate_on_then_apply_writes_file(vigil_home):
...
def test_skill_create_diff_is_full_content(vigil_home):
    from tools.skill_manager_tool import skill_manage
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(vigil_home):
    from tools import write_approval as wa
...
def test_handle_pending_list_empty(vigil_home):
    from vigil_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    out = handle_pending_subcommand(wa.MEMORY, ["pending"])
    assert "No pending memory" in out


def test_handle_approve_all(vigil_home):
    from vigil_cli.write_approval_commands import handle_pending_subcommand
...
def test_handle_reject(vigil_home):
    from vigil_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_approval_on(vigil_home):
    from vigil_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(vigil_home):
    from vigil_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_handle_mode_alias_still_works(vigil_home):
...
def test_handle_approval_invalid(vigil_home):
    from vigil_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_unknown_subcommand_returns_none(vigil_home):
    from vigil_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_memory_inline_approve_writes(vigil_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(vigil_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_callback_error_stages(vigil_home, approval_callback_cleanup):
...
def test_gateway_context_stages_not_prompts(vigil_home, monkeypatch):
...
def test_skills_never_prompt_inline_even_with_callback(vigil_home, approval_callback_cleanup):
...
def test_memory_invalid_params_rejected_before_staging(vigil_home):
