...
Hosted/container VIGIL keeps user-writable state under VIGIL_HOME
(/opt/data). The installed source, venv, TUI bundle, and node_modules under
/opt/vigil must remain root-owned/non-writable by the runtime vigil user so
...
def test_stage2_does_not_chown_install_tree_to_hermes(stage2_text: str) -> None:
    ...
    for install_tree in (
        ...
        assert install_tree not in stage2_text, (
            f"stage2 must not chown {install_tree} back to vigil; "
            "the Dockerfile keeps /opt/vigil immutable and writable state "
            "belongs under VIGIL_HOME"
        )
...
def test_stage2_documents_immutable_install_contract(stage2_text: str) -> None:
    ...
    assert "VIGIL_DISABLE_LAZY_INSTALLS=1" in stage2_text
    assert "/opt/vigil" in stage2_text
