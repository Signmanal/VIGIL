...
Before the s6 migration, ``docker run nousresearch/vigil-agent gateway
...
def test_gateway_run_no_supervise_env_var(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "VIGIL_GATEWAY_NO_SUPERVISE=1",
    ...
    if inspect.stdout.strip() == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "VIGIL_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${VIGIL_HOME}/logs/gateways/<profile>/current``.
