...
def test_dashboard_not_running_by_default(
    ...
    time.sleep(5)
    r = docker_exec(container_name, "pgrep", "-f", "vigil dashboard")
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without VIGIL_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `vigil doctor` and any other health check).
    ...
    VIGIL_DASHBOARD is unset, so the slot reflects reality.
    """
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name, built_image,
         "sleep", "60"],
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without VIGIL_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with VIGIL_DASHBOARD=1, s6-svstat reports the slot as up."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "VIGIL_DASHBOARD=1",
         # The default dashboard host is 0.0.0.0, which now engages the
         # OAuth auth gate. Without a provider registered (no
         # VIGIL_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
         ...
         "-e", "VIGIL_DASHBOARD_BASIC_AUTH_USERNAME=admin",
         "-e", "VIGIL_DASHBOARD_BASIC_AUTH_PASSWORD=test-dashboard-pw",
...
def test_dashboard_opt_in_starts(
    ...
    """With VIGIL_DASHBOARD=1, a dashboard process should be visible."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "VIGIL_DASHBOARD=1",
    ...
    ok, _ = _poll(
        container_name, "pgrep -f 'vigil dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with VIGIL_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """VIGIL_DASHBOARD_PORT changes the dashboard's listen port."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "VIGIL_DASHBOARD=1", "-e", "VIGIL_DASHBOARD_PORT=9120",
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "vigil dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as vigil, so the
    # vigil user can kill it (same UID).
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "VIGIL_DASHBOARD=1",
         "-e", "VIGIL_DASHBOARD_HOST=0.0.0.0",
         "-e", "VIGIL_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
         built_image, "sleep", "120"],
        check=True, capture_output=True, timeout=30,
    )
