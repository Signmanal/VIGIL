...
The shim (docker/vigil-exec-shim.sh, installed at /opt/vigil/bin/vigil)
...
failed: VIGIL is not logged into VIGIL Portal" on every message.
...
def test_shim_drops_root_to_vigil_uid(sleep_container: str) -> None:
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "vigil", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
...
def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user vigil already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "vigil", sleep_container,
         "vigil", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user vigil failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up vigil:vigil — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """VIGIL_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", "VIGIL_DOCKER_EXEC_AS_ROOT=1",
         sleep_container,
         "vigil", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "root:root", (
        f"With VIGIL_DOCKER_EXEC_AS_ROOT=1, expected root:root, "
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``VIGIL_DOCKER_EXEC_AS_ROOT=0``) doesn't
    silently keep the user as root. Mirrors the policy used by
    ``VIGIL_GATEWAY_NO_SUPERVISE`` in #33583.
    """
    subprocess.run(
        ["docker", "exec", "--user", "root", sleep_container,
         "rm", "-f", "/opt/data/config.yaml"],
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", f"VIGIL_DOCKER_EXEC_AS_ROOT={falsy_value}",
         sleep_container,
         "vigil", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "vigil:vigil", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected vigil:vigil"
    )
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    "VIGIL is not logged into VIGIL Portal" on every message.
    ...
    `docker exec` path produces is vigil-readable.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "vigil", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
