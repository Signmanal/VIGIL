...
class TestBlueBubblesHelpers:
    ...
    def test_default_mention_patterns_match_vigil_variants(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter.require_mention is True
        assert adapter._message_matches_mention_patterns("VIGIL, summarize this")
        assert adapter._message_matches_mention_patterns("@VIGIL agent help")
    ...
    def test_custom_mention_patterns_override_defaults(self, monkeypatch):
        ...
        assert not adapter._message_matches_mention_patterns("VIGIL what is next?")

    def test_clean_mention_text_strips_leading_wake_word(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter._clean_mention_text("VIGIL, summarize this") == "summarize this"
        assert adapter._clean_mention_text("VIGIL agent: summarize this") == "summarize this"
        assert adapter._clean_mention_text("please ask VIGIL about this") == "please ask VIGIL about this"
