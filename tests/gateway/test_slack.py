...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@vigil`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@vigil"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@vigil")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /vigil btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_vigil_prefix_still_works(self, adapter):
    ...
    @pytest.mark.asyncio
    async def test_legacy_vigil_freeform_question(self, adapter):
        """/vigil <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/vigil",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_vigil_slash_stashes_context(self, adapter):
        """Legacy /vigil <subcommand> also stashes context."""
        command = {
            "command": "/vigil",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_vigil_question_does_not_stash_context(self, adapter):
        """Free-form /vigil <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/vigil",
