...
class TestIRCProtocolHelpers:
    ...
    def test_parse_numeric_reply(self):
        msg = _parse_irc_message(":server 001 vigil-bot :Welcome to IRC")
        assert msg["prefix"] == "server"
        assert msg["command"] == "001"
        assert msg["params"] == ["vigil-bot", "Welcome to IRC"]

    def test_parse_nick_collision(self):
        msg = _parse_irc_message(":server 433 * vigil-bot :Nickname is already in use")
...
class TestIRCAdapterInit:
    ...
    def test_init_from_config_extra(self, monkeypatch):
        ...
        assert adapter.nickname == "vigil"
        assert adapter.channel == "#vigil-dev"
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_welcome(self, adapter):
        ...
        await adapter._handle_line(":server 001 vigil :Welcome to IRC")
    ...
    async def test_handle_nick_collision(self, adapter):
        ...
        await adapter._handle_line(":server 433 * vigil :Nickname in use")
        assert adapter._current_nick == "vigil_"
        sent = writer.write.call_args[0][0]
        assert b"NICK vigil_" in sent
    ...
    async def test_handle_dm(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG vigil :private message")
        assert len(dispatched) == 1
    ...
    async def test_ignores_own_messages(self, adapter):
        ...
        await adapter._handle_line(":vigil!bot@host PRIVMSG #test :my own msg")
        assert len(dispatched) == 0
    ...
    async def test_allowed_users_case_insensitive(self, monkeypatch):
        ...
        await adapter._handle_line(":admin!u@host PRIVMSG #test :vigil: hello")
        assert len(dispatched) == 1
    ...
    async def test_nick_collision_retry(self, adapter):
        ...
        await adapter._handle_line(":server 433 * vigil :Nickname in use")
        assert adapter._current_nick == "vigil_"
        await adapter._handle_line(":server 433 * vigil_ :Nickname in use")
        assert adapter._current_nick == "vigil_1"
        await adapter._handle_line(":server 433 * vigil_1 :Nickname in use")
        assert adapter._current_nick == "vigil_2"
