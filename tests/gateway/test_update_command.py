...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        runner = _make_runner()
        event = _make_event()
        monkeypatch.setenv("VIGIL_MANAGED", "homebrew")

        # Guard: prevent any accidental fall-through from spawning a real
        # `vigil update --gateway` against the CI checkout. The managed-install
        ...
        assert "brew upgrade vigil-agent" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _vigil_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._vigil_home", tmp_path), \
        ...
        with patch("gateway.run._vigil_home", tmp_path):
            ...
            # project_root will be the real vigil-agent dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_no_vigil_binary(self, tmp_path):
        """Returns error when vigil is not on PATH and vigil_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "vigil update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m vigil_cli.main when vigil not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        with patch("gateway.run._vigil_home", vigil_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", return_value=None), \
        ...
        # The update_cmd uses sys.executable -m vigil_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "vigil_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_vigil_bin_prefers_which(self, tmp_path):
        """_resolve_vigil_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_vigil_bin

        with patch("shutil.which", return_value="/custom/path/vigil"):
            result = _resolve_vigil_bin()

        assert result == ["/custom/path/vigil"]

    @pytest.mark.asyncio
    async def test_resolve_vigil_bin_fallback(self):
        """_resolve_vigil_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_vigil_bin
        ...
        assert result == [sys.executable, "-m", "vigil_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_vigil_bin_returns_none_when_both_fail(self):
        """_resolve_vigil_bin returns None when both strategies fail."""
        from gateway.run import _resolve_vigil_bin
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._vigil_home", vigil_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/vigil" if x == "vigil" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = vigil_home / ".update_pending.json"
        ...
        assert not (vigil_home / ".update_exit_code").exists()
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((vigil_home / ".update_pending.json").read_text())
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (vigil_home / ".update_pending.json").exists()
        assert not (vigil_home / ".update_exit_code").exists()
...
class TestUpdateCommandPlatformGate:
    ...
    async def test_allows_plugin_platform_via_registry_fallback(self, monkeypatch):
        ...
        from vigil_cli.plugins import PluginManager
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (vigil_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = vigil_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (vigil_home / ".update_output.txt").write_text("done")
        (vigil_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (vigil_home / ".update_pending.json").write_text(json.dumps(pending))
        (vigil_home / ".update_output.txt").write_text(
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (vigil_home / ".update_pending.json").write_text(json.dumps(pending))
        (vigil_home / ".update_output.txt").write_text("x" * 5000)
        (vigil_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (vigil_home / ".update_pending.json").write_text(json.dumps(pending))
        (vigil_home / ".update_output.txt").write_text("Traceback: boom")
        (vigil_home / ".update_exit_code").write_text("1")
    ...
    async def test_cleans_up_files_after_notification(self, tmp_path):
        ...
        pending_path = vigil_home / ".update_pending.json"
        output_path = vigil_home / ".update_output.txt"
        exit_code_path = vigil_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
    ...
    async def test_no_adapter_for_platform_preserves_markers(self, tmp_path):
        ...
        assert not (vigil_home / ".update_pending.claimed.json").exists()
