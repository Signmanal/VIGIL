"""Tests for VIGILCLI initialization -- catches configuration bugs
...
def _make_cli(env_overrides=None, config_overrides=None, **kwargs):
    """Create a VIGILCLI instance with minimal mocking."""
    import importlib
    ...
    with patch.dict(sys.modules, prompt_toolkit_stubs), \
        ...
        with patch.object(_cli_mod, "get_tool_definitions", return_value=[]), \
            ...
            return _cli_mod.VIGILCLI(**kwargs)
...
class TestMaxTurnsResolution:
    ...
    def test_env_var_max_turns(self):
        ...
        cli_obj = _make_cli(env_overrides={"VIGIL_MAX_ITERATIONS": "42"})
        assert cli_obj.max_turns == 42

    def test_invalid_env_var_max_turns_falls_back_to_default(self):
        """Invalid env values should not crash CLI init."""
        cli_obj = _make_cli(env_overrides={"VIGIL_MAX_ITERATIONS": "not-a-number"})
...
class TestHistoryDisplay:
    ...
    def test_resume_updates_hermes_session_id_env_and_context(self, tmp_path):
        ...
        os.environ["VIGIL_SESSION_ID"] = "current_session"
        _VAR_MAP["VIGIL_SESSION_ID"].set("current_session")

        try:
            cli._handle_resume_command("/resume target_session")

            assert cli.session_id == "target_session"
            assert os.environ["VIGIL_SESSION_ID"] == "target_session"
            assert get_session_env("VIGIL_SESSION_ID") == "target_session"
        finally:
