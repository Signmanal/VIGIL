...
Regression: the old implementation wrote ``vigil_conversation_<ts>.json``
...
def vigil_home(tmp_path, monkeypatch):
    ...
    # Clear any cached vigil_home computation
    import vigil_constants
    if hasattr(vigil_constants, "_vigil_home_cache"):
        vigil_constants._vigil_home_cache = None
    return home
...
def test_save_conversation_writes_under_vigil_home(vigil_home, tmp_path, monkeypatch, capsys):
    ...
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "vigil_constants"]:
    ...
    cli.VIGILCLI.save_conversation(stub)
    ...
    saved_dir = vigil_home / "sessions" / "saved"
...
def test_save_conversation_empty_history_does_nothing(vigil_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "vigil_constants"]:
        sys.modules.pop(mod, None)
    import cli
