...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("vigil/vigil-")
...
class TestOrphanedBranchPruning:
    ...
    def test_prunes_orphaned_vigil_branch(self, git_repo):
        """vigil/vigil-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "vigil/vigil-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "vigil/vigil-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "vigil/vigil-deadbeef" in result.stdout
        ...
        assert "vigil/vigil-deadbeef" in orphaned
        ...
        assert "vigil/vigil-deadbeef" not in result.stdout
