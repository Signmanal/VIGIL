...
def test_ttfb_default_tolerates_slow_first_event(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("VIGIL_CODEX_TTFB_TIMEOUT_SECONDS", raising=False)
    monkeypatch.delenv("VIGIL_CODEX_TTFB_MAX_SECONDS", raising=False)
...
def test_ttfb_high_env_is_capped_for_openai_codex(tmp_path, monkeypatch):
    ...
    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("VIGIL_CODEX_TTFB_TIMEOUT_SECONDS", "90")
    monkeypatch.setenv("VIGIL_CODEX_TTFB_MAX_SECONDS", "1")
...
def test_event_idle_kills_after_first_event_then_silence(tmp_path, monkeypatch):
    ...
    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("VIGIL_CODEX_TTFB_TIMEOUT_SECONDS", "10")
    monkeypatch.setenv("VIGIL_CODEX_EVENT_STALE_TIMEOUT_SECONDS", "1")
...
def test_ttfb_disabled_via_env_zero(tmp_path, monkeypatch):
    """Setting VIGIL_CODEX_TTFB_TIMEOUT_SECONDS=0 disables the TTFB watchdog;
    a no-event stall then falls through to the (here, 60s) stale timeout, so a
    short hang is NOT killed by TTFB."""
    from agent import chat_completion_helpers as h

    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("VIGIL_CODEX_TTFB_TIMEOUT_SECONDS", "0")
...
def test_huge_codex_request_waits_instead_of_ttfb_reconnect(tmp_path, monkeypatch):
    """Huge Codex inputs can legitimately take longer than the small-request
    ...
    large_input = "x" * 420_000  # ~105k estimated tokens, above huge-request gate.
    resp = h.interruptible_api_call(agent, {"model": "gpt-5.5", "input": large_input})
    assert resp is sentinel
    assert "codex_ttfb_kill" not in closes


def test_document_sized_codex_request_keeps_ttfb_watchdog(tmp_path, monkeypatch):
    """Document-sized desktop turns (~30k tokens) should still reconnect on a
    no-byte stall instead of waiting for the provider read timeout."""
    from agent import chat_completion_helpers as h
    ...
    document_input = "x" * 120_000  # ~30k estimated tokens.
    try:
        with pytest.raises(TimeoutError) as excinfo:
            h.interruptible_api_call(agent, {"model": "gpt-5.5", "input": document_input})
        assert "TTFB threshold: 1s" in str(excinfo.value)
        assert "codex_ttfb_kill" in closes
    finally:
        stop["flag"] = True
...
def test_large_codex_request_strict_ttfb_env_still_reconnects(tmp_path, monkeypatch):
    ...
    with VIGIL_CODEX_TTFB_STRICT=1."""
    from agent import chat_completion_helpers as h

    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("VIGIL_CODEX_TTFB_TIMEOUT_SECONDS", "1")
    monkeypatch.setenv("VIGIL_CODEX_TTFB_STRICT", "1")
