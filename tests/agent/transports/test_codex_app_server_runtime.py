...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("VIGIL_HOME", "/users/alice/.vigil/profiles/backend-worker")
        monkeypatch.setenv("VIGIL_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "VIGIL_KANBAN_DB",
            "/users/alice/.vigil/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.vigil/kanban/boards/smoke"]'
            in cmd
        )
