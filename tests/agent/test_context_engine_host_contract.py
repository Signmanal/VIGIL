...
engine plugins (e.g. vigil-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``vigil plugins`` picker."""
    from vigil_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "vigil_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "vigil_cli.plugins.get_plugin_context_engine",
