...
""",
        ...
        from vigil_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        vigil_home = tmp_path / ".vigil"
        vigil_home.mkdir()
        (vigil_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("VIGIL_HOME", str(vigil_home))
        from vigil_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from vigil_cli.plugins import PluginContext, PluginManifest, PluginManager
