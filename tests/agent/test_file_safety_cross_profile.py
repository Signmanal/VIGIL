"""Tests for the cross-VIGIL-profile write guard in agent/file_safety.
...
Reference: May 2026 incident — a vigil-security profile session
accidentally edited skills under both ~/.vigil/profiles/vigil-security/skills/
...
def _set_active_home(monkeypatch, vigil_home: Path):
    """Point file_safety._vigil_home_path at a specific profile dir."""
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_vigil_home_path", lambda: vigil_home)
...
class TestResolveActiveProfileName:
    ...
    def test_named_profile(self, fake_hermes, monkeypatch):
        ...
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "vigil-security"
    ...
    def test_falls_back_to_default_on_resolution_failure(self, fake_hermes, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_vigil_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_hermes, monkeypatch):
        ...
        assert result["active_profile"] == "vigil-security"
    ...
    def test_default_writing_security_skill(self, fake_hermes, monkeypatch):
        ...
        assert result["target_profile"] == "vigil-security"
    ...
    def test_non_vigil_path_returns_none(self, fake_hermes, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_hermes["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any VIGIL root
        assert classify_cross_profile_target(str(tmp_path / "random.txt")) is None

    def test_vigil_config_not_classified_as_cross_profile(self, fake_hermes, monkeypatch):
