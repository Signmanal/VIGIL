"""Tests for vigil_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every VIGIL entry point
(vigil, vigil-agent, vigil-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every VIGIL entry point imports vigil_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported vigil_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("vigil_bootstrap", None)
    import vigil_bootstrap  # noqa: WPS433
    return vigil_bootstrap
...
class TestEntryPointsImportBootstrap:
    """Every VIGIL entry point must import vigil_bootstrap as its
    ...
    # import vigil_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "vigil_cli/main.py",   # vigil CLI (console_script)
        "run_agent.py",          # vigil-agent (console_script)
        "acp_adapter/entry.py",  # vigil-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import vigil_bootstrap' and that
        ...
        ``vigil_bootstrap`` as a top-level module) leaves vigil
        ...
        """
        # Resolve relative to the vigil-agent repo root.  Tests live
        # at tests/test_vigil_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # vigil start even when vigil_bootstrap hasn't been
        ...
        assert first_import_name == "vigil_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'vigil_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import vigil_bootstrap' line to be the first import."
        )
...
class TestHardenImportPath:
    ...
    def _run(self, hb, path_seed, env=None):
        ...
        try:
            ...
            hb.harden_import_path(src_root="/opt/vigil")
            return sys.path[:]
        finally:
            sys.path[:] = original
    ...
    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/vigil", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/vigil"])
        assert result[0] == "/opt/vigil"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the VIGIL root.
        assert result.index("/opt/vigil") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/vigil", "/opt/vigil", ""])
        assert result.count("/opt/vigil") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["VIGIL_PYTHON_SRC_ROOT"] = "/env/vigil"
            hb.harden_import_path()
            assert sys.path[0] == "/env/vigil"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("VIGIL_PYTHON_SRC_ROOT", None)
