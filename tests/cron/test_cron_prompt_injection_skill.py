...
def cron_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("VIGIL_BUNDLES_DIR", str(vigil_home / "skill-bundles"))
    ...
    monkeypatch.setattr(_skills_tool, "VIGIL_HOME", vigil_home)
    ...
    return vigil_home, _scheduler


def _plant_skill(vigil_home: Path, name: str, body: str) -> None:
    """Drop a SKILL.md into ~/.vigil/skills/<name>/ bypassing skills_guard."""
    skill_dir = vigil_home / "skills" / name
...
def _plant_bundle(vigil_home: Path, name: str, skills: list[str], instruction: str = "") -> None:
    """Drop a bundle YAML into ~/.vigil/skill-bundles/ and refresh cache."""
    bundles_dir = vigil_home / "skill-bundles"
...
class TestBuildJobPromptScansSkillContent:
    def test_clean_skill_builds_normally(self, cron_env):
        vigil_home, scheduler = cron_env
        _plant_skill(vigil_home, "news-digest", "Fetch the top 5 headlines and summarize.")
    ...
    def test_skill_with_env_exfil_command_in_prose_is_allowed(self, cron_env):
        ...
        false positive in the bundled `vigil-agent-dev` skill that
    ...
    def test_skill_with_invisible_unicode_sanitized_not_blocked(self, cron_env):
        ...
        _plant_skill(vigil_home, "zwsp-skill", "clean looking\u200bskill content")
    ...
    def test_skill_bundle_in_job_skills_loads_referenced_skills(self, cron_env):
        vigil_home, scheduler = cron_env
        _plant_skill(vigil_home, "alpha-skill", "Alpha guidance for the cron task.")
        _plant_skill(vigil_home, "beta-skill", "Beta guidance for the cron task.")
        _plant_bundle(
            vigil_home,
            "article-pipeline",
    ...
    def test_bundle_name_shadows_skill_name_for_cron_jobs(self, cron_env):
        vigil_home, scheduler = cron_env
        _plant_skill(vigil_home, "article-pipeline", "Standalone skill should not win.")
        _plant_skill(vigil_home, "bundle-member", "Bundle member should win.")
        _plant_bundle(vigil_home, "article-pipeline", ["bundle-member"])
...
class TestScriptOutputNotStrictScanned:
    ...
    Live incident: the ``vigil-triage`` cron was blocked every 5 minutes
    ...
    def test_command_shapes_in_context_from_output_not_blocked(self, cron_env, monkeypatch):
        ...
        output_root = vigil_home / "cron" / "output"
