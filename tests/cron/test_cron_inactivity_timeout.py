...
class TestInactivityTimeout:
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """VIGIL_CRON_TIMEOUT=0 yields None (unlimited)."""
    ...
    def test_timeout_invalid_value_falls_back_to_default(self, monkeypatch):
        """VIGIL_CRON_TIMEOUT=abc should fall back to 600s, not raise ValueError."""
        monkeypatch.setenv("VIGIL_CRON_TIMEOUT", "abc")
        raw = os.getenv("VIGIL_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
    ...
    def test_timeout_empty_string_uses_default(self, monkeypatch):
        """VIGIL_CRON_TIMEOUT='' (empty) should use the 600s default."""
        monkeypatch.setenv("VIGIL_CRON_TIMEOUT", "")
        raw = os.getenv("VIGIL_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        assert _cron_timeout == 600.0
...
class TestSysPathOrdering:
    ...
    def test_vigil_time_importable(self):
        """vigil_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _vigil_now
        assert callable(_vigil_now)

    def test_vigil_constants_importable(self):
        """vigil_constants should be importable from cron context."""
        from vigil_constants import get_vigil_home
        assert callable(get_vigil_home)
