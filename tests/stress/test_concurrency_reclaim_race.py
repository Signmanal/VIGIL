...
def worker_loop(worker_id: int, vigil_home: str, result_file: str) -> None:
...
def reclaimer_loop(vigil_home: str, result_file: str) -> None:
    os.environ["VIGIL_HOME"] = vigil_home
    os.environ["HOME"] = vigil_home
    sys.path.insert(0, WT)
    from vigil_cli import kanban_db as kb
...
def main():
    home = tempfile.mkdtemp(prefix="vigil_reclaim_race_")
