"""Tests for ACP Registry metadata shipped with VIGIL."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "vigil-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "VIGIL Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/vigil-agent"
    assert data["website"].startswith("https://vigil-agent.nousresearch.com/")
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"vigil-agent[acp]=={_pyproject_version()}"
    )
