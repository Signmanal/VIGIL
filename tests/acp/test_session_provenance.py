...
def test_root_session_no_compression(db):
    ...
    assert prov["currentVIGILSessionId"] == "root1"
    assert prov["rootVIGILSessionId"] == "root1"
    assert prov["parentVIGILSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_vigil_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentVIGILSessionId"] == "old"
    assert prov["rootVIGILSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousVIGILSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootVIGILSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootVIGILSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_vigil_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousVIGILSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"vigil"}
    assert "sessionProvenance" in meta["vigil"]
    assert meta["vigil"]["sessionProvenance"]["currentVIGILSessionId"] == "root1"
