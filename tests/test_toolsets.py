...
class TestToolsetConsistency:
    ...
    def test_vigil_platforms_share_core_tools(self):
        """All vigil-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on vigil-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["vigil-cli", "vigil-telegram", "vigil-discord", "vigil-whatsapp", "vigil-slack", "vigil-signal", "vigil-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_vigil_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("vigil-whatsapp")

    def test_vigil_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("vigil-api-server")
