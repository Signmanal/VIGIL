"""Shared fixtures for the vigil-agent test suite.
...
2. **Isolated VIGIL_HOME.** VIGIL_HOME points to a per-test tempdir so
   code reading ``~/.vigil/*`` via ``get_vigil_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".vigil"`` instead of the canonical
   ``get_vigil_home()`` is a bug to fix at the callsite.)
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    # 3. Redirect VIGIL_HOME to a per-test tempdir. Code that reads
    #    ``~/.vigil/*`` via ``get_vigil_home()`` now gets the tempdir.
    #
    ...
    #    ``Path.home() / ".vigil"`` instead of ``get_vigil_home()``
    #    is a bug to fix at the callsite.
    fake_vigil_home = tmp_path / "vigil_test"
    fake_vigil_home.mkdir()
    (fake_vigil_home / "sessions").mkdir()
    (fake_vigil_home / "cron").mkdir()
    (fake_vigil_home / "memories").mkdir()
    (fake_vigil_home / "skills").mkdir()
    monkeypatch.setenv("VIGIL_HOME", str(fake_vigil_home))
    ...
    try:
        import vigil_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_vigil_home(_hermetic_environment):
...
# environment and finds the developer's live ``vigil-gateway`` process
...
# correlated with a ``tests/vigil_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> vigil-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart vigil-gateway"``,
    ...
    def _matches_vigil_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
    ...
    def _is_blocked_systemctl(cmd) -> bool:
        ...
        if "systemctl" not in cmd_str:
            return False
        if not _matches_vigil_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch vigil-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m vigil_cli.main".
                if (
                    "vigil" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live vigil-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting vigil/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `vigil update` (or the
        # equivalent `python -m vigil_cli.main update`).  These commands
        ...
        if "update" in low and (
            ...
            ("vigil" in low and "update" in low.split())
            or
            # python -m vigil_cli.main update --gateway
            ("vigil_cli" in low and "update" in low.split())
            or
            # venv/bin/vigil update  (absolute path variant used in tests)
            (".venv/bin/vigil" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
