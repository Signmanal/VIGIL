...
guidance in the ``vigil-agent-dev`` skill before changing the
expectations.
"""
...
def test_camofox_is_not_in_root_dependencies() -> None:
    ...
    assert "@askjo/camofox-browser" not in deps, (
        ...
        "Camofox post_setup handler in vigil_cli/tools_config.py so it "
        "only installs when the user explicitly selects Camofox via "
        "`vigil tools` → Browser Automation → Camofox."
    )
...
def test_agent_browser_stays_eager() -> None:
    ...
    assert "agent-browser" in deps, (
        ...
        "after `vigil setup` / `vigil update` without an explicit "
        "post_setup step."
    )
