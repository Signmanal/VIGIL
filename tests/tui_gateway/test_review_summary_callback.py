...
def server():
    with patch.dict(
        "sys.modules",
        {
            "vigil_constants": MagicMock(
                get_vigil_home=MagicMock(return_value="/tmp/vigil_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'vigil-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'vigil-release' patched"
    }
