...
shared ``vigil_bootstrap.harden_import_path`` guard.
...
def test_entry_calls_shared_harden_guard_before_heavy_imports():
    """entry.py must call vigil_bootstrap.harden_import_path() before it
    ...
    assert harden_call_line is not None, (
        "entry.py must call vigil_bootstrap.harden_import_path()"
    )
...
def test_entry_does_not_reimplement_guard_inline():
    ...
    assert '{"", "."}' not in source and "{'', '.'}" not in source, (
        "entry.py should delegate to vigil_bootstrap.harden_import_path, "
...
def test_guard_handles_absolute_cwd_path():
    ...
    original = sys.path[:]
    try:
        sys.path[:] = ["/home/user/tg-ws-proxy", "/opt/vigil", "/usr/lib"]
        vigil_bootstrap.harden_import_path(src_root="/opt/vigil")
        assert sys.path[0] == "/opt/vigil"
        assert sys.path.index("/opt/vigil") < sys.path.index(
