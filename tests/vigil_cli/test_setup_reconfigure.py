...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("vigil_cli.setup.ensure_vigil_home", {}),
        ("vigil_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("vigil_cli.config.is_managed", {"return_value": False}),
        ("vigil_cli.setup.load_config", {"return_value": {}}),
        ("vigil_cli.setup.save_config", {}),
        ("vigil_cli.setup.get_env_value", {"return_value": None}),
        ("vigil_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("vigil_cli.setup._print_setup_summary", {}),
        ("vigil_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("vigil_cli.setup.load_config", {"return_value": {}}),
        ("vigil_cli.setup.save_config", {}),
        ("vigil_cli.auth.get_active_provider", {"return_value": None}),
        ("vigil_cli.setup.get_env_value", {"return_value": None}),
        ("vigil_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="vigil_cli.setup.prompt_choice",
                quick="vigil_cli.setup._run_quick_setup",
                model="vigil_cli.setup.setup_model_provider",
                terminal="vigil_cli.setup.setup_terminal_backend",
                agent="vigil_cli.setup.setup_agent_settings",
                gateway="vigil_cli.setup.setup_gateway",
                tools="vigil_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`vigil setup --reconfigure` behaves the same as bare `vigil setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("vigil_cli.setup.prompt_choice", {"return_value": 0}),
                first="vigil_cli.setup._run_first_time_quick_setup",
            )
            from vigil_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "vigil_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["vigil", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["vigil", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["vigil", "setup"])
        try:
            main()
        except SystemExit:
            pass
