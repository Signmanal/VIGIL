...
def container_env(tmp_path, monkeypatch):
    ...
    container_mode = vigil_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=vigil-agent\n"
        "exec_user=vigil\n"
        "vigil_bin=/data/current-package/bin/vigil\n"
    )
    return vigil_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("vigil_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "vigil-agent"
    assert info["exec_user"] == "vigil"
    assert info["vigil_bin"] == "/data/current-package/bin/vigil"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("vigil_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_vigil_dev(container_env, monkeypatch):
...
def test_get_container_exec_info_not_skipped_when_vigil_dev_zero(container_env, monkeypatch):
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        vigil_home = Path(tmpdir) / ".vigil"
        vigil_home.mkdir()
        (vigil_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("vigil_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_vigil_home": lambda: vigil_home}), \
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=vigil-custom\n"
        "exec_user=myuser\n"
        "vigil_bin=/opt/vigil/bin/vigil\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "vigil-custom"
    assert info["exec_user"] == "myuser"
    assert info["vigil_bin"] == "/opt/vigil/bin/vigil"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from vigil_cli.main import _exec_in_container
