...
from vigil_cli.dashboard_auth import clear_providers, register_provider
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        r = gated_app_proxied.get(
            "/sessions",
            headers={"x-forwarded-prefix": "/vigil"},
...
class TestPublicUrlOverride:
    ...
    def patch_config(self, monkeypatch):
        """Replace ``vigil_cli.config.load_config`` with a stub
        ...
        def _set(public_url) -> None:
            ...
            monkeypatch.setattr(
                "vigil_cli.config.load_config", lambda: cfg
            )
    ...
    def test_public_url_with_path_prefix_baked_in(
        ...
        (``https://example.com/vigil``), the OAuth callback URL is
        the path appended verbatim. The operator is declaring the
        whole authority; we trust them."""
        patch_config(None)
        monkeypatch.setenv(
            "VIGIL_DASHBOARD_PUBLIC_URL", "https://example.com/vigil",
        )
        redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        from vigil_cli.dashboard_auth import prefix as prefix_mod

        # Reset the per-value dedup cache so the warning fires in-test
        # regardless of test ordering.
        prefix_mod._warned_malformed_public_urls.clear()
        patch_config(None)
        monkeypatch.setenv("VIGIL_DASHBOARD_PUBLIC_URL", "vigil.domain.com")
    ...
    def test_scheme_less_public_url_warning_is_deduplicated(
        ...
        scheme_warnings = [
            ...
            for r in caplog.records
            if r.levelno == logging.WARNING
            and "vigil.domain.com" in r.getMessage()
        ]
    ...
    def test_valid_public_url_emits_no_warning(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "VIGIL_DASHBOARD_PUBLIC_URL", "https://vigil.domain.com"
        )

        with caplog.at_level(logging.WARNING, logger=prefix_mod.__name__):
            result = prefix_mod.resolve_public_url()

        assert result == "https://vigil.domain.com"
        assert not [
...
class TestCookiePathRespectsPrefix:
    ...
    def test_pkce_cookie_uses_prefix_path(self, gated_app_proxied):
        ...
        pkce = next(c for c in cookies if "vigil_session_pkce" in c)
        # Browser only sends cookie back if the request path is under
        # the cookie's Path attribute, so we need /vigil here. Bare
        # /-rooted cookies would still be sent but would also be sent
        # to /billing/... etc.
        assert "Path=/vigil" in pkce, (
    ...
    def test_pkce_cookie_uses_secure_prefix_when_proxied(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Secure-vigil_session_pkce=")
        ]
    ...
    def test_pkce_cookie_uses_host_prefix_when_direct(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Host-vigil_session_pkce=")
        ]
        assert pkce_candidates, (
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from vigil_cli.dashboard_auth.cookies import set_pkce_cookie
        ...
        assert any(c.startswith("vigil_session_pkce=") for c in cookies), (
    ...
    def test_cookies_read_back_round_trip_through_prefix(
        ...
        pkce_set = next(
            ...
            if "vigil_session_pkce" in c
        )
        # Parse "__Secure-vigil_session_pkce=...; HttpOnly; ...".
        pkce_kv = pkce_set.split(";", 1)[0]  # "__Secure-vigil_session_pkce=value"
        ...
        at_cookies = [
            c for c in cookies
            if c.startswith("__Secure-vigil_session_at=")
        ]
        assert at_cookies, (
            f"session_at missing __Secure- prefix: {cookies!r}"
        )
        assert "Path=/vigil" in at_cookies[0]
