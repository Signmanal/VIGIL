"""Tests for `vigil chat --safe-mode` — pristine troubleshooting runs.
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in vigil_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("VIGIL_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under VIGIL_SAFE_MODE=1."""
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("VIGIL_SAFE_MODE", raising=False)
        from vigil_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under VIGIL_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `vigil chat`."""
