...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./vigil` should use `vigil_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "vigil"
    called = []

    fake_main_module = types.ModuleType("vigil_cli.main")

    def fake_main():
        called.append("vigil_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "vigil_cli.main", fake_main_module)
