...
import pytest
from vigil_cli.model_switch import list_authenticated_providers, switch_model
from vigil_cli import runtime_provider as rp
...
def test_list_authenticated_providers_includes_full_models_list_from_user_providers(monkeypatch):
    ...
    monkeypatch.setattr("vigil_cli.providers.VIGIL_OVERLAYS", {})
...
def test_list_authenticated_providers_uses_live_models_for_user_provider(monkeypatch):
    ...
    monkeypatch.setattr("vigil_cli.models.fetch_api_models", fake_fetch_api_models)
...
def test_switch_model_resolves_user_provider_credentials(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "vigil_cli.models.validate_requested_model",
...
def _run_user_provider_override_case(
    *,
    ...
    with patch("vigil_cli.model_switch.resolve_alias", return_value=None), \
         patch("vigil_cli.model_switch.list_provider_models", return_value=[]), \
         patch("vigil_cli.model_switch.normalize_model_for_provider", side_effect=lambda model, provider: model), \
         patch("vigil_cli.models.validate_requested_model", return_value=_REJECTED_VALIDATION), \
         patch("vigil_cli.models.detect_provider_for_model", return_value=None), \
         patch("vigil_cli.model_switch.get_model_info", return_value=None), \
         patch("vigil_cli.model_switch.get_model_capabilities", return_value=None), \
         patch("vigil_cli.runtime_provider.resolve_runtime_provider", return_value={"api_key": "***", "base_url": base_url, "api_mode": "anthropic_messages"}):
        return switch_model(
            raw_input=raw_input,
...
def test_section3_probes_no_key_endpoint_without_explicit_models(monkeypatch):
    ...
    monkeypatch.setattr("vigil_cli.models.fetch_api_models", _fake_fetch)
...
def test_section3_skips_probe_when_no_key_but_explicit_models(monkeypatch):
    ...
    monkeypatch.setattr("vigil_cli.models.fetch_api_models", _fail_fetch)
