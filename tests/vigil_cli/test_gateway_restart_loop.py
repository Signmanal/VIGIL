...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "Vigil Gateway Restart",            # case handled
        "VIGIL GATEWAY RESTART",           # uppercase
    ])
    def test_vigil_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.vigil.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.vigil.gateway.plist",
        "launchctl stop ai.vigil.gateway",
        "systemctl restart vigil-gateway",
        "systemctl stop vigil-gateway.service",
        "systemctl start vigil-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill vigil gateway process",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_vigil_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.vigil.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestTerminalToolGatewayLifecycleGuard:
    ...
    Issue #37453: systemctl --user restart vigil-gateway runs as a child of the
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart vigil-gateway", force=True
        ))
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _VIGIL_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart vigil-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart vigil-gateway"]
