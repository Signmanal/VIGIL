"""Tests for the VIGIL plugin system (vigil_cli.plugins)."""
...
from vigil_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<vigil_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<vigil_home>/plugins/``; we derive
    ``<vigil_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        ...
        import os
        vigil_home_str = os.environ.get("VIGIL_HOME")
        if vigil_home_str:
            vigil_home = Path(vigil_home_str)
        else:
            vigil_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.vigil/plugins/ are discovered."""
        plugins_dir = tmp_path / "vigil_test" / "plugins"
    ...
    def test_execution_middleware_does_not_retry_downstream_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("vigil_cli.plugins.get_plugin_manager", lambda: manager)
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("VIGIL_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".vigil" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        vigil_home = tmp_path / "vigil_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under vigil_plugins.<name>."""
        plugins_dir = tmp_path / "vigil_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("VIGIL_HOME", str(tmp_path / "vigil_test"))

        # Clean up any prior namespace module
        sys.modules.pop("vigil_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "vigil_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="vigil_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import vigil_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("vigil_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        vigil_home = tmp_path / "vigil_test"
        plugins_dir = vigil_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "vigil_test" / "plugins"
        # Set VIGIL_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("vigil_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("vigil_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("vigil_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("vigil_cli.plugins.PluginContext.dispatch_tool.__module__", "vigil_cli.plugins"):
...
class TestPluginDebugLogging:
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        from vigil_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With VIGIL_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("VIGIL_PLUGINS_DEBUG", "1")
        from vigil_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".vigil" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("VIGIL_HOME", str(prof))
        ctx = self._ctx()
