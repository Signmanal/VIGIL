"""Tests for vigil_cli.uninstall.remove_node_symlinks.
...
only when they still resolve into the VIGIL-managed node dir.
"""
...
def _make_vigil_node(vigil_home: Path) -> Path:
    """Create a fake $VIGIL_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = vigil_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_vigil_node(fake_home):
    vigil_home = fake_home / ".vigil"
    node_bin = _make_vigil_node(vigil_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(vigil_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(vigil_home) == []


def test_removes_dangling_symlink_into_vigil_node(fake_home):
    """A link into the VIGIL node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the VIGIL links that exist; ignores the ones that don't."""
    vigil_home = fake_home / ".vigil"
    node_bin = _make_vigil_node(vigil_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are VIGIL-managed; node is a real user binary.
