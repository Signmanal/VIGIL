"""Comprehensive tests for vigil_cli.profiles module.
...
def profile_env(tmp_path, monkeypatch):
    ...
    * VIGIL_HOME  -> tmp_path/.vigil  (so get_vigil_home() agrees)
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["vigil", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the VIGIL install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_vigil_home(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".vigil" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".vigil" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.vigil must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".vigil"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes vigil-agent, .worktrees,
        ...
        (default_home / "vigil-agent" / ".git").mkdir(parents=True)
        (default_home / "vigil-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "vigil-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "vigil-agent").exists()
...
class TestNoSkillsOptOut:
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `vigil update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestBackfillProfileEnvs:
    ...
    def test_never_overwrites_existing_profile_env(self, profile_env):
        tmp_path = profile_env
        (tmp_path / ".vigil" / ".env").write_text("KEY=root\n")
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("vigil_cli.profiles._cleanup_gateway_service"):
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("vigil_cli.profiles._cleanup_gateway_service"), \
             patch("vigil_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
...
class TestGetActiveProfileName:
    ...
    def test_default_vigil_home_returns_default(self, profile_env):
        # VIGIL_HOME points to tmp_path/.vigil which is the default
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("vigil")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        monkeypatch.setattr("vigil_cli.profiles.shutil.which", lambda name: "/opt/vigil/bin/vigil")
        from vigil_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "exec /opt/vigil/bin/vigil -p mybot" in content
    ...
    def test_creates_bat_on_windows(self, profile_env, monkeypatch):
        ...
        assert "vigil -p mybot" in content
        assert "%*" in content

    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from vigil_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from vigil_cli.profiles import remove_wrapper_script
    ...
    def test_custom_alias_target_on_posix(self, profile_env, monkeypatch):
        ...
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "vigil -p redqueen" in content
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from vigil_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from vigil_cli.profiles import find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr("sys.platform", "darwin")
        from vigil_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("vigil_cli.profiles.check_alias_collision", return_value="skip"):
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "vigil.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["vigil_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["vigil_heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "vigil.ssi_health": {"workspace": "vigil", "enabled": True}
            }
        }))
        ...
        assert "vigil.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["vigil_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["vigil_heimdall"]["workspace"] == "vigil"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "vigil.ssi_health": {"aiPeer": "ssi_health"},
                "vigil_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["vigil.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["vigil_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("vigil-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".vigil_history",
        ...
        excluded_prefixes = [
            "default/vigil-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.vigil_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultVIGILHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_vigil_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".vigil" / "profiles"

    def test_default_vigil_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_vigil_home()
        assert home == tmp_path / ".vigil"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (VIGIL_HOME outside ~/.vigil), profiles go under VIGIL_HOME."""
    ...
    def test_default_vigil_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_vigil_home() returns VIGIL_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("VIGIL_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (VIGIL_HOME under ~/.vigil), profiles root is still ~/.vigil/profiles."""
        native = tmp_path / ".vigil"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under VIGIL_HOME."""
        from vigil_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("VIGIL_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".vigil" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_rejects_pid_reused_by_other_profile(self, profile_env):
        ...
        gateway green while ``vigil -p <name> gateway status`` showed it
        stopped.
        ...
        tmp_path = profile_env
        coder_home = tmp_path / ".vigil" / "profiles" / "coder"
    ...
    def test_gateway_running_check_detects_matching_named_profile(self, profile_env):
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            "gateway.status._read_process_cmdline",
            return_value="vigil -p coder gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_default(self, profile_env):
        ...
        name, home = serve[0]
        assert name == "default"
        assert home == _get_default_vigil_home()
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_vigil_home()
