...
import vigil_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "vigil-gateway.service"
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `vigil gateway restart`)."""
        unit_path = tmp_path / "vigil-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``/tmp/pytest-of-.../vigil_test`` VIGIL_HOME — silently breaking
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E VIGIL_HOME like
        ``/tmp/vigil-e2e-41264`` carries none of the pytest markers but
        poisons the unit identically (seen live 2026-06-11 — an E2E probe ran
        ``vigil gateway restart`` with a /tmp VIGIL_HOME exported; the
        ...
        must refuse ANY temp-dir VIGIL_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "vigil-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="VIGIL_HOME=/tmp/vigil-e2e-41264"\n'
            "WorkingDirectory=/tmp/vigil-e2e-41264\n"
        )
        monkeypatch.setattr(
            gateway_cli,
            "generate_systemd_unit",
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="VIGIL_HOME=/var/tmp/vigil-x"\n'
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="VIGIL_HOME={tmp_path}/vigil-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None

    def test_detects_tmp_home_in_launchd_plist(self):
        plist = (
            "<dict>\n  <key>VIGIL_HOME</key>\n"
            "  <string>/tmp/vigil-e2e-99999</string>\n</dict>\n"
        )
        assert (
    ...
    def test_accepts_macos_real_home_plist(self):
        plist = (
            "<dict>\n  <key>VIGIL_HOME</key>\n"
            "  <string>/Users/alice/.vigil</string>\n</dict>\n"
        )
        assert gateway_cli._temp_home_in_service_definition(plist) is None

    def test_accepts_unit_without_vigil_home(self):
        unit = "[Service]\nExecStart=/usr/bin/python -m vigil_cli.main gateway run\n"
        assert gateway_cli._temp_home_in_service_definition(unit) is None
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="VIGIL_HOME=/tmpfs-data/.vigil"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_vigil_home_for_target_user", lambda home: "/home/alice/.vigil")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        ...
        monkeypatch.setenv("VIGIL_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("VIGIL_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.vigil.gateway.plist"
        ...
        # VIGIL_HOME resolves under the (pytest tmp) test VIGIL_HOME.
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test VIGIL_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>VIGIL_HOME</key>"
                "<string>/Users/alice/.vigil</string></plist>"
            ),
        )
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup vigil gateway run" in out
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_vigil_units", lambda: False)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: vigil gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup vigil gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.vigil.gateway"])
            ),
        )
...
class TestSystemUnitVIGILHome:
    ...
    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: VIGIL_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("VIGIL_HOME", "/root/.vigil/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'VIGIL_HOME=/home/alice/.vigil/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_vigil_home(self, monkeypatch):
        # Custom VIGIL_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("VIGIL_HOME", "/opt/vigil-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'VIGIL_HOME=/opt/vigil-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's VIGIL_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        vigil_home = str(gateway_cli.get_vigil_home().resolve())
        assert f'VIGIL_HOME={vigil_home}' in unit


class TestVIGILHomeForTargetUser:
    """Unit tests for _vigil_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("VIGIL_HOME", raising=False)

        result = gateway_cli._vigil_home_for_target_user("/home/alice")
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._vigil_home_for_target_user("/home/alice")
        assert result == "/home/alice/.vigil/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("VIGIL_HOME", "/opt/vigil")

        result = gateway_cli._vigil_home_for_target_user("/home/alice")
        assert result == "/opt/vigil"
...
class TestProfileArg:
    ...
    def test_default_vigil_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.vigil should not produce a --profile flag."""
        vigil_home = tmp_path / ".vigil"
        vigil_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("VIGIL_HOME", str(vigil_home))
        result = gateway_cli._profile_arg(str(vigil_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.vigil/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".vigil" / "profiles" / "mybot"
    ...
    def test_named_profile_under_target_user_root_returns_flag(self, tmp_path):
        ...
        target_root = tmp_path / "home" / "alice" / ".vigil"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile VIGIL_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "vigil"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.vigil/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".vigil" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".vigil" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_vigil_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_vigil_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.vigil.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".vigil" / "vigil-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".vigil" / "vigil-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/vigil", str(tmp_path / "alice"))
        assert result == "/opt/vigil"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".vigil" / "vigil-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".vigil" / "vigil-agent"
        ...
        monkeypatch.setattr(gateway_cli, "get_vigil_home", lambda: root_home / ".vigil")
        ...
        assert "WorkingDirectory=/home/alice/.vigil/vigil-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "vigil-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "vigil-gateway"])
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'vigil gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'vigil gateway start' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyVIGILUnitDetection:
    """Tests for _find_legacy_vigil_units / has_legacy_vigil_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``vigil.service`` unit behind when the service was
    renamed to ``vigil-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``vigil.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``vigil-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=VIGIL Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m vigil_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_vigil_service_in_user_scope(self, tmp_path, monkeypatch):
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = user_dir / "vigil.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_vigil_units()

        assert len(results) == 1
        name, path, is_system = results[0]
        assert name == "vigil.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_vigil_units() is True

    def test_detects_legacy_vigil_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "vigil.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_vigil_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``vigil-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "vigil-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "vigil-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "vigil-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_vigil_units() is False

    def test_ignores_unrelated_vigil_service(self, tmp_path, monkeypatch):
        """Third-party ``vigil.service`` that isn't ours stays untouched.

        If a user has some other package named ``vigil`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "vigil.service").write_text(
            "[Unit]\nDescription=Some Other VIGIL\n[Service]\n"
            "ExecStart=/opt/other-vigil/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_vigil_units() == []
        assert gateway_cli.has_legacy_vigil_units() is False
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "vigil.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "vigil.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m vigil_cli.main gateway run
          - python path/to/vigil_cli/main.py gateway run
          - vigil gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m vigil_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/vigil/vigil_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/vigil gateway run --replace",
            "ExecStart=/venv/bin/python /opt/vigil/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = f"vigil.service" if i == 0 else f"vigil.service"  # same name
            # Test each variant fresh
            (user_dir / "vigil.service").write_text(
                f"[Unit]\nDescription=Old VIGIL\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_vigil_units()
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "vigil.service"
...
class TestRemoveLegacyVIGILUnits:
    """Tests for remove_legacy_vigil_units (the migration action)."""
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_vigil_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_vigil_units(
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop vigil.service" in c for c in cmds_joined)
        assert any("--user disable vigil.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo vigil gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop vigil.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable vigil.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "vigil.service"
        system_legacy = system_dir / "vigil.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (vigil-gateway-coder.service)
        ...
        profile_unit = user_dir / "vigil-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "vigil-gateway.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_vigil_units(interactive=True)
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import vigil_cli.main as cli_main
        ...
        if project_root is None:
            import vigil_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "vigil_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_vigil_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "vigil_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_vigil_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_vigil_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_vigil_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "vigil-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "vigil-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "vigil-gateway.service",
        )
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "vigil-gateway")
        ...
        assert "sudo systemctl start vigil-gateway" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart vigil-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop vigil-gateway" in out
...
class TestServiceWorkingDirIsStable:
    ...
    def test_stable_working_dir_uses_vigil_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".vigil"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_vigil_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # VIGIL_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".vigil"
        monkeypatch.setattr(gateway_cli, "get_vigil_home", lambda: missing)
        assert gateway_cli._stable_service_working_dir() == str(gateway_cli.PROJECT_ROOT)

    def test_user_unit_workingdirectory_is_vigil_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".vigil"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_vigil_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_vigil_home(self, tmp_path, monkeypatch):
        import re
