"""Regression tests for #34107 — Docker UID/GID handling in ensure_vigil_home.
...
The fix is a ``_chown_to_vigil_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_vigil_uid_gid
# ---------------------------------------------------------------------------


class TestResolveVIGILUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        ...
        from vigil_cli.config import _resolve_vigil_uid_gid
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("VIGIL_UID", "not-a-number")
        monkeypatch.setenv("VIGIL_GID", "911")
        from vigil_cli.config import _resolve_vigil_uid_gid
        uid, gid = _resolve_vigil_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("VIGIL_UID", "")
        monkeypatch.setenv("VIGIL_GID", "")
        from vigil_cli.config import _resolve_vigil_uid_gid
        uid, gid = _resolve_vigil_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToVIGILUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_vigil_uid(d)  # must not raise
