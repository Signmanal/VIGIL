...
``vigil_cli.main`` skips eager plugin discovery at argparse-setup time
...
def _live_subcommand_names() -> set[str]:
    ...
    built-in-only set.
    """
    from vigil_cli import main as _main
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``vigil help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"vigil_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
