...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    vigil = home / ".vigil"
    vigil.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("VIGIL_HOME", str(vigil))
...
def _env_value(vigil_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from vigil_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("vigil_cli.main._require_tty", lambda *_a, **_kw: None)
