...
argparse → cmd_chat → cli.main() → VIGILCLI → AIAgent call chain.
"""
...
class TestIgnoreUserConfigEnvGate:
    ...
    When the env var is set, user config at ``<vigil_home>/config.yaml`` is
    ...
    def test_user_config_skipped_when_flag_set(self, tmp_path, monkeypatch):
        ...
        # user's value
        assert cfg["model"].get("default", "") != "openrouter/user-config-test-model"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``VIGILCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting VIGIL_IGNORE_RULES=1 flips VIGILCLI.ignore_rules True."""
        monkeypatch.setenv("VIGIL_IGNORE_RULES", "1")

        # Import VIGILCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of VIGILCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.VIGILCLI)
        # Replicate the exact logic from cli.py VIGILCLI.__init__:
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``vigil_cli/main.py`` must set
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real vigil parser accepts both flags."""
        ...
        import inspect
        import vigil_cli.main as hm
        src = inspect.getsource(hm)
        assert "VIGIL_IGNORE_USER_CONFIG" in src
        assert "VIGIL_IGNORE_RULES" in src
