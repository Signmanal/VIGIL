"""Tests for the unified provider catalog (vigil_cli.provider_catalog).
...
from vigil_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
...
def test_catalog_covers_every_vigil_model_provider():
    """PARITY CONTRACT: the catalog == the `vigil model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `vigil model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_vigil_model():
    """The catalog must not invent providers `vigil model` doesn't show."""
