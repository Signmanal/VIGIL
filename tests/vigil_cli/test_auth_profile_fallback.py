...
# logged into VIGIL Portal"`` even though the user has a valid global Nous
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from vigil_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    vigil_home = tmp_path / "classic"
    vigil_home.mkdir()
    monkeypatch.setenv("VIGIL_HOME", str(vigil_home))

    _write(vigil_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (VIGIL_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.vigil as the "real home" — which is NOT equal
    # to the VIGIL_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    vigil_home = tmp_path / "classic"
    ...
    _write(vigil_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from vigil_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: VIGIL_HOME is set to a custom path that is NOT under
    # ~/.vigil/profiles/ — get_default_vigil_root() returns VIGIL_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from vigil_cli.auth import read_credential_pool, write_credential_pool
