...
import vigil_cli.doctor as doctor
import vigil_cli.gateway as gateway_cli
from vigil_cli import doctor as doctor_mod
from vigil_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "VIGIL_HOME", vigil_home)
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("VIGIL_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_vigil_home(self, tmp_path, provider=""):
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_vigil_home(tmp_path, provider)
...
def test_run_doctor_accepts_vigil_provider_ids_that_catalog_aliases(
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from vigil_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("vigil_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "vigil_cli.doctor", raising=False)
        from vigil_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorXaiOAuthStatus:
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "VIGIL Portal auth" in out
        assert "logged in" in out
...
class TestDoctorStaleMaxIterationsDrift:
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        (vigil_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "VIGIL_HOME", vigil_home)
        monkeypatch.setattr(doctor_mod, "get_vigil_home", lambda: vigil_home)
        ...
        return buf.getvalue(), vigil_home

    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        out, vigil_home = self._run_config_section(
        ...
        assert "VIGIL_MAX_ITERATIONS=90" in (vigil_home / ".env").read_text(encoding="utf-8")
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        env_after = (vigil_home / ".env").read_text(encoding="utf-8")
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`vigil doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken VIGIL install.
