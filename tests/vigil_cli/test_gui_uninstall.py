"""Tests for vigil_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import vigil_cli.gui_uninstall as gu


def _make_agent(vigil_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = vigil_home / "vigil-agent"
    (agent_root / "vigil_cli").mkdir(parents=True)
    (agent_root / "vigil_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(vigil_home: Path) -> None:
    """Create the source-built GUI artifacts a `vigil desktop` run produces."""
    desktop = vigil_home / "vigil-agent" / "apps" / "desktop"
    ...
    (vigil_home / "vigil-agent" / "node_modules").mkdir(parents=True)
    (vigil_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(vigil_home: Path) -> None:
    (vigil_home / "config.yaml").write_text("x: 1\n")
    (vigil_home / ".env").write_text("KEY=secret\n")
    (vigil_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    vigil_home = tmp_path / ".vigil"
    vigil_home.mkdir()
    assert gu.agent_is_installed(vigil_home) is False
    _make_agent(vigil_home)
    assert gu.agent_is_installed(vigil_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    vigil_home = tmp_path / ".vigil"
    (vigil_home / "vigil-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(vigil_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    vigil_home = tmp_path / ".vigil"
    _make_gui_build(vigil_home)
    artifacts = gu.source_built_gui_artifacts(vigil_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(vigil_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(vigil_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    vigil_home = tmp_path / ".vigil"
    agent_root = _make_agent(vigil_home)
    _make_gui_build(vigil_home)
    _make_user_data(vigil_home)
    ...
    removed = gu.uninstall_gui(vigil_home)
    ...
    assert not (vigil_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "vigil_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (vigil_home / "config.yaml").exists()
    assert (vigil_home / ".env").exists()
    assert (vigil_home / "sessions").exists()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(vigil_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(vigil_home)
    ...
    assert summary["vigil_home"] == str(vigil_home)
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    """
    import vigil_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_vigil_home", lambda: vigil_home)
    ...
    from vigil_cli import gui_uninstall as gu_mod
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    ...
    vigil_home = tmp_path / ".vigil"
    (vigil_home / "vigil-agent" / "vigil_cli").mkdir(parents=True)
    (vigil_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not vigil_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m vigil_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_vigil_home", lambda: vigil_home)
    ...
    assert not (desktop / "release").exists()
    assert not (vigil_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "vigil_cli").exists()
    assert (vigil_home / "config.yaml").exists()
