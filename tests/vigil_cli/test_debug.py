...
@pytest.fixture
def vigil_home(tmp_path, monkeypatch):
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO vigil_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from vigil_cli.debug import _upload_paste_rs
        ...
        with patch("vigil_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "vigil_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from vigil_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from vigil_cli.debug import upload_to_pastebin

        with patch("vigil_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("vigil_cli.debug._upload_paste_rs",
             ...
             patch("vigil_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, vigil_home):
        from vigil_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, vigil_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (vigil_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, vigil_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = vigil_home / "logs" / "agent.log"
        from vigil_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, vigil_home):
        ...
        (vigil_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, vigil_home):
    ...
    def test_unknown_log_returns_none(self, vigil_home):
        from vigil_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, vigil_home):
    ...
    def test_prefers_primary_over_rotated(self, vigil_home):
    ...
    def test_falls_back_when_primary_empty(self, vigil_home):
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def vigil_home_with_secret(self, tmp_path, monkeypatch):
    ...
    def test_default_redacts_tail_and_full_text(self, vigil_home_with_secret):
        from vigil_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, vigil_home_with_secret):
        from vigil_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, vigil_home_with_secret, monkeypatch
    ):
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, vigil_home_with_secret
    ):
        from vigil_cli.debug import _capture_log_snapshot

        log_path = vigil_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, vigil_home_with_secret):
        from vigil_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from vigil_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, vigil_home):
        from vigil_cli.debug import collect_debug_report

        with patch("vigil_cli.dump.run_dump") as mock_dump:
    ...
    def test_report_includes_agent_log(self, vigil_home):
        from vigil_cli.debug import collect_debug_report

        with patch("vigil_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, vigil_home):
        from vigil_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, vigil_home):
        from vigil_cli.debug import collect_debug_report
    ...
    def test_report_includes_gui_log(self, vigil_home):
        from vigil_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, vigil_home):
        from vigil_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, vigil_home, capsys):
        """Slash-command path should sweep old pending deletes before uploading."""
        from vigil_cli.debug import run_debug_share
        ...
        with patch("vigil_cli.dump.run_dump"), \
             patch("vigil_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("vigil_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, vigil_home, capsys):
        ...
        with patch("vigil_cli.dump.run_dump"), \
             patch(
                 "vigil_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, vigil_home, capsys):
    ...
    def test_share_uploads_five_pastes(self, vigil_home, capsys):
    ...
    def test_share_keeps_report_and_full_log_on_same_snapshot(self, vigil_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from vigil_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("vigil_cli.dump.run_dump"), \
             patch("vigil_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("vigil_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, vigil_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, vigil_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, vigil_home_with_secret, capsys
    ):
        ...
        with patch("vigil_cli.dump.run_dump"), \
             patch("vigil_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("vigil_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    ...
    def test_share_subcommand_routes(self, vigil_home):
        from vigil_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from vigil_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from vigil_cli.debug import delete_paste
        ...
        with patch("vigil_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    def test_does_not_spawn_subprocess(self, vigil_home):
        ...
        import ast
        import inspect
        from vigil_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, vigil_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from vigil_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, vigil_home):
    ...
    def test_merges_with_existing_pending(self, vigil_home):
        """Subsequent calls merge into existing pending.json."""
        from vigil_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, vigil_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, vigil_home):
    ...
    def test_sweep_deletes_expired_entries(self, vigil_home):
        from vigil_cli.debug import (
        ...
        with patch("vigil_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, vigil_home):
        from vigil_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("vigil_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, vigil_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, vigil_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, vigil_home):
        from vigil_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("vigil_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, vigil_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: vigil debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from vigil_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("vigil_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("vigil_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, vigil_home, capsys):
        from vigil_cli.debug import run_debug_share
        ...
        with patch("vigil_cli.dump.run_dump"), \
             ...
             patch("vigil_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, vigil_home, capsys):
        from vigil_cli.debug import run_debug_share
        ...
        with patch("vigil_cli.dump.run_dump"), \
             patch("vigil_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("vigil_cli.debug._schedule_auto_delete"):
            run_debug_share(args)

        out = capsys.readouterr().out
        assert "public paste service" in out

    def test_local_no_privacy_notice(self, vigil_home, capsys):
        from vigil_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    def test_returns_structured_urls(self, vigil_home):
        from vigil_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("vigil_cli.dump.run_dump"), patch(
            "vigil_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("vigil_cli.debug._schedule_auto_delete"):
    ...
    def test_skips_missing_logs_without_failure(self, vigil_home):
        from vigil_cli.debug import build_debug_share

        # Remove desktop.log so it should be neither uploaded nor reported failed.
        (vigil_home / "logs" / "desktop.log").unlink()

        with patch("vigil_cli.dump.run_dump"), patch(
            "vigil_cli.debug.upload_to_pastebin",
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, vigil_home):
        from vigil_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, vigil_home):
        from vigil_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, vigil_home):
        from vigil_cli.debug import build_debug_share
