...
def fresh_home(tmp_path, monkeypatch):
    ...
    # Also reset vigil_constants cache so get_default_vigil_root() re-reads.
    try:
        import vigil_constants
        vigil_constants._cached_default_vigil_root = None  # type: ignore[attr-defined]
    except Exception:
        pass
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "vigil-agent", "proj_1", "a",
...
class TestPathResolution:
    ...
    def test_env_var_workspaces_override(self, fresh_home, tmp_path, monkeypatch):
        forced = tmp_path / "ws"
        monkeypatch.setenv("VIGIL_KANBAN_WORKSPACES_ROOT", str(forced))
...
class TestCurrentBoard:
    ...
    def test_env_var_takes_precedence(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("VIGIL_KANBAN_BOARD", "envboard")
    ...
    def test_env_beats_file(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("VIGIL_KANBAN_BOARD", "b")
...
class TestConnectionIsolation:
    ...
    def test_connect_stale_env_uses_fallback_board_without_recreating_it(
        ...
        kb.create_board("persist")
        kb.set_current_board("persist")
        monkeypatch.setenv("VIGIL_KANBAN_BOARD", "ephemeral")
...
class TestWorkerSpawnEnv:
    ...
    def test_default_board_spawn_keeps_legacy_paths(self, fresh_home, monkeypatch):
        ...
        env = captured["env"]
        assert env["VIGIL_KANBAN_BOARD"] == "default"
        assert env["VIGIL_KANBAN_DB"] == str(fresh_home / "kanban.db")
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "vigil_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
