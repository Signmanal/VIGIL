...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_vigil_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_vigil_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_probe_runs_inside_profile_scope(
        ...
        import vigil_cli.mcp_config as mcp_config
        from vigil_constants import get_vigil_home
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        monkeypatch.setattr(
            "vigil_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid():
            seen_homes.append(str(get_vigil_home()))
            return None
        ...
        assert resp.json()["vigil_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_vigil_home(self, isolated_profiles, monkeypatch):
        import vigil_cli.web_server as web_server

        monkeypatch.setattr(
            "vigil_cli.main._make_tui_argv",
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("VIGIL_HOME") != str(isolated_profiles["worker_beta"])
