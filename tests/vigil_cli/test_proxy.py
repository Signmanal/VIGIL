...
from vigil_cli.proxy.adapters import ADAPTERS, get_adapter
from vigil_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from vigil_cli.proxy.adapters.nous_portal import NousPortalAdapter
from vigil_cli.proxy.adapters.xai import XAIGrokAdapter
...
def _write_auth_store(vigil_home: Path, nous_state: Dict[str, Any]) -> Path:
...
def test_nous_adapter_metadata():
    ...
    assert adapter.display_name == "VIGIL Portal"
...
def test_nous_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # VIGIL_HOME is already set by conftest, but make doubly sure
...
def test_nous_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    ...
    with patch(
        "vigil_cli.proxy.adapters.nous_portal.resolve_nous_runtime_credentials",
...
def test_nous_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("VIGIL_HOME", str(tmp_path))
    adapter = NousPortalAdapter()
    with pytest.raises(RuntimeError, match="vigil auth add nous"):
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("vigil_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("vigil_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from vigil_cli.proxy.server import create_app  # noqa: E402
...
def test_server_forwards_chat_completions():
    async def run():
        ...
        try:
            ...
            assert "VIGIL-4-70B" in req["body"]
        finally:
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("VIGIL_HOME", str(tmp_path))
    from vigil_cli.proxy.cli import cmd_proxy_status
    ...
    assert "nous" in out
    assert "VIGIL Portal" in out
    assert "not logged in" in out


def test_cmd_proxy_providers_runs(capsys):
    from vigil_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from vigil_cli.proxy.cli import cmd_proxy_start
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    assert rc == 2
    err = capsys.readouterr().err
    assert "vigil auth add nous" in err
