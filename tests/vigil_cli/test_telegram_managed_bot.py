"""Tests for vigil_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert name.startswith("vigil_")
        assert name.endswith("_bot")
        assert len(name) == len("vigil_") + 16 + len("_bot")
        assert len(name) <= 32
    ...
    def test_slug_uses_telegram_safe_base32_chars(self):
        name = generate_bot_username()
        slug = name.removeprefix("vigil_").removesuffix("_bot")
        assert len(slug) == 16
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "vigil_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/VIGILSetupBot/vigil_abcdefghijklmnop_bot?name=VIGIL+Agent",
            "qr_payload": "https://t.me/newbot/VIGILSetupBot/vigil_abcdefghijklmnop_bot?name=VIGIL+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "vigil_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="vigil_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/VIGILSetupBot/vigil_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/VIGILSetupBot/vigil_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "vigil_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "vigil_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("vigil_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("vigil_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("vigil_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("vigil_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("vigil_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "vigil_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
    ...
    def test_eventual_success(self):
        ...
        with patch("vigil_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("vigil_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    ...
    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "vigil_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from vigil_cli.setup import _profile_name_from_vigil_home

        assert (
            _profile_name_from_vigil_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\vigil\profiles\oracle")
            )
            == "oracle"
        )
