...
``~/.vigil/logs/`` (profile-aware via ``get_vigil_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``vigil_cli.web_server``,
    ``vigil_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from vigil_constants import get_config_path, get_vigil_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_vigil_session_injector", False):
    ...
    _session_record_factory._vigil_session_injector = True  # type: ignore[attr-defined]
...
COMPONENT_PREFIXES = {
    ...
    "gateway": ("gateway", "vigil_plugins", "plugins.platforms"),
...
def setup_logging(
    ...
    *force* is ``True``.
    ...
    home = vigil_home or get_vigil_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_vigil_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._vigil_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from vigil_cli.config import is_managed
