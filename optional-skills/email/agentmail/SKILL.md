---
name: agentmail
description: Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. vigil-agent@agentmail.to).
version: 1.0.0
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `vigil-agent`)
   - The agent gets address: `vigil-agent@agentmail.to`
2. Send an email:
