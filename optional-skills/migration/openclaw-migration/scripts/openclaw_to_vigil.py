...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "messaging-settings": {
        ...
        "description": "Import VIGIL-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of VIGIL-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
...
class Migrator:
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No VIGIL-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted VIGIL-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
        ...
        for oc_key, vigil_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and vigil_key not in secret_additions:
                secret_additions[vigil_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, vigil_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and vigil_key not in secret_additions:
                        secret_additions[vigil_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        vigil_config = load_yaml_file(destination)
        current_model = vigil_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = vigil_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                vigil_config["model"] = {"default": model_str}
            dump_yaml_file(destination, vigil_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        vigil_config = load_yaml_file(destination)
        existing_tts = vigil_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        partially_extracted = [
            ("openclaw.json", "Selected VIGIL-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected VIGIL-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        vigil_cfg_path = self.target_root / "config.yaml"
        vigil_cfg = load_yaml_file(vigil_cfg_path)
        existing_mcp = vigil_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            vigil_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                vigil_srv["command"] = srv["command"]
                if srv.get("args"):
                    vigil_srv["args"] = srv["args"]
                if srv.get("env"):
                    vigil_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    vigil_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                vigil_srv["url"] = srv["url"]
                if srv.get("headers"):
                    vigil_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    vigil_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                vigil_srv["enabled"] = False
            if srv.get("timeout"):
                vigil_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                vigil_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                vigil_srv["tools"] = {}
                if tools_cfg.get("include"):
                    vigil_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    vigil_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(vigil_cfg_path)
            vigil_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(vigil_cfg_path, vigil_cfg)
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = vigil_cfg.get("agent") or {}
        ...
        if compaction:
            compression = vigil_cfg.get("compression") or {}
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        vigil_cfg_path = self.target_root / "config.yaml"
        vigil_cfg = load_yaml_file(vigil_cfg_path)
        sr = vigil_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        vigil_cfg_path = self.target_root / "config.yaml"
        vigil_cfg = load_yaml_file(vigil_cfg_path)
        custom_providers = vigil_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(vigil_cfg_path)
            vigil_cfg["custom_providers"] = custom_providers
            dump_yaml_file(vigil_cfg_path, vigil_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if discord_cfg:
            vigil_cfg_path = self.target_root / "config.yaml"
            vigil_cfg = load_yaml_file(vigil_cfg_path)
            discord_hermes = vigil_cfg.get("discord") or {}
            changed = False
            ...
            if changed and self.execute:
                vigil_cfg["discord"] = discord_hermes
                dump_yaml_file(vigil_cfg_path, vigil_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        vigil_cfg_path = self.target_root / "config.yaml"
        vigil_cfg = load_yaml_file(vigil_cfg_path)
        browser_hermes = vigil_cfg.get("browser") or {}
        changed = False
        ...
        if changed:
            vigil_cfg["browser"] = browser_hermes
            if self.execute:
                self.maybe_backup(vigil_cfg_path)
                dump_yaml_file(vigil_cfg_path, vigil_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            vigil_mode = mode_map.get(mode, "manual")
            vigil_cfg.setdefault("approvals", {})["mode"] = vigil_mode
            if self.execute:
                self.maybe_backup(vigil_cfg_path)
                dump_yaml_file(vigil_cfg_path, vigil_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{vigil_mode}'")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of VIGIL-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
