...
$VIGIL_OSINT_CACHE/icij/ (default: ~/.cache/vigil-osint/icij/) and search
...
def _cache_dir() -> Path:
    ...
    return Path.home() / ".cache" / "vigil-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "vigil-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $VIGIL_OSINT_CACHE/icij or ~/.cache/vigil-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
