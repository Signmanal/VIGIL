...
Module-level run.py helpers a handler needs (``_vigil_home``,
...
from vigil_cli.config import cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
    atomic_yaml_write,
...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _vigil_home
        ...
        try:
            ...
            atomic_json_write(
                _vigil_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _vigil_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running VIGIL Agent version."""
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from vigil_cli.commands import gateway_help_lines
        lines = [
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _vigil_home, _load_gateway_config
        import yaml
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected(
                        ...
                        try:
                            from vigil_cli.context_switch_guard import (
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_vigil_home())
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from vigil_cli.goals import draft_contract
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _vigil_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _vigil_home, _platform_config_key
        import yaml
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> str:
        ...
        from gateway.run import _vigil_home, _load_gateway_config, _resolve_gateway_model
        import yaml
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _vigil_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _vigil_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from vigil_cli.session_listing import (
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update VIGIL Agent to the latest version.
        ...
        """
        from gateway.run import _vigil_home, _resolve_vigil_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        from vigil_cli.config import is_managed, format_managed_message
        ...
        if is_managed():
            return f"✗ {format_managed_message('update VIGIL Agent')}"
        ...
        if not vigil_cmd:
            return t("gateway.update.vigil_cmd_not_found")
        ...
        try:
            if sys.platform == "win32":
                ...
                # vigil_cmd is a list of argv parts we can pass directly
                ...
                vigil_cmd_str = " ".join(shlex.quote(part) for part in vigil_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {vigil_cmd_str} update --gateway"
