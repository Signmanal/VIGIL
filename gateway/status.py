...
def _get_lock_dir() -> Path:
    ...
    return state_home / "vigil" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    ...
    test also matched ``vigil_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\vigil-gateway.exe"``) survive, and strips
    ...
    for token in tokens:
        ...
        if basename in ("vigil-gateway", "vigil-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "vigil_cli.main" in joined
        or "vigil_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("vigil", "vigil.exe") for t in tokens)
    )
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating VIGIL-owned runtime records
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``vigil_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"vigil_home={home_lc}" in command_lc
        )
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "vigil_home=" in command_lc and f"vigil_home={home_lc}" not in command_lc:
        return False
    return True
...
# enabled in the user's systemd (e.g. ``vigil.service`` + ``vigil-
# gateway.service``).
#
