...
# ``sessions_dir / f"{session_id}.json"`` in vigil_state, request-dump
...
def build_session_context_prompt(
    ...
    *,
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_vigil_home()}/cron/output/)"
    )
    ...
    def _save(self) -> None:
        ...
        data = {
            "_README": (
                ...
                "~/.vigil/state.db and are shown by `vigil sessions list` and "
