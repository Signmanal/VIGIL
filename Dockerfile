...
# Do not write .pyc files at runtime: /opt/vigil is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/vigil/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when vigil
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# vigil process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main vigil process, the dashboard,
...
RUN set -eu; \
    ...
    # 'VIGIL WebUI' catalog). The image has moved to s6-overlay /init\
...
RUN useradd -u 10000 -m -d /opt/data vigil
...
#
# ui-tui/packages/vigil-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/vigil-ink/ ui-tui/packages/vigil-ink/
...
# lock on the @vigil/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/vigil/.venv at first
...
# ---------- Permissions ----------
# Link vigil-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Keep /opt/vigil immutable for the runtime vigil user. Hosted/container
...
# supervised VIGIL processes drop to the non-root vigil user.
USER root
RUN mkdir -p /opt/vigil/bin && \
    cp /opt/vigil/docker/vigil-exec-shim.sh /opt/vigil/bin/vigil && \
    chmod 0755 /opt/vigil/bin/vigil && \
    printf 'docker\n' > /opt/vigil/.install_method && \
    chown -R root:root /opt/vigil && \
    chmod -R a+rX /opt/vigil && \
    chmod -R a-w /opt/vigil
...
# host install's marker and wrongly block its ``vigil update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the vigil user via
# `s6-setuidgid vigil` in its run script. If VIGIL_UID is unset, services
# run as the default vigil user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `vigil dump` reports
...
# /opt/vigil/.vigil_build_sha at build time, and have
# vigil_cli/build_info.py read it at runtime.  Both `vigil dump` and
...
RUN if [ -n "${VIGIL_GIT_SHA}" ]; then \
        chmod u+w /opt/vigil && \
        printf '%s\n' "${VIGIL_GIT_SHA}" > /opt/vigil/.vigil_build_sha && \
        chmod a-w /opt/vigil /opt/vigil/.vigil_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-vigil + dashboard.
...
# `exec vigil`. Wired in as cont-init.d/01- so it
...
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/vigil/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-vigil-setup && \
    chmod +x /etc/cont-init.d/01-vigil-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV VIGIL_WEB_DIST=/opt/vigil/vigil_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/vigil/ui-tui/dist/entry.js`) and skip the
...
ENV VIGIL_TUI_DIR=/opt/vigil/ui-tui
ENV VIGIL_HOME=/opt/data
ENV VIGIL_WRITE_SAFE_ROOT=/opt/data
ENV VIGIL_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/vigil (root-owned, read-only) so a runtime
...
# is seeded + chowned to the vigil user by docker/stage2-hook.sh and lives
...
# `docker exec <c> vigil ...` they default to root, and any file the
# command writes under $VIGIL_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/vigil/bin/vigil, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid vigil`
# when invoked as root. Non-root callers (supervised processes,
# `--user vigil`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/vigil/.venv/bin/vigil). See the shim source for
...
# bin globally so `docker exec <container> vigil ...` and any
# subprocess that doesn't activate the venv first still find vigil.
#
# /opt/vigil/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/vigil/bin:/opt/vigil/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. vigil
# subcommand vs. no-args), drops to the vigil user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/vigil/docker/main-wrapper.sh" ]
CMD [ ]
