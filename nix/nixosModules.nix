# nix/nixosModules.nix — NixOS module for vigil-agent
#
# Two modes:
...
{ inputs, ... }: {
  ...
  let
    ...
    vigil-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-vigil-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "vigil-config-attrs";
      description = "VIGIL YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "vigil-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "vigil-documents" { } (
      ''
        mkdir -p $out
    ...
    containerEntrypoint = pkgs.writeShellScript "vigil-container-entrypoint" ''
      set -eu
      ...
      if [ ! -f /var/lib/vigil-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/vigil-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/vigil ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/vigil
        chmod 0440 /etc/sudoers.d/vigil
      fi
    ...
    # Default: /var/lib/vigil/workspace → /data/workspace.
  ...
  in {
    options.services.vigil-agent = with lib; {
      enable = mkEnableOption "VIGIL Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = vigil-agent;
        description = "The vigil-agent package to use.";
      };
      ...
      stateDir = mkOption {
        ...
        description = "State directory. Contains .vigil/ subdir (VIGIL_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative VIGIL config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          VIGIL reads this file on every startup via load_vigil_dotenv().
        '';
      };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `vigil gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the vigil user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the vigil plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. VIGIL discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          vigil_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as vigil (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in vigil-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the vigil CLI to environment.systemPackages and export
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          description = ''
            ...
            stateDir. These users are automatically added to the vigil group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.vigil-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      (lib.mkIf cfg.addToSystemPackages {
        ...
        environment.variables.VIGIL_HOME = "${cfg.stateDir}/.vigil";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.vigil-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.vigil-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed vigil
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure vigil is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.vigil        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.vigil/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.vigil/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.vigil/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.vigil/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.vigil/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."vigil-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.vigil
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.vigil ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.vigil ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.vigil -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.vigil/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.vigil/$_subdir"
            chmod 2770 "${cfg.stateDir}/.vigil/$_subdir"
            find "${cfg.stateDir}/.vigil/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # vigil-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.vigil/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.vigil/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.vigil/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.vigil/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.vigil/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.vigil/.managed
          chmod 0644 ${cfg.stateDir}/.vigil/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.vigil/.container-mode <<'VIGIL_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    vigil_bin=${containerDataDir}/current-package/bin/vigil
    VIGIL_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.vigil/.container-mode
            chmod 0644 ${cfg.stateDir}/.vigil/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.vigil/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.vigil";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.vigil" ]; then
                  rm -f "${symlinkPath}"
                  echo "vigil-agent: removed symlink ${symlinkPath}"
                fi
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.vigil";
                target = "${cfg.stateDir}/.vigil";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "vigil-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.vigil/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.vigil/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.vigil/auth.json
              fi
            ''}
          ''}
          ...
          # VIGIL reads $VIGIL_HOME/.env at startup via load_vigil_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.vigil/.env"
    ...
    VIGIL_NIX_ENV_EOF
        ...
        find ${cfg.stateDir}/.vigil/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.vigil/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.vigil/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.vigil-agent = {
          description = "VIGIL Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $VIGIL_HOME/.env by the activation script. load_vigil_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/vigil"
              "gateway"
            ] ++ cfg.extraArgs);
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.vigil-agent = {
          description = "VIGIL Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env VIGIL_HOME=${containerDataDir}/.vigil \
                --env VIGIL_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/vigil gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
