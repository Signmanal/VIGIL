...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      vigil-agent = self'.packages.default;
      hermesVenv = vigil-agent.hermesVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "vigil-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "vigil-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "vigil-build-package" { } ''
          echo "PASS: package built at ${vigil-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "vigil-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "vigil-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${vigil-agent}/bin/vigil || (echo "FAIL: vigil binary missing"; exit 1)
          test -x ${vigil-agent}/bin/vigil-agent || (echo "FAIL: vigil-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${vigil-agent}/bin/vigil version 2>&1 | grep -qi "vigil" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "vigil-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in vigil vigil-agent vigil-acp; do
            test -x ${vigil-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "vigil-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking vigil --help ==="
          ${vigil-agent}/bin/vigil --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${vigil-agent}/bin/vigil --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "vigil-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${vigil-agent}/share/vigil-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${vigil-agent}/share/vigil-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "VIGIL_BUNDLED_SKILLS" ${vigil-agent}/bin/vigil || \
        ...
        bundled-plugins = pkgs.runCommand "vigil-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${vigil-agent}/share/vigil-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${vigil-agent}/share/vigil-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "VIGIL_BUNDLED_PLUGINS" ${vigil-agent}/bin/vigil || \
        ...
        bundled-locales = pkgs.runCommand "vigil-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${vigil-agent}/share/vigil-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${vigil-agent}/share/vigil-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${vigil-agent}/share/vigil-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "VIGIL_BUNDLED_LOCALES" ${vigil-agent}/bin/vigil || \
            (echo "FAIL: VIGIL_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: VIGIL_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (VIGIL_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && VIGIL_BUNDLED_LOCALES=${vigil-agent}/share/vigil-agent/locales \
          ...
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without VIGIL_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "vigil-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${vigil-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${vigil-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "VIGIL_TUI_DIR" ${vigil-agent}/bin/vigil || \
        ...
        # Verify VIGIL_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        vigil-node = pkgs.runCommand "vigil-node-version" { } ''
          set -e
          echo "=== Checking VIGIL_NODE in wrapper ==="
          grep -q "VIGIL_NODE" ${vigil-agent}/bin/vigil || \
            (echo "FAIL: VIGIL_NODE not set in wrapper"; exit 1)
          echo "PASS: VIGIL_NODE present in wrapper"

          VIGIL_NODE=$(sed -n "s/^export VIGIL_NODE='\(.*\)'/\1/p" ${vigil-agent}/bin/vigil)
        ...
        managed-guard = pkgs.runCommand "vigil-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked "config set" ${vigil-agent}/bin/vigil config set model foo
          check_blocked "config edit" ${vigil-agent}/bin/vigil config edit
        ...
        extra-python-packages = let
          ...
          hermesWithExtra = vigil-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "vigil-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${hermesWithExtra}/bin/vigil || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${hermesWithExtra}/bin/vigil || \
          ...
          if grep -q "PYTHONPATH" ${vigil-agent}/bin/vigil; then
        ...
        extra-dependency-groups = let
          hermesWithGroups = vigil-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "vigil-extra-dependency-groups" { } ''
          set -e
        ...
        messaging-variant = pkgs.runCommand "vigil-messaging-variant" { } ''
          set -e
        ...
        in pkgs.runCommand "vigil-config-roundtrip" {
          ...
          merge_and_load() {
            local vigil_home="$1"
            export VIGIL_HOME="$vigil_home"
            ${configMergeScript} ${nixSettings} "$vigil_home/config.yaml"
            ${hermesVenv}/bin/python3 -c '
import json, sys
