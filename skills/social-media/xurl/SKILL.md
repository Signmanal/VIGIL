...
version: 1.1.1
author: xdevplatform + openclaw + VIGIL Agent
license: MIT
...
> **Docker HOME pitfall:** In the official VIGIL Docker layout, `/opt/data` is `VIGIL_HOME`, but VIGIL tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for VIGIL-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
