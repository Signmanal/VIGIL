...
if [[ ! -f "$VIGIL_CFG" ]]; then
    echo -e " ${FAIL} VIGIL config not found at ${VIGIL_CFG}"
    manual_steps+=("Create ${VIGIL_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$VIGIL_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in VIGIL config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to VIGIL config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
    ...
    manual_steps+=("Restart VIGIL session to pick up config change")
fi
