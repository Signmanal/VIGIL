...
version: 1.0.0
author: VIGIL Agent (adapted from gsd-build/get-shit-done)
license: MIT
...
If `gsd-sketch` shows up as a sibling skill (installed via `npx get-shit-done-cc --vigil`), prefer **`gsd-sketch`** for the full workflow: persistent `.planning/sketches/` with MANIFEST, frontier mode analysis, consistency audits across past sketches, and integration with the rest of GSD. This skill is the lightweight standalone version — one-off sketching without the state machinery.
