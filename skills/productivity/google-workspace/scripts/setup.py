...
# Ensure sibling modules (_vigil_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _vigil_home import display_vigil_home, get_vigil_home
...
def install_deps():
    ...
    print("  pip install 'vigil-agent[google]'")
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_vigil_home()}/google_token.json")
