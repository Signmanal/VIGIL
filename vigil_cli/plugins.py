...
from vigil_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
...
VALID_HOOKS: Set[str] = {
    ...
    # Kanban task lifecycle hooks. Fired by vigil_cli.kanban_db when a task
...
ENTRY_POINTS_GROUP = "vigil_agent.plugins"

_NS_PARENT = "vigil_plugins"
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        ...
        :func:`vigil_cli.profiles.get_active_profile_name`, so it works in
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`vigil_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        # Lazy import to avoid circular: vigil_cli.main imports plugins indirectly
        from vigil_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to vigil_cli/. Two layouts are
        ...
        user_dir = get_vigil_home() / "plugins"
        ...
        for manifest in winners.values():
            ...
            # for the same reason: every platform VIGIL ships must be
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``vigil_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``vigil_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="vigil-plugin-command-await",
        daemon=True,
    )
    thread.start()
