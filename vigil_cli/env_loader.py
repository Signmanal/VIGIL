...
# load_vigil_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_vigil_dotenv().
...
# process.  ``load_vigil_dotenv()`` is called at module-import time from
# several hot modules (cli.py, vigil_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_vigil_dotenv()`` call.  Returns
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``vigil_cli.config._sanitize_env_lines`` which
    already knows all valid VIGIL env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from vigil_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_vigil_dotenv(
    *,
    vigil_home: str | os.PathLike | None = None,
    ...
    home_path = Path(vigil_home or os.getenv("VIGIL_HOME", Path.home() / ".vigil"))
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of VIGIL reads
    ``os.environ`` for credentials.  Any failure here is logged and
    swallowed — external secret sources must never block startup.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_vigil_dotenv()`` runs at import
    time from several hot modules (cli.py, vigil_cli/main.py,
    run_agent.py, trajectory_compressor.py, ...), so without this guard
    the Bitwarden status line would print 3-5x per CLI startup.  Use
    ``reset_secret_source_cache()`` if you need to force a re-pull
    (tests, future ``vigil secrets bitwarden sync`` from a long-running
    process).
    """
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `vigil model`
