...
Pure display functions with no VIGILCLI state dependency.
"""
...
_UPSTREAM_REPO_URL = "https://github.com/NousResearch/vigil-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/vigil-agent"
...
def _fetch_pypi_latest(package: str = "vigil-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    cache_file = vigil_home / ".update_check"
    embedded_rev = os.environ.get("VIGIL_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # VIGIL_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `vigil update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/vigil/update/check`
    ...
    if embedded_rev:
        ...
        # $VIGIL_HOME/vigil-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = vigil_home / "vigil-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    ...
    because ``$VIGIL_HOME/vigil-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``vigil_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from vigil_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/vigil-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical NousResearch/vigil-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"VIGIL Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    """Build and print a welcome banner with sentinel art on left and info on right.
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else VIGIL_SENTINEL_ART
    except Exception:
        _bskin = None
        _hero = VIGIL_SENTINEL_ART
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]VIGIL SOC[/]")
    ...
    try:
        from vigil_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from vigil_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built vigil; we know an update
    ...
    # Pip-install warning — `pip install vigil-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else VIGIL_AGENT_LOGO
        console.print(_logo)
        console.print()
