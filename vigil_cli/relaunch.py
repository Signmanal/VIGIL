...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched vigil."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_vigil_bin() -> Optional[str]:
    ...
    ``vigil.exe`` on PATH or the ``sys.executable -m vigil_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with vigil.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_vigil_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "vigil_cli.main"]
...
def relaunch(
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``vigil.exe`` (a Python console-script shim that wraps
    ``python -m vigil_cli.main``) or a ``.cmd`` batch file, and both
