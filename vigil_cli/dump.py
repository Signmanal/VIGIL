...
from vigil_cli.config import get_vigil_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    ``VIGIL_GIT_SHA`` build-arg (see ``vigil_cli/build_info.py``).
...
def _gateway_status() -> str:
    ...
    try:
        from vigil_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(vigil_home: Path) -> int:
...
def _cron_summary(vigil_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = vigil_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"vigil_home:      {display_vigil_home()}")
    ...
    toolsets = config.get("toolsets", ["vigil-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(vigil_home)}")
    lines.append(f"  skills:             {_count_skills(vigil_home)}")
