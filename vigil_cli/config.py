...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "VIGIL_TOOL_PROGRESS", "VIGIL_TOOL_PROGRESS_MODE",
...
from vigil_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_vigil_home() / ".managed"
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade vigil-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``vigil_cli/`` — i.e. the git checkout for source
    installs, ``/opt/vigil`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$VIGIL_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how VIGIL was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.
    ...
    3. VIGIL_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/vigil-agent`` image bakes a ``docker``
...
def _running_in_container() -> bool:
    """Thin wrapper around ``vigil_constants.is_container`` (import-safe)."""
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* VIGIL lives in a ``uv tool`` layout.

    ``uv tool install vigil-agent`` places the install at
    ``.../uv/tools/vigil-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``vigil-agent`` happens to be uv-tool-installed on the machine while
    the *active* VIGIL is a regular pip/venv install, causing
    ``vigil update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/vigil-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade vigil-agent"
    if method == "docker":
        return "docker pull nousresearch/vigil-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade vigil-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade vigil-agent"
        return "pip install --upgrade vigil-agent"
    return "vigil update"
...
# vigil_cli/main.py; lives here so the wording stays consistent and we
...
VIGIL Agent runs as a published image (nousresearch/vigil-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/vigil-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate vigil-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/vigil-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/vigil-agent/tags
...
def format_managed_message(action: str = "modify this VIGIL installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            ...
            "Edit services.vigil-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this VIGIL installation is managed by Homebrew "
            f"(VIGIL_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade vigil-agent"
        )
...
def get_container_exec_info() -> Optional[dict]:
    ...
    Returns a dict with keys: backend, container_name, exec_user, vigil_bin
    ...
    container_mode_file = get_vigil_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "vigil-agent")
    exec_user = info.get("exec_user", "vigil")
    vigil_bin = info.get("vigil_bin", "/data/current-package/bin/vigil")
...
# Re-export from vigil_constants — canonical definition lives there.
from vigil_constants import get_vigil_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_vigil_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_vigil_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_vigil_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    subdirectories created at runtime by ``ensure_vigil_home()`` (especially
...
def _chown_to_vigil_uid(path) -> None:
    ...
    directories created by :func:`ensure_vigil_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_vigil_uid_gid()
...
def ensure_vigil_home():
    """Ensure ~/.vigil directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_vigil_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_vigil_home_managed(home: Path):
...
DEFAULT_CONFIG = {
    ...
    # AGENTS.md, CLAUDE.md, .vigil.md, or .cursorrules before VIGIL applies
    ...
    # See tools/kanban_tools.py and vigil_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``vigil dashboard`` -> Notifications).
    "kanban": {
    ...
    "display": {
        ...
        "pet": {
            ...
            # get_vigil_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled VIGIL Portal plugin reads
    ...
    # This section is only needed for vigil-specific overrides; everything else
    ...
    "security": {
        ...
        # ``vigil_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # agent's existing VIGIL Portal token — there is no token key here.
        "chronos": {
    ...
    "code_execution": {
        ...
        #     vigil-agent's own python (sys.executable). Maximum isolation
    ...
    # curated model lists for OpenRouter and VIGIL Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a vigil-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://vigil-agent.nousresearch.com/docs/api/model-catalog.json",
    ...
    "sessions": {
        ...
        # default of ``vigil sessions prune``.
        "retention_days": 90,
    ...
    "updates": {
        # Run a full ``vigil backup``-style zip of VIGIL_HOME before every
        # ``vigil update``.  Backups land in ``<VIGIL_HOME>/backups/`` and
        # can be restored with ``vigil import <path>``.  Defaults to true
        # after the #48200 incident: a ``vigil update --yes`` run that
...
# selection step (VIGIL Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": {
        "description": "VIGIL Portal base URL override",
        "prompt": "VIGIL Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the VIGIL auth store)",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: vigil-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'vigil-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    # VIGIL_TOOL_PROGRESS and VIGIL_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.vigil.config`` entries, then checks
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older VIGIL versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``vigil_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("VIGIL_HOME", "~/.vigil")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("VIGIL_TOOL_PROGRESS")
            old_mode = get_env_value("VIGIL_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from VIGIL_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from VIGIL_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("VIGIL_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from VIGIL_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    #      on top of ensure_vigil_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_vigil_home() / "logs" / "curator"
    ...
    # metadata.vigil.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _vigil_home / "config.yaml"`` local variable
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.vigil/config.yaml.
...
#   nous         (OAuth — vigil auth) — VIGIL Portal
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real VIGIL env var names, avoiding false positives from values
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.vigil/.env or environment."""
...
def show_config():
    ...
    from vigil_cli.auth import get_anthropic_key
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: vigil config set <key> <value>")
            print()
            print("Examples:")
