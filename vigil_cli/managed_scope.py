...
This is DISTINCT from ``vigil_cli.config.is_managed()`` / ``VIGIL_MANAGED``,
...
_DEFAULT_MANAGED_DIR = Path("/etc/vigil")
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through vigil_cli.config.load_config) to honor managed
    scope. Mirrors vigil_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from vigil_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
