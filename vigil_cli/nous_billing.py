"""VIGIL Portal terminal-billing HTTP client (Phase 2b).

Thin, fail-loud client for the four ``/api/billing/*`` endpoints the terminal
billing screens drive. Companion to ``vigil_cli/nous_account.py`` (which owns
...
# vigil_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into VIGIL Portal — run `vigil portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
...
def _request(
    ...
    *,
    ...
    try:
        ...
        raise BillingError(
            f"Could not reach VIGIL Portal: {exc.reason}", error="network_error"
        ) from exc
