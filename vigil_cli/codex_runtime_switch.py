...
The actual runtime resolution happens in vigil_cli.runtime_provider's
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        try:
            from vigil_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the vigil-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "vigil-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            if "vigil-tools" in mig_report.migrated:
                msg_lines.append(
