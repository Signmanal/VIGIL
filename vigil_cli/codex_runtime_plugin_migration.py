...
MIGRATION_END_MARKER = (
    "# end vigil-agent managed section"
)
...
def _translate_one_server(
    ...
    if not isinstance(vigil_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(vigil_cfg.get("command"))
    has_url = bool(vigil_cfg.get("url"))
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="vigil-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    codex-routed vigil-tools call fails silently once the directory is GC'd.
...
def _build_vigil_tools_mcp_entry() -> dict:
    ...
    # (not get_vigil_home()) on purpose: when the env var is unset we want
    ...
    vigil_home = os.environ.get("VIGIL_HOME") or ""
    if vigil_home and _looks_like_test_tempdir(vigil_home):
        vigil_home = ""
    if vigil_home:
        env["VIGIL_HOME"] = vigil_home
    ...
    out: dict[str, Any] = {
        ...
        "args": ["-m", "agent.transports.vigil_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    expose_vigil_tools: bool = True,
) -> MigrationReport:
    """Translate VIGIL mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        vigil_config: full ~/.vigil/config.yaml dict
        ...
        expose_vigil_tools: when True (default), register VIGIL' own
    ...
    vigil_servers = (vigil_config or {}).get("mcp_servers") or {}
    if not isinstance(vigil_servers, dict):
    ...
    for name, cfg in vigil_servers.items():
    ...
    # The server itself is agent/transports/vigil_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_vigil_tools:
        translated["vigil-tools"] = _build_vigil_tools_mcp_entry()
        if "vigil-tools" not in report.migrated:
            report.migrated.append("vigil-tools")
