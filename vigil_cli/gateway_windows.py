...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m vigil_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "VIGIL_Gateway"
...
def _current_profile_cli_args() -> list[str]:
    ...
    from vigil_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "vigil_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    Default profile: ``VIGIL_Gateway``
    Named profile X: ``VIGIL_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during vigil_cli boot.
    from vigil_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    script_dir = Path(get_vigil_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    source checkout only if ``VIGIL_HOME`` cannot be resolved yet.
    """
    from vigil_cli.config import get_vigil_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``pythonw -m vigil_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "VIGIL_HOME={vigil_home}"')
    ...
    # if someone imports vigil_constants-based logic during startup.
    ...
    prog_args = [pythonw_path, "-m", "vigil_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('VIGIL_HOME')}) = {_quote_vbs_string(vigil_home)}",
...
def _write_task_script() -> Path:
    ...
    vigil_home = str(Path(get_vigil_home()).resolve())
    profile_arg = _profile_arg(vigil_home)

    content = _build_gateway_cmd_script(python_path, working_dir, vigil_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, vigil_home, profile_arg)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "vigil_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m vigil_cli.main gateway run``
    ...
    log_dir = Path(get_vigil_home()) / "logs"
...
def _prompt_install_choices(
    ...
    from vigil_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from vigil_cli.gateway import find_gateway_pids, _profile_arg
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from vigil_cli.config import get_vigil_home
        print(f"    type {Path(get_vigil_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_vigil_home()).resolve()}\\logs\\gateway-stdio.log")
...
def _print_next_steps() -> None:
    ...
    print(f"  type {vigil_home}\\logs\\gateway.log       # View logs")
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_vigil_home()).resolve()
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from vigil_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
...
def restart() -> None:
    ...
    """
    _assert_windows()
    from vigil_cli.gateway import kill_gateway_processes
