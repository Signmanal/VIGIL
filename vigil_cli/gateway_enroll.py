...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh VIGIL Portal access token from the existing login
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Connector rejected the caller identity (401). Your VIGIL Portal "
                "token could not be verified — try `vigil auth login nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
