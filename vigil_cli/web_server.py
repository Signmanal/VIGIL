...
Usage:
    python -m vigil_cli.main web          # Start on http://127.0.0.1:9119
    python -m vigil_cli.main web --port 8080
"""
...
from vigil_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
def _warm_gateway_module() -> None:
    try:
        import vigil_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from vigil_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire vigil_cli.gateway import into a background thread so the event
    # loop is not blocked and VIGIL_DASHBOARD_READY fires without delay.
...
from vigil_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("VIGIL_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
#
# This list is defined in ``vigil_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from vigil_cli.dashboard_auth.public_paths import (
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``vigil-0day``
...
async def _dashboard_auth_gate(request: Request, call_next):
    from vigil_cli.dashboard_auth.middleware import gated_auth_middleware
...
class ModelAssignment(BaseModel):
    ...
    # ``vigil model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    """
    from vigil_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from vigil_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from vigil_cli.models import _AGGREGATOR_PROVIDERS
...
def _default_vigil_root_is_opt_data() -> bool:
    ...
    try:
        ...
        root = get_default_vigil_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    a container — e.g. the vigil-webui image sharing the VIGIL source tree),
    ...
    """
    if _default_vigil_root_is_opt_data():
        return True
    try:
        from vigil_constants import is_container
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_vigil_home() at call time
    ...
    try:
        ...
        # Windows install the first import of vigil_cli.gateway blocks the
        ...
        try:
            from vigil_cli.dashboard_auth import list_providers as _list_providers
...
async def get_system_stats():
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_vigil_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_vigil_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# Portal endpoint — VIGIL Portal auth + Tool Gateway routing status (read-only).
# ---------------------------------------------------------------------------
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from vigil_cli.auth import get_nous_auth_status
...
async def run_prompt_size():
    try:
        proc = _spawn_vigil_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_vigil_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_vigil_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from vigil_cli.debug import build_debug_share
...
_ACTION_LOG_DIR: Path = get_vigil_home() / "logs"
...
def _spawn_vigil_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``vigil <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``vigil_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "vigil_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "VIGIL_NONINTERACTIVE": "1"},
    }
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    ...
    return _spawn_vigil_action(subcommand, "gateway-restart"), False
...
async def update_hermes():
    ...
    if _dashboard_local_update_managed_externally():
        ...
        _record_completed_action("vigil-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "vigil-update",
    ...
    try:
        proc = _spawn_vigil_action(["update"], "vigil-update")
    except Exception as exc:
...
@app.get("/api/vigil/update/check")
async def check_vigil_update(force: bool = False):
    ...
    try:
        ...
        if force:
            try:
                (get_vigil_home() / ".update_check").unlink()
            except OSError:
                pass
...
def _memory_provider_config_path(provider: MemoryProvider) -> Path:
    return get_vigil_home() / provider.name / "config.json"
...
# in vigil_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `vigil model`
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (vigil_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from vigil_cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from vigil_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    """
    try:
        from vigil_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from vigil_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "email": {
        ...
        "docs_url": "https://vigil-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "weixin": {
        ...
        "docs_url": "https://vigil-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
    ...
    "webhook": {
        ...
        "docs_url": "https://vigil-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.vigil-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"VIGILDashboard/{__version__}"
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's VIGIL_HOME.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_vigil_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_vigil_oauth_credentials,
            _VIGIL_OAUTH_FILE,
        )
    except ImportError:
        read_vigil_oauth_credentials = None  # type: ignore
        _VIGIL_OAUTH_FILE = None  # type: ignore

    vigil_creds = None
    if read_vigil_oauth_credentials:
        try:
            vigil_creds = read_vigil_oauth_credentials()
        except Exception:
            vigil_creds = None
    if vigil_creds and vigil_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "vigil_pkce",
            "source_label": f"VIGIL PKCE ({_VIGIL_OAUTH_FILE})",
            "token_preview": _truncate_token(vigil_creds.get("accessToken")),
            "expires_at": vigil_creds.get("expiresAt"),
            "has_refresh_token": bool(vigil_creds.get("refreshToken")),
        }
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "docs_url": "https://vigil-agent.nousresearch.com/docs/guides/xai-grok-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from vigil_cli import auth as hauth
        if provider_id == "nous":
            raw = hauth.get_nous_auth_status()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "nous_portal",
                "source_label": raw.get("portal_base_url") or "VIGIL Portal",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("vigil_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _VIGIL_OAUTH_FILE
                if _VIGIL_OAUTH_FILE.exists():
                    _VIGIL_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
            # Also clear the credential pool entry if present.
            try:
                from vigil_cli.auth import clear_provider_auth
        ...
        try:
            from vigil_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    _VIGIL_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _VIGIL_OAUTH_FILE.with_name(
        f"{_VIGIL_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _VIGIL_OAUTH_FILE)
        try:
            _VIGIL_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "vigil-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
# the tokens exactly like `vigil auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `vigil auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from vigil_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from vigil_cli.auth import _save_codex_tokens
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `vigil sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_vigil_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from vigil_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_vigil_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from vigil_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process VIGIL_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (vigil_cli.mcp_config), so
...
async def list_mcp_servers(profile: Optional[str] = None):
    from vigil_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from vigil_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from vigil_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from vigil_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # VIGIL_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    """
    try:
        from vigil_cli import mcp_catalog
    except Exception as exc:
...
#
# Wraps the same JSON store the CLI uses (vigil_cli.webhook); the webhook
...
async def list_webhooks():
    import vigil_cli.webhook as wh
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_vigil_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_vigil_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from vigil_cli.auth import read_credential_pool
...
# `vigil memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `vigil memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_vigil_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_vigil_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_vigil_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_vigil_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    try:
        from vigil_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <vigil_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_vigil_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_vigil_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from vigil_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_vigil_home()
    if default_home.is_dir():
        ...
        profiles.append({
            ...
            "enabled_skill_count": _safe(lambda: profiles_mod._count_enabled_skills(default_home), 0),
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            ...
            profiles.append({
                ...
                "enabled_skill_count": _safe(lambda entry=entry: profiles_mod._count_enabled_skills(entry), 0),
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    token = set_vigil_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    """
    from vigil_constants import set_vigil_home_override, reset_vigil_home_override
    from vigil_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    """
    from vigil_constants import set_vigil_home_override, reset_vigil_home_override
    from vigil_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    _resolve_profile_dir(name)
    try:
        from vigil_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_vigil_home()`` at call
       time — the context-local override from ``set_vigil_home_override``
    ...
    retargeted to the *current* ``get_vigil_home()`` so writes land in the
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_vigil_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_vigil_home()`` at call time
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``vigil tools`` picker uses, so the GUI and CLI stay in
...
if sys.platform.startswith("win"):
    try:
        from vigil_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from vigil_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from vigil_cli.dashboard_auth.audit import AuditEvent, audit_log
        from vigil_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    matching what ``vigil_cli.main._launch_tui`` does for the CLI path.
    ...
    ``get_vigil_home()`` from that env var at its own import, so the child
    ...
    """
    from vigil_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from vigil_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`vigil_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from vigil_cli.dashboard_auth.prefix import normalise_prefix
...
def _discover_user_themes() -> list:
    ...
    """
    themes_dir = get_vigil_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as vigil_cli.plugins):
    ...
    from vigil_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_vigil_home() / "plugins", "user"),
    ...
    # ``vigil_cli/plugins.py`` and the documented user contract.
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_vigil_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from vigil_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from vigil_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from vigil_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"vigil_dashboard_plugin_{plugin['name']}"
...
from vigil_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    # the vigil-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `vigil dashboard register` (VIGIL Portal) or "
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"VIGIL_DASHBOARD_READY port={actual_port}", flush=True)
            print(f"  VIGIL Web UI → http://{host}:{actual_port}")
