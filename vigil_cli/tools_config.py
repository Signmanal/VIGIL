...
from vigil_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from vigil_cli.nous_account import format_nous_portal_entitlement_message
...
from vigil_cli.cli_output import (  # noqa: E402 — late import block
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from vigil_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from vigil_cli.plugins import discover_plugins, get_plugin_toolsets
...
from vigil_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "video_gen": {
        ...
        # FAL video generation billed via the VIGIL Portal.  Plugin-backed
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_vigil_home()}/vigil-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in vigil-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/vigil-agent:latest"
            )
            return
        ...
        try:
            from vigil_cli.auth import login_spotify_command
        except Exception as exc:
        ...
        try:
            from vigil_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
...
def _get_platform_tools(
    ...
    *,
    ...
    # "vigil-cli" (which include all _VIGIL_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[vigil-cli, spotify]`` after enabling Spotify via ``vigil
        ...
        # (e.g. "vigil-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
    ...
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"vigil-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("vigil-")}
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (vigil-cli, vigil-telegram, etc.)
...
def _visible_providers(
    ...
    *,
    ...
    that the capability exists.  Selecting one drives an inline VIGIL Portal
    ...
    *,
    ...
    if len(providers) == 1:
        ...
        for p in providers:
            ...
            # "included" star; everyone else gets a "via VIGIL Portal" hint so
            ...
            if p.get("managed_nous_feature"):
                if _nous_logged_in:
                    ...
                    sub_marker = "  ★ via VIGIL Portal (login on select)"
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on VIGIL Portal auth, or run interactive model
    ...
    *,
    ...
    if managed_feature:
        ...
        if not ensure_nous_portal_access(
            ...
            _print_warning(
                "  Not enabled — VIGIL Portal access is required for this backend."
            )
            return
    ...
    if provider.get("requires_nous_auth") and not managed_feature:
        ...
        if not features.nous_auth_present or not entitled:
            ...
            _print_warning(
                f"  {message or 'Nous Subscription is only available after logging into VIGIL Portal.'}"
            )
            return
    ...
    if _show_portal_hint:
        _print_info("  Available through VIGIL Portal subscription.")
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline VIGIL Portal login + entitlement gate as _configure_provider:
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: https://vigil-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from vigil_constants import display_vigil_home
    print(color(f"  Tool configuration saved to {display_vigil_home()}/config.yaml", Colors.DIM))
