...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # skips the external API sources and serves everything from "vigil-index".
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                "It ships with vigil-agent but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_vigil_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
    ...
    # metadata.vigil.blueprint block, it is a runnable automation. Register it as
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "vigil-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"vigil-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying vigil-index AND the external APIs at once
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_vigil_home()}/.env or run 'gh auth login'.\n")
            return
...
def skills_command(args) -> None:
    """Router for `vigil skills <subcommand>` — called from vigil_cli/main.py."""
