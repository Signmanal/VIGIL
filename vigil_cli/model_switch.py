...
- ``vigil_cli.providers``        -- canonical provider identity + overlays
- ``vigil_cli.model_normalize``  -- per-provider name formatting
...
from vigil_cli.providers import (
    ProviderDef,
...
_VIGIL_MODEL_WARNING = (
    "Nous Research VIGIL 3 & 4 models are NOT agentic and are not designed "
...
# unrelated local Modelfiles like ``vigil-brain:qwen3-14b-ctx16k`` that just
# happen to carry "vigil" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/VIGIL-3-Llama-3.1-70B, vigil-4-405b, openrouter/vigil-3:70b
# Negative examples it must NOT match:
#   vigil-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
...
def is_nous_vigil_non_agentic(model_name: str) -> bool:
...
def _check_vigil_model_warning(model_name: str) -> str:
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from vigil_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            ...
            try:
                from vigil_cli.config import validate_config_structure
        ...
        from vigil_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from vigil_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from vigil_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in vigil_cli.runtime_provider.resolve_runtime_provider;
    ...
    vigil_warn = _check_vigil_model_warning(new_model)
    if vigil_warn:
        warnings.append(vigil_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from vigil_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from vigil_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from vigil_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # https://vigil-agent.nousresearch.com/docs/api/model-catalog.json so
    ...
    if "ollama-cloud" not in curated:
        from vigil_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from vigil_cli.models import fetch_lmstudio_models
    ...
    from vigil_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from vigil_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for vigil_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(vigil_id)
        if (
            _alias_target
            and _alias_target != vigil_id
        ...
        pconfig = PROVIDER_REGISTRY.get(vigil_id)
        ...
        if not has_creds:
            try:
                from vigil_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(vigil_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        model_ids = cached_provider_model_ids(vigil_id)
        if not model_ids:
            model_ids = curated.get(vigil_id, [])
            if vigil_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(vigil_id, model_ids)
        total = len(model_ids)
    ...
    from vigil_cli.providers import VIGIL_OVERLAYS
    from vigil_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    _mdev_to_vigil = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in VIGIL_OVERLAYS.items():
        ...
        vigil_slug = _mdev_to_vigil.get(pid, pid)
        if vigil_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(vigil_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, vigil_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or vigil_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(vigil_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", vigil_slug, exc)
        ...
        # configured.
        if not has_creds and vigil_slug == "anthropic":
            try:
                ...
                if (vigil_creds and vigil_creds.get("accessToken")) or \
        ...
        if vigil_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(vigil_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(vigil_slug)
                model_ids = _ids if _ids else (curated.get(vigil_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(vigil_slug, []) or curated.get(pid, [])
        elif vigil_slug == "nous":
            ...
            try:
                ...
                from vigil_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(vigil_slug, []) or curated.get(pid, [])
                if vigil_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(vigil_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models] if max_models is not None else model_ids

        results.append({
            "slug": vigil_slug,
            "name": get_label(vigil_slug),
            "is_current": vigil_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "vigil",
        ...
        seen_slugs.add(vigil_slug.lower())
        _record_builtin_endpoint(vigil_slug)
    ...
    # with `vigil model`).
    try:
        from vigil_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is VIGIL's canonical write key (matches
            ...
            # VIGIL writes ``models:`` as a dict keyed by model id
            # (see vigil_cli/main.py::_save_custom_provider); older
            ...
            if should_probe:
                try:
                    from vigil_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # VIGIL's own writer uses; a hyphen variant is accepted
            ...
            # run_agent.py, vigil_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`vigil_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from vigil_cli.models import fetch_openrouter_models
