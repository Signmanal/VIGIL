...
from vigil_cli.config import get_project_root, get_vigil_home, get_env_path
from vigil_cli.env_loader import load_vigil_dotenv
from vigil_constants import display_vigil_home
from vigil_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
VIGIL_HOME = get_vigil_home()
_DHH = display_vigil_home()  # user-facing display path (e.g. ~/.vigil or ~/.vigil/profiles/coder)

# Load environment variables from ~/.vigil/.env so API key checks work
_env_path = get_env_path()
load_vigil_dotenv(vigil_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from vigil_cli.colors import Colors, color
from vigil_cli.models import _VIGIL_USER_AGENT
...
from vigil_constants import is_termux as _is_termux
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from vigil_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches vigil_cli.__version__.
    ...
    """
    try:
        from vigil_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != vigil_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'vigil update', or set "
            "vigil_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from vigil_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # Detect drift between pyproject.toml and vigil_cli/__init__.py versions
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from vigil_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from vigil_cli.auth import PROVIDER_REGISTRY, get_auth_status
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from vigil_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from vigil_cli.config import check_config_version, migrate_config
        ...
        try:
            import yaml
            from vigil_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
    ...
    try:
        ...
        if nous_status.get("logged_in"):
            check_ok("VIGIL Portal auth", "(logged in)")
        else:
            check_warn("VIGIL Portal auth", "(not logged in)")
    ...
    for subdir_name in expected_subdirs:
        subdir_path = vigil_home / subdir_name
    ...
    memories_dir = vigil_home / "memories"
    ...
    state_db_path = vigil_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from vigil_state import is_malformed_db_error, repair_state_db_schema
    ...
    wal_path = vigil_home / "state.db-wal"
    if wal_path.exists():
        try:
        ...
        if _venv_bin is None:
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/vigil → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/vigil points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/vigil → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/vigil — run 'vigil doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/vigil exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/vigil not found",
                    "(vigil command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/vigil → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/vigil symlink — run 'vigil doctor --fix'")
    ...
    try:
        from vigil_constants import is_container as _is_container
    ...
    try:
        from vigil_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
