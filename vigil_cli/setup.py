...
_DOCS_BASE = "https://vigil-agent.nousresearch.com/docs"
...
# display_vigil_home imported lazily at call sites (stale-module safety during vigil update)
...
from vigil_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from vigil_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def is_noninteractive() -> bool:
    ...
    ``VIGIL_NONINTERACTIVE=1`` (see ``vigil_cli/web_server.py``). In that
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from vigil_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, vigil_home):
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from vigil_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {vigil_home}/cron/, sessions/, logs/"
    )
    print()
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from vigil_cli.main import select_provider_and_model
    try:
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    """
    try:
        from vigil_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.vigil/.env."
            )
...
def _setup_telegram_auto_result():
    ...
    try:
        from vigil_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_vigil_home(Path(get_vigil_home()))
    except Exception:
        pass
...
def _profile_name_from_vigil_home(vigil_home) -> str | None:
    """Return the active profile name when VIGIL_HOME is a profile dir."""
    if vigil_home.parent.name == "profiles":
        return vigil_home.name
    return None
...
def _setup_qqbot():
    ...
    from vigil_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://vigil-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://vigil-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    from vigil_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if any_messaging:
        ...
        if supports_systemd and has_legacy_vigil_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo vigil gateway install --system")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``vigil_cli.auth`` — lists every supported
    ...
    """
    try:
        from vigil_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from vigil_cli.config
    ...
    if section_key == "model":
        ...
        from vigil_cli.gateway import _all_platforms, _platform_status
...
def _offer_openclaw_migration(vigil_home: Path) -> bool:
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=vigil_home.resolve(),
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot VIGIL Portal setup — OAuth + model pick + provider + Tool Gateway.
    ...
    print(color("│     ◆ VIGIL Setup — VIGIL Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your VIGIL Portal sub.")
    print()
    ...
    try:
        from vigil_cli.main import _model_flow_nous
        ...
        print()
        print_error(f"  VIGIL Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `vigil portal`.")
        return
...
def run_setup_wizard(args):
    ...
    """
    from vigil_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal: one-shot VIGIL Portal setup. Skips the rest of the wizard.
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, vigil_home)
            return
        ...
        migration_ran = _offer_openclaw_migration(vigil_home)
        if migration_ran:
            config = load_config()
        ...
        setup_mode = prompt_choice(
            ...
            [
                "Quick Setup (VIGIL Portal) — free OAuth login, no API keys, model + tools (recommended)",
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, vigil_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, vigil_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {vigil_home}")
    ...
    _print_setup_summary(config, vigil_home)


def _run_first_time_quick_setup(config: dict, vigil_home, is_existing: bool):
    """Streamlined first-time setup via VIGIL Portal: OAuth, model, terminal & messaging.

    Routes straight to the VIGIL Portal provider — runs the device-code OAuth
    ...
    # Step 1: VIGIL Portal — OAuth login + model selection.
    # _model_flow_nous() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Nous model picker). Provider is set to "nous" by the login/model save.
    print()
    print_header("VIGIL Portal")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.nousresearch.com/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("VIGIL Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_nous error during quick setup: %s", exc)
        print_warning(f"VIGIL Portal setup encountered an error: {exc}")
        print_info("You can try again later with: vigil model")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from vigil_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _run_blank_slate_setup(config: dict, vigil_home, is_existing: bool):
    ...
    _blank_slate_walkthrough(config, vigil_home)


def _blank_slate_walkthrough(config: dict, vigil_home):
...
def _run_quick_setup(config: dict, vigil_home):
