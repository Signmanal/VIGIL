...
Usage:
    ...
    vigil honcho setup                    # Configure Honcho AI memory integration
    vigil honcho status                   # Show Honcho config and connection status
    vigil honcho sessions                 # List directory → session name mappings
    vigil honcho map <name>               # Map current directory to a session name
    vigil honcho peer                     # Show peer names and dialectic settings
    vigil honcho peer --user NAME         # Set user peer name
    vigil honcho peer --ai NAME           # Set AI peer name
    vigil honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    vigil honcho mode                     # Show current memory mode
    vigil honcho mode [hybrid|honcho|local]  # Set memory mode
    vigil honcho tokens                   # Show token budget settings
    vigil honcho tokens --context N       # Set session.context() token cap
    vigil honcho tokens --dialectic N     # Set dialectic result char cap
    vigil honcho identity                 # Show AI peer identity representation
    vigil honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    vigil honcho migrate                  # Step-by-step migration guide: OpenClaw native → VIGIL + Honcho
    vigil version             Show version
    vigil update              Update to latest version
    vigil uninstall           Uninstall VIGIL Agent
...
# IMPORTANT: vigil_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``vigil_bootstrap`` is a
...
# new code references ``vigil_bootstrap`` but the editable install's
...
def _set_process_title() -> None:
    ...
    Strategy (try in order):
      ...
      4. No-op on Windows (the .exe name is already ``vigil.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("vigil")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"vigil", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"vigil")
        # Windows: the .exe name is already ``vigil.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* vigil_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        if home:
            ...
            cfg_path = os.path.join(os.path.expanduser("~"), ".vigil", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``VIGIL_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
...
def _print_fast_version_info() -> None:
    from vigil_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"VIGIL Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``vigil --version`` before config/logging imports on Termux."""
...
from vigil_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from vigil_cli.subcommands.cron import build_cron_parser
from vigil_cli.subcommands.gateway import build_gateway_parser
from vigil_cli.subcommands.profile import build_profile_parser
from vigil_cli.subcommands.model import build_model_parser
from vigil_cli.subcommands.setup import build_setup_parser
from vigil_cli.subcommands.postinstall import build_postinstall_parser
from vigil_cli.subcommands.whatsapp import build_whatsapp_parser
from vigil_cli.subcommands.slack import build_slack_parser
from vigil_cli.subcommands.login import build_login_parser
from vigil_cli.subcommands.logout import build_logout_parser
from vigil_cli.subcommands.auth import build_auth_parser
from vigil_cli.subcommands.status import build_status_parser
from vigil_cli.subcommands.webhook import build_webhook_parser
from vigil_cli.subcommands.hooks import build_hooks_parser
from vigil_cli.subcommands.doctor import build_doctor_parser
from vigil_cli.subcommands.security import build_security_parser
from vigil_cli.subcommands.dump import build_dump_parser
from vigil_cli.subcommands.debug import build_debug_parser
from vigil_cli.subcommands.backup import build_backup_parser
from vigil_cli.subcommands.import_cmd import build_import_cmd_parser
from vigil_cli.subcommands.config import build_config_parser
from vigil_cli.subcommands.version import build_version_parser
from vigil_cli.subcommands.update import build_update_parser
from vigil_cli.subcommands.uninstall import build_uninstall_parser
from vigil_cli.subcommands.dashboard import build_dashboard_parser
from vigil_cli.subcommands.gui import build_gui_parser
from vigil_cli.subcommands.logs import build_logs_parser
from vigil_cli.subcommands.prompt_size import build_prompt_size_parser
from vigil_cli.subcommands.memory import build_memory_parser
from vigil_cli.subcommands.acp import build_acp_parser
from vigil_cli.subcommands.tools import build_tools_parser
from vigil_cli.subcommands.insights import build_insights_parser
from vigil_cli.subcommands.skills import build_skills_parser
from vigil_cli.subcommands.pairing import build_pairing_parser
from vigil_cli.subcommands.plugins import build_plugins_parser
from vigil_cli.subcommands.mcp import build_mcp_parser
from vigil_cli.subcommands.claw import build_claw_parser
...
def _apply_profile_override() -> None:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        ...
        candidate = home / ".vigil" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # Mirrors vigil_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # See issue #22502.
    vigil_home_env = os.environ.get("VIGIL_HOME", "")
    if profile_name is None and vigil_home_env:
        if Path(vigil_home_env).parent.name == "profiles":
            return
    ...
    if profile_name is None and not os.environ.get("VIGIL_S6_SUPERVISED_CHILD"):
        try:
            ...
            active_path = get_default_vigil_root() / "active_profile"
    ...
    if profile_name is not None:
        try:
            from vigil_cli.profiles import resolve_profile_env

            vigil_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            vigil_home = _resolve_sudo_user_profile_env(profile_name)
            if not vigil_home:
...
load_vigil_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → VIGIL_REDACT_SECRETS env
# var BEFORE vigil_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_vigil_home() / "config.yaml"
    if _cfg_path.exists():
...
try:
    from vigil_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from vigil_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if vigil_constants not importable yet
...
# Provider model-selection wizard flows extracted to vigil_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (vigil_cli.main._model_flow_*) keep resolving unchanged.
from vigil_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_vigil_home() / "skills" / ".termux_bundled_sync_stamp"
...
def _has_any_provider_configured() -> bool:
    ...
    from vigil_cli.config import get_env_path, get_vigil_home, load_config
    from vigil_cli.auth import get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_vigil_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check for VIGIL Portal OAuth credentials
    auth_file = get_vigil_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, vigil_bin
    ...
    vigil_bin = container_info["vigil_bin"]
...
def _tui_need_npm_install(root: Path) -> bool:
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@vigil" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/vigil-ink/package.json",
    "packages/vigil-ink/index.js",
    "packages/vigil-ink/text-input.js",
)
...
def _ensure_tui_node() -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "VIGIL_HOME": vigil_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _find_bundled_tui(vigil_cli_dir: Path | None = None) -> Path | None:
    ...
    if vigil_cli_dir is None:
        vigil_cli_dir = Path(__file__).parent
    bundled = vigil_cli_dir / "tui_dist" / "entry.js"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    if tui_dev:
        ...
        # packages/vigil-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "vigil-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from vigil_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from vigil_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="vigil-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["VIGIL_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["VIGIL_PYTHON_SRC_ROOT"] = os.environ.get(
        "VIGIL_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("VIGIL_PYTHON", sys.executable)
    env.setdefault("VIGIL_CWD", os.getcwd())
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("VIGIL_KANBAN_BOARD"):
        return
    try:
        from vigil_cli.kanban_db import get_current_board
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``VIGIL_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("VIGIL_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    try:
        from vigil_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from vigil_cli.config import load_config as _load_config_for_xai_check
    ...
    # setup (config, rules files, plugins, MCP servers) or from VIGIL itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (VIGIL_SAFE_MODE is checked by
    # vigil_cli/plugins.py and tools/mcp_tool.py).
...
def cmd_gateway(args):
    ...
    from vigil_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from vigil_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    session_dir = get_vigil_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '◆ VIGIL Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  vigil gateway")
...
def cmd_whatsapp_cloud(args):
    ...
    ``vigil_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from vigil_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from vigil_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from vigil_cli.config import stamp_install_method
    from vigil_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("◆ VIGIL post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    persistence.
    """
    from vigil_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from vigil_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from vigil_cli.providers import resolve_provider_full
    ...
    # PROVIDER_GROUPS in vigil_cli/models.py). A multi-member group shows one
...
def _clear_stale_openai_base_url():
    ...
    """
    from vigil_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`vigil_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  VIGIL Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``vigil model`` flow, then route aux tasks to them here.
    """
    from vigil_cli.config import load_config
    from vigil_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from vigil_cli.auth import _prompt_model_selection
    from vigil_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from vigil_cli.setup import _curses_prompt_choice
...
def _remove_custom_provider(config):
    ...
    try:
        from vigil_cli.curses_ui import curses_radiolist
...
# downstream call sites read `vigil_cli.main._PROVIDER_MODELS` directly,
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from vigil_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    """
    from vigil_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from vigil_cli.env_loader import format_secret_source_suffix
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from vigil_constants import display_vigil_home as _dhh_fn
...
def cmd_login(args):
    ...
    from vigil_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from vigil_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from vigil_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from vigil_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from vigil_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from vigil_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from vigil_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from vigil_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from vigil_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from vigil_cli.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from vigil_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from vigil_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from vigil_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from vigil_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from vigil_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from vigil_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from vigil_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from vigil_cli.banner import format_banner_version_label
    ...
    try:
        from vigil_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall VIGIL Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from vigil_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from vigil_cli.uninstall import run_gui_uninstall
    ...
    from vigil_cli.uninstall import run_uninstall
...
# Critical files that every ``vigil`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``vigil update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "vigil_cli/main.py",
    "vigil_cli/config.py",
    "vigil_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="vigil-syntax-check-") as tmpdir:
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``vigil_cli/web_dist/`` (per vite.config.ts
    outDir: "../vigil_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "vigil_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``vigil`` launcher present but ``vigil_cli`` not importable.
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m vigil_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_vigil_node_path()
...
def _desktop_stamp_path() -> Path:
    ...
    from vigil_constants import get_vigil_home
    return get_vigil_home() / "desktop-build-stamp.json"
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_vigil_node_path(env)
    if mirror:
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "VIGIL is damaged and can't be opened." The
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from vigil_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "vigil_root", None):
        env["VIGIL_DESKTOP_VIGIL_ROOT"] = str(Path(args.vigil_root).expanduser().resolve())
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``vigil dashboard`` processes other than ourselves.
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``vigil dashboard`` as
    ...
    patterns = [
        "vigil dashboard",
        "vigil_cli.main dashboard",
        "vigil_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `vigil_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://github.com/Signmanal/VIGIL/docs/user-guide/features/curator"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``vigil dashboard`` processes.
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.vigil-update-staging"
    backup = f"{dst}.vigil-update-old"
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/Signmanal/VIGIL/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="vigil-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"vigil-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for vigil-agent itself
        ...
        # GitHub ZIPs extract to vigil-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"vigil-agent-{branch}")
    ...
    from vigil_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from vigil_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "vigil-update-autostash-%Y%m%d-%H%M%S"
    )
...
OFFICIAL_REPO_URLS = {
    "https://github.com/Signmanal/VIGIL.git",
    "git@github.com:Signmanal/VIGIL.git",
    "https://github.com/Signmanal/VIGIL",
    "git@github.com:Signmanal/VIGIL",
}
OFFICIAL_REPO_URL = "https://github.com/Signmanal/VIGIL.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_vigil_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_vigil_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from Signmanal/VIGIL.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/Signmanal/VIGIL.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/Signmanal/VIGIL.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    default_home = get_default_vigil_root()
...
# Install-scoped breadcrumb dropped right before ``vigil update`` mutates the
...
# survives and the next ``vigil`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $VIGIL_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``vigil update`` left half-done.
    ...
    try:
        ...
        try:
            from vigil_cli.managed_uv import ensure_uv
...
def _vigil_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "vigil.exe",
        scripts_dir / "vigil-gateway.exe",
    ]


def _detect_concurrent_vigil_instances(
    ...
    The VIGIL Desktop Electron app spawns ``vigil.EXE`` as a backend child,
    ...
    shims (``vigil.exe`` / ``vigil-gateway.exe``) and returns ``(pid,
    ...
    launcher (``vigil.exe``) is a separate process from the Python
    ...
    for shim in _vigil_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated vigil.exe launcher is a separate native process
    ...
    #      vigil.exe sitting *under* a non-VIGIL parent (e.g. a VIGIL
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "vigil.exe"
    lines = ["✗ Another vigil.exe is running:"]
...
def _quarantine_running_vigil_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``vigil.exe``.
    ...
    when ``vigil update`` runs, ``vigil.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``vigil.exe.old.<unix-ms>`` first. uv then writes
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_vigil_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``vigil.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``vigil.exe`` is the running process —
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_vigil_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``vigil.exe.old.*`` left by prior updates.
...
def _refresh_active_lazy_features() -> None:
    ...
    if failed:
        ...
        print("  `vigil update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``vigil.exe`` / ``vigil-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_vigil_exe`` for the rationale.
    """
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    #
    # Quarantine the running ``vigil.exe`` first: ``--reinstall -e .``
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from vigil_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    """
    from vigil_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    nixos_env = with_vigil_node_path(_nixos_build_env())
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # vigil_cli.config.get_vigil_home to simulate setup failure.
        from vigil_cli.config import get_vigil_home as _get_vigil_home

        logs_dir = _get_vigil_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    dropping the flag.
    """
    from vigil_cli.config import detect_install_method
    ...
    if method == "docker":
        ...
        from vigil_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from vigil_cli.config import recommended_update_command
        from vigil_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    # /usr/local/bin/vigil, code at /usr/local/lib/vigil-agent).
    fhs_link = Path("/usr/local/bin/vigil")
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of VIGIL_HOME before running the update.
    ...
    VIGIL_HOME directories.  The ``--backup`` flag on ``vigil update``
    ...
    # ``vigil_cli/config.py``). Fall back to true if the key is missing
    ...
    try:
        from vigil_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_vigil_home so the user sees ~/.vigil/...
    try:
        from vigil_constants import get_vigil_home, display_vigil_home

        home = get_vigil_home()
        try:
            display_path = f"{display_vigil_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    vigil.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from vigil_cli import gateway_windows
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m vigil_cli.main
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    try:
        from vigil_cli import gateway_windows
        from vigil_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def cmd_update(args):
    """Update VIGIL Agent to the latest version.
    ...
    if is_managed():
        managed_error("update VIGIL Agent")
        return
...
def _cmd_update_pip(args):
    """Update VIGIL via pip (for PyPI installs)."""
    from vigil_cli import __version__
    from vigil_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "vigil-agent"]
        ...
        cmd = [pipx, "upgrade", "vigil-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "vigil-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "vigil-agent"]
    ...
    print("✓ Update complete! Restart vigil to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # On Windows, abort early if another vigil.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_vigil_instances(scripts_dir)
            if concurrent:
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://github.com/Signmanal/VIGIL/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # though `git pull` can't touch $VIGIL_HOME, this is cheap
        ...
        try:
            from vigil_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in vigil_cli/config.py bricked
        ...
        # the old bytecode (e.g. get_vigil_home added to vigil_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``vigil`` launch finishes the
        ...
        from vigil_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "vigil_cli.main", "desktop", "--build-only"]
        ...
        # modules in this process.  Reload vigil_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_vigil_home() added since the last release.
        try:
            import importlib
            import vigil_constants as _hc
        ...
        try:
            from vigil_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from vigil_cli.profiles import backfill_profile_envs
        ...
        try:
            from vigil_cli.backup import restore_cron_jobs_if_emptied
        ...
        try:
            if sys.platform in ("darwin", "win32", "linux") and shutil.which("cua-driver"):
                from vigil_cli.tools_config import install_cua_driver
        ...
        if gateway_mode:
            _exit_code_path = get_vigil_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... vigil-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... vigil-gateway*)
            ...
            # Discover all vigil-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. vigil-gateway.service or vigil-gateway-coder.service
        ...
        # Warn if legacy VIGIL gateway unit files are still installed.
        # When both vigil.service (from a pre-rename install) and the
        # current vigil-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `vigil update` surfaces the issue until the user migrates.
        try:
            from vigil_cli.gateway import (
                has_legacy_vigil_units,
                _find_legacy_vigil_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_vigil_units():
                print()
                print("⚠ Legacy VIGIL gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_vigil_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (vigil.service) fight the current")
                print("  vigil-gateway.service for the bot token and cause SIGTERM")
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.vigil)")
            else:
        ...
        from vigil_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from vigil_constants import get_vigil_home as _hh
        ...
        from vigil_cli import profile_describer as _pd
        ...
        from vigil_cli.profiles import profile_exists
        ...
        from vigil_cli.profiles import rename_profile
        ...
        from vigil_cli.profiles import export_profile
        ...
        from vigil_cli.profiles import import_profile
        ...
        import tempfile
        from vigil_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="vigil_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
        ...
        from vigil_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from vigil_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("vigil_requires"):
            print(f"Requires:     VIGIL {data['vigil_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from vigil_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.vigil_requires:
        print(f"  Requires: VIGIL {mf.vigil_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``vigil dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``vigil dashboard --status``
    ...
    print(f"{len(pids)} vigil dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from vigil_cli.web_server import should_require_auth
    ...
    try:
        from vigil_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via VIGIL Portal (run `vigil dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "  It provisions a VIGIL Portal OAuth client and writes "
            "VIGIL_DASHBOARD_OAUTH_CLIENT_ID into ~/.vigil/.env for you.\n"
            "  Docs: https://github.com/Signmanal/VIGIL/docs/"
...
def cmd_dashboard(args):
    ...
    if (
        ...
        reexec_argv = [
            sys.executable, "-m", "vigil_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        # update-button guard also misfires).  get_default_vigil_root()
        ...
        try:
            from vigil_constants import get_default_vigil_root
            env["VIGIL_HOME"] = str(get_default_vigil_root())
        except Exception:
    ...
    if "VIGIL_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "vigil_cli" / "web_dist"
        )
    ...
    from vigil_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    """Register a self-hosted dashboard OAuth client with VIGIL Portal."""
    from vigil_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from vigil_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from vigil_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from vigil_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from vigil_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from vigil_cli.models import CANONICAL_PROVIDERS as _cp
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        "gui", "desktop", "destkop", "kanban", "login", "logout", "logs", "lsp", "mcp", "memory", "migrate",
...
# the top-level flags declared in ``vigil_cli/_parser.py``.
#
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from vigil_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from vigil_cli.oneshot import run_oneshot
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_vigil_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_vigil_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_vigil_home()}/memories/\n")
    else:
        from vigil_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch VIGIL Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from vigil_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from vigil_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from vigil_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from vigil_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from vigil_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from vigil_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from vigil_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from vigil_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from vigil_cli.claw import claw_command
...
def main():
    ...
    # Sweep stale ``vigil.exe.old.*`` quarantine files left by previous
    # ``vigil update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_vigil_exe``.
    try:
    ...
    # =========================================================================
    # model command  (parser built in vigil_cli/subcommands/model.py)
    # =========================================================================
    ...
    # =========================================================================
    from vigil_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://github.com/Signmanal/VIGIL/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.vigil/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://github.com/Signmanal/VIGIL/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from vigil_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from vigil_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in vigil_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in vigil_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in vigil_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in vigil_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    # =========================================================================
    # slack command  (parser built in vigil_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from vigil_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in vigil_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in vigil_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in vigil_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in vigil_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in vigil_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in vigil_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # portal command — VIGIL Portal status + Tool Gateway routing
    # =========================================================================
    from vigil_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from vigil_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    # hooks command  (parser built in vigil_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in vigil_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in vigil_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in vigil_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in vigil_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in vigil_cli/subcommands/backup.py)
    # =========================================================================
    ...
    from vigil_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in vigil_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in vigil_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in vigil_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in vigil_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from vigil_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in vigil_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    try:
        from vigil_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from vigil_cli.pets import register_cli as _register_pets_cli
    ...
    # =========================================================================
    # memory command  (parser built in vigil_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in vigil_cli/subcommands/tools.py)
    # =========================================================================
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            # Honor VIGIL_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from vigil_cli.tools_config import _cua_driver_env
    ...
    # =========================================================================
    # mcp command  (parser built in vigil_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    # =========================================================================
    # insights command  (parser built in vigil_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in vigil_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in vigil_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in vigil_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in vigil_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in vigil_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in vigil_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in vigil_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # =========================================================================
    # gui command  (parser built in vigil_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in vigil_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in vigil_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    from vigil_cli.config import get_container_exec_info
