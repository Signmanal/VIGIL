...
the existing module-level functions in vigil_cli.gateway and
vigil_cli.gateway_windows directly. This protocol is a thin facade
...
# in ``vigil_cli.gateway`` (systemd/launchd) and ``vigil_cli.gateway_windows``
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in vigil_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from vigil_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from vigil_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from vigil_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from vigil_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in vigil_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from vigil_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from vigil_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from vigil_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from vigil_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``vigil_cli.gateway_windows`` (Scheduled Task /
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    vigil_home = Path(os.environ.get("VIGIL_HOME", "/opt/data"))
    if vigil_home.parent.name == "profiles":
        root = vigil_home.parent.parent
    else:
        root = vigil_home
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # Running as the vigil user already — directory is vigil-
    ...
    # when the parent slot's supervise/ is vigil-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        ``vigil_cli.gateway`` maps that empty string to the
        ...
        allocator in ``vigil_cli.profiles._allocate_gateway_port``).
        ...
        lines.append(f"exec s6-setuidgid vigil {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        return (
            ...
            # later then runs its log service as vigil and crash-loops on
            ...
            f'chown vigil:vigil "$VIGIL_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R vigil:vigil "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid vigil s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is vigil-owned (see ``_seed_supervise_skeleton``), and on
