...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-vigil-setup
...
def reconcile_profile_gateways(
    *,
    ...
    not under ``profiles/``). The dispatcher in ``vigil_cli.gateway``
    ...
    Args:
        vigil_home: The container's VIGIL_HOME (typically /opt/data).
            Profiles live under ``<vigil_home>/profiles/<name>/``;
            the default profile lives at ``<vigil_home>`` itself.
    ...
    default_prior_state = legacy_default_state or _read_desired_state(vigil_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(vigil_home)
    ...
    profiles_root = vigil_home / "profiles"
    ...
    if not dry_run:
        _write_reconcile_log(vigil_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = vigil_home / "gateway_state.json"
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    ...
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
    ...
    # The wrapper re-execs `vigil <subcommand>`; peel an explicit vigil.
    if args and Path(args[0]).name == "vigil":
        args = args[1:]
    return args
...
def _write_reconcile_log(
    vigil_home: Path, actions: list[ReconcileAction],
) -> None:
...
def main() -> int:
    ...
    actions = reconcile_profile_gateways(
        vigil_home=vigil_home, scandir=scandir,
    )
    for a in actions:
        print(
