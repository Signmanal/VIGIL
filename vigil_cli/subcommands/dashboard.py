"""``vigil dashboard`` subcommand parser.

Extracted verbatim from ``vigil_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    dashboard_parser.add_argument(
        "--port", type=int, default=9779, help="Port (default 9779, 0 for auto-assign by OS)"
    )
    dashboard_parser.add_argument(
    ...
    # for `vigil dashboard` cmdlines and SIGTERM them directly — the same
    # path `vigil update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running vigil dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running vigil dashboard processes and exit",
    )
    # Backward-compat shim: older VIGIL desktop app shells (<= 0.15.x) spawn the
    ...
    # client with VIGIL Portal and write the client_id into ~/.vigil/.env.
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        "register",
        help="Register a self-hosted dashboard with VIGIL Portal (writes the OAuth client ID to .env)",
        description=(
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://vigil.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        default=None,
        help=(
            "Override the VIGIL Portal base URL for registration (default: the "
