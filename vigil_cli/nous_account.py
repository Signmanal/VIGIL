"""Normalized VIGIL Portal account entitlement helpers."""
...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current VIGIL Portal "
    ...
    if account_info is None:
        return (
            f"VIGIL could not verify your VIGIL Portal entitlement, so {capability} "
    ...
    if reason == "account_missing":
        return (
            f"VIGIL could not find a VIGIL Portal account or organisation for this "
    ...
    return (
        f"Your VIGIL Portal account does not currently have paid service access, "
...
def _no_paid_access_message(
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your VIGIL Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current VIGIL Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your VIGIL Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your VIGIL Portal account has no usable paid credits{credit_detail}, so "
...
def get_nous_portal_account_info(
    *,
    ...
    """Return normalized VIGIL Portal account entitlement information.
...
def _fresh_account_info(
    *,
    ...
    try:
        from vigil_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from vigil_cli.auth import _decode_jwt_claims
    except Exception:
        return None
