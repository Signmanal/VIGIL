...
class NousPortalAdapter(UpstreamAdapter):
    """Proxy upstream for the VIGIL Portal inference API."""
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into VIGIL Portal. Run `vigil auth add nous` first."
                )
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to refresh VIGIL Portal credentials: {exc}"
                ) from exc
            ...
            if not runtime_key:
                raise RuntimeError(
                    "VIGIL Portal refresh did not return a usable inference JWT. "
    ...
    # to vigil_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
