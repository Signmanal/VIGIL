...
logger = logging.getLogger("vigil.security_audit")
...
def _container_no_volume_mount(vigil_home: Optional[Path]) -> Optional[str]:
    if not _in_container():
        return None
    home = vigil_home or Path(
        os.environ.get("VIGIL_HOME", os.path.expanduser("~/.vigil"))
    )
    try:
...
def run_security_audit(
    *, vigil_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(vigil_home)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(vigil_home=vigil_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(
