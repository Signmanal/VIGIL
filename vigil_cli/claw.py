...
from vigil_cli.config import get_vigil_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_vigil.py script in known locations."""
...
def _cmd_migrate(args):
    ...
    print_info(f"Target:      {vigil_home}")
    ...
    # Delegates to vigil_cli.backup.create_pre_migration_backup(), which
    ...
    if not no_backup:
        try:
            from vigil_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(vigil_home=vigil_home)
            if backup_archive:
