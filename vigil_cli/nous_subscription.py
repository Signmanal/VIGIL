...
# coverage category (vigil_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from vigil_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline VIGIL Portal login for the Tool Gateway picker (`vigil tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    backend (e.g. "Firecrawl (VIGIL Portal)").  Unlike ``vigil model``'s Nous
    login, this:
    ...
    It only performs the VIGIL Portal device-code OAuth (when the user isn't
...
def _run_nous_portal_login_only(*, capability: str) -> bool:
    """Run the VIGIL Portal device-code OAuth and persist credentials only.
    ...
    try:
        ...
        print(f"  Could not start VIGIL Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a VIGIL Portal login.")
    try:
        proceed = input("  Log in to VIGIL Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped VIGIL Portal login.")
        return False
    ...
    try:
        ...
        _sync_nous_pool_from_auth_store()
        print("  VIGIL Portal login successful.")
        return True
        ...
        print(f"  VIGIL Portal login failed: {exc}")
        return False
