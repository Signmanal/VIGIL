...
coding workloads.  See ``docs/vigil-kanban-v1-spec.pdf`` for the full
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "vigil_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``vigil-agent``
...
def kanban_home() -> Path:
    ...
    2. ``get_default_vigil_root()``, which already returns ``<root>``
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See vigil_state._WAL_INCOMPAT_MARKERS for detection logic.
                from vigil_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from vigil_cli.profiles import normalize_profile_name
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from vigil_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from vigil_cli.profiles import profile_exists as _pe
...
def _vigil_path_argv(path: str) -> list[str]:
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_vigil_argv()
    return [_absolute_vigil_path(path)]


def _resolve_vigil_argv() -> list[str]:
    ...
    2. ``shutil.which("vigil")`` — the console-script shim, normalized to
       ...
       ``.\\vigil.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m vigil_cli.main`` — fallback for setups where
       VIGIL is launched from a venv and the ``vigil`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_vigil_bin`` for the same reason. Kept
    local (not imported from gateway) because ``vigil_cli`` sits below
    ...
    if env_bin:
        if _looks_like_path(env_bin):
            return _vigil_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _vigil_path_argv(resolved_env_bin)
        return _module_vigil_argv()

    vigil_bin = _safe_which_no_cwd("vigil") if _IS_WINDOWS else shutil.which("vigil")
    if vigil_bin:
        return _vigil_path_argv(vigil_bin)
    return _module_vigil_argv()
...
def _resolve_worker_cli_toolsets(vigil_home: Optional[str]) -> Optional[list[str]]:
    ...
    try:
        ...
        token = set_vigil_home_override(vigil_home)
        try:
            cfg = load_config()
...
def _default_spawn(
    ...
    *,
    ...
    # _apply_profile_override() runs *before* vigil_constants is imported.
    # If VIGIL_HOME is absent from the child's env, get_vigil_home() falls
    ...
    # dispatcher's. Belt-and-braces with the `get_default_vigil_root()`
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled VIGIL_PROFILE values like "vigil-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``vigil_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from vigil_constants import get_default_vigil_root
