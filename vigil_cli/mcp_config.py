...
from vigil_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_vigil_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from vigil_cli.cli_output import prompt as _shared_prompt
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    try:
        ...
        return VIGILTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # vigil_cli/main.py for why the dest is renamed.
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_vigil_home()}/.env as {env_key}")
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_vigil_home()}/config.yaml ({tool_count}/{total} tools enabled)")
