...
def _codex_curated_models() -> list[str]:
    ...
    """
    from vigil_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# ---------------------------------------------------------------------------
# VIGIL Portal free-model helper
# ---------------------------------------------------------------------------
# The VIGIL Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# VIGIL Portal account tier detection
# ---------------------------------------------------------------------------
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current VIGIL Portal user is on a free (unpaid) tier.
    ...
    try:
        from vigil_cli.nous_account import get_nous_portal_account_info
...
# ---------------------------------------------------------------------------
# VIGIL Portal recommended models
#
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_vigil_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    """Fetch the VIGIL Portal's curated recommended-models payload.
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("nous",           "VIGIL Portal",              "VIGIL Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
# sensible default, but VIGIL Portal is a per-token *metered aggregator* whose
...
# path. The *interactive* default (GUI onboarding / ``vigil model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in vigil_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    vigil-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (VIGIL Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from vigil_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — vigil-agent
...
def get_curated_nous_model_ids() -> list[str]:
    """Return the curated VIGIL Portal model-id list.
    ...
    """
    try:
        from vigil_cli.model_catalog import get_curated_nous_models
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, VIGIL Portal).
    """
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for VIGIL Portal pricing.
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from vigil_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from vigil_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``vigil_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    """
    try:
        from vigil_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from vigil_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        # Try live VIGIL Portal /models endpoint
        try:
            from vigil_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``vigil model`` shows.
...
def _provider_models_cache_path() -> Path:
    from vigil_constants import get_vigil_home
    return get_vigil_home() / "provider_models_cache.json"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from vigil_constants import get_vigil_home
    return get_vigil_home() / "ollama_cloud_models_cache.json"
