...
from ``vigil_constants`` or other vigil_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$VIGIL_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``vigil_constants.get_vigil_home`` semantics: env var wins,
    else ``~/.vigil``. A local copy avoids an import cycle with the
    middleware which lives below ``vigil_cli``.
    """
