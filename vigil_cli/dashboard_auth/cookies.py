...
Three cookies in play:
  - vigil_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - vigil_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         VIGIL Portal issues a rotating refresh token for the
  ...
  - vigil_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-vigil_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "vigil_session_at"
SESSION_RT_COOKIE = "vigil_session_rt"
PKCE_COOKIE = "vigil_session_pkce"
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. VIGIL Portal
