...
from vigil_cli.auth import AuthError, resolve_provider
from vigil_cli.colors import Colors, color
from vigil_cli.config import get_env_path, get_env_value, get_vigil_home, load_config
...
from vigil_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    ...
    print(
        f"  {'VIGIL Portal':<12}  {check_mark(nous_logged_in)} "
        f"{nous_label}"
    )
    ...
    if managed_nous_tools_enabled():
        ...
        if not features.nous_auth_present:
            print("  VIGIL Portal   ✗ not logged in")
        else:
            print("  VIGIL Portal   ✓ managed tools available")
    ...
    if _effective_provider_label() == "LM Studio":
        from vigil_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from vigil_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_vigil_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_vigil_home() / "sessions" / "sessions.json"
