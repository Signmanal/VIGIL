"""``vigil portal`` — the human-readable entry point for VIGIL Portal.
...
Subcommands:
  (none)   Log in to VIGIL Portal + set it up (one-shot onboarding).
...
DOCS_URL = "https://vigil-agent.nousresearch.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    print()
    print(color("  VIGIL Portal", Colors.MAGENTA))
    ...
    for feat in features.items():
        if feat.managed_by_nous:
            state = color("via VIGIL Portal", Colors.GREEN)
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into VIGIL Portal — sign in with `vigil portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    """Run the one-shot VIGIL Portal onboarding (login + model + provider + tools).
    ...
    """
    from vigil_cli.setup import _run_portal_one_shot
...
def add_parser(subparsers) -> None:
    ...
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up VIGIL Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `vigil portal` with no subcommand to log in to VIGIL Portal "
    ...
    portal_sub.add_parser(
        "login",
        help="Log in to VIGIL Portal + set it up (default; one-shot onboarding)",
    )
    portal_sub.add_parser(
        "info",
