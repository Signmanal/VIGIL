...
def _build_full_manifest(
    ...
    ``/new``, etc. work inline.
    """
    from vigil_cli.commands import slack_app_manifest
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``vigil_cli/main.py``):
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("VIGIL_HOME") or str(Path.home() / ".vigil")) / "slack-manifest.json"
        else:
