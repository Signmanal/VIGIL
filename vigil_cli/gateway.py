...
# display_vigil_home is imported lazily at call sites to avoid ImportError
# when vigil_constants is cached from a pre-update version during `vigil update`.
...
def _scan_gateway_pids(
    ...
    current_home = str(get_vigil_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"vigil_home={current_home_lc}" in command_lc
            )
        ...
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "vigil_home=" in command_lc
            and f"vigil_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from vigil_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "vigil_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m vigil_cli.main gateway
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    from vigil_cli._subprocess_compat import (
...
def _sync_vigil_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``VIGIL_HOME``.

    Under ``sudo``, ``VIGIL_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_vigil_home` falls back to ``/root/.vigil`` — the wrong
    ...
    unit_home = env.get("VIGIL_HOME", "").strip()
    if not unit_home:
        return
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from vigil_cli.service_manager import detect_service_manager, get_service_manager
...
def _gateway_list() -> None:
    ...
    """
    try:
        from vigil_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (vigil_cli/gateway.py restart
...
from vigil_constants import is_container, is_termux, is_wsl
...
def _profile_suffix() -> str:
    ...
    home = get_vigil_home().resolve()
    default = get_default_vigil_root().resolve()
    if home == default:
        return ""
...
def _profile_arg(vigil_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        vigil_home: Optional explicit VIGIL_HOME path. Defaults to the current
            ``get_vigil_home()`` value. Should be passed when generating a
        ...
        default_root: Optional VIGIL root to compare against. Used when
            ...
            process, where ``Path.home()`` and ``get_default_vigil_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from vigil_constants import get_default_vigil_root

    home = Path(vigil_home or str(get_vigil_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_vigil_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(vigil_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".vigil"
    try:
        Path(vigil_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(vigil_home, default_root=target_root)
    except ValueError:
        return _profile_arg(vigil_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this VIGIL_HOME.

    Default ``~/.vigil`` returns ``vigil-gateway`` (backward compatible).
    Profile ``~/.vigil/profiles/coder`` returns ``vigil-gateway-coder``.
...
# vigil-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (vigil-gateway-*.service) and unrelated third-party
# "vigil" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("vigil.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "vigil_cli.main gateway",
    "vigil_cli/main.py gateway",
    "gateway/run.py",
    " vigil gateway ",
    "/vigil gateway ",
)
...
def _find_legacy_vigil_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy VIGIL gateway units.

    Detects unit files installed by older VIGIL versions that used a
    different service name (e.g. ``vigil.service`` before the rename to
    ``vigil-gateway.service``). When both a legacy unit and the current
    ``vigil-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``vigil-gateway-coder.service`` and unrelated third-party
      ``vigil-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``vigil.service`` running an unrelated
...
def has_legacy_vigil_units() -> bool:
    """Return True when any legacy VIGIL gateway unit files exist."""
    return bool(_find_legacy_vigil_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy VIGIL gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_vigil_units()
    if not legacy:
        return
    print_warning("Legacy VIGIL gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current vigil-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    vigil gateway migrate-legacy")


def remove_legacy_vigil_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy VIGIL gateway unit files.

    Iterates over whatever ``_find_legacy_vigil_units()`` returns — which is
    ...
    legacy = _find_legacy_vigil_units()
    if not legacy:
        print("No legacy VIGIL gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy VIGIL gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so VIGIL can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo vigil gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo vigil gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo vigil gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.vigil`` → ``ai.vigil.gateway.plist`` (backward compatible).
    Profile ``~/.vigil/profiles/coder`` → ``ai.vigil.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.vigil.gateway-{suffix}" if suffix else "ai.vigil.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.vigil/vigil-agent  -> /home/alice/.vigil/vigil-agent
...
def _vigil_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current VIGIL_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_vigil_home() resolves to
    ...
    """
    current_hermes = get_vigil_home().resolve()
    current_default = (Path.home() / ".vigil").resolve()
    target_default = Path(target_home_dir) / ".vigil"

    # Default ~/.vigil → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.vigil → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.vigil) — keep as-is
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    vigil_node = vigil_home / "node" / "bin"
    if _is_dir(vigil_node):
        candidates.append(str(vigil_node))
    vigil_nm = vigil_home / "node_modules" / ".bin"
    if _is_dir(vigil_nm):
        candidates.append(str(vigil_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m vigil_cli.main``, so module
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        vigil_home = _vigil_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(vigil_home, home_dir)
        ...
        working_dir = str(vigil_home) if vigil_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m vigil_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="VIGIL_HOME={vigil_home}"
Restart=always
RestartSec=5
...
"""
    ...
    vigil_home = str(get_vigil_home().resolve())
    profile_arg = _profile_arg(vigil_home)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    ...
    with ``VIGIL_HOME=/tmp/vigil-e2e-<pr>`` exported; the restart path's
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # generated unit bakes that ``/tmp/pytest-of-.../vigil_test`` path into
    ...
    if not system and (
        ...
        or '/vigil_test"' in new_unit
        or "/vigil_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # VIGIL_HOME into the unit (manual E2E homes like /tmp/vigil-e2e-NNN
...
def systemd_install(
    ...
    # Offer to remove legacy units (vigil.service from pre-rename installs)
    # before installing the new vigil-gateway.service. If both remain, they
...
def systemd_stop(system: bool = False):
    ...
    _sync_vigil_home_from_systemd_unit(system=system)
    try:
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.vigil.gateway-{suffix}" if suffix else "ai.vigil.gateway"
...
# (the documented `nohup vigil gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m vigil_cli.main [--profile X] gateway run --replace` argv.
    ...
    """
    cmd = [get_python_path(), "-m", "vigil_cli.main"]
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = get_vigil_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    try:
        from vigil_constants import get_default_vigil_root
        default_root = get_default_vigil_root()
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print(
        ...
        "/opt/vigil/docker/entrypoint.sh before the VIGIL command."
    )
    print(
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_vigil_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from vigil_cli.telegram_managed_bot import (
...
def _setup_weixin():
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_vigil_home())))
    except KeyboardInterrupt:
        print()
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "VIGILAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``vigil_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from vigil_cli import setup as _s
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`vigil_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from vigil_cli.service_manager import (
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            from vigil_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
        ...
        # Stop, disable, and remove legacy VIGIL gateway unit files from
        # pre-rename installs (e.g. vigil.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_vigil_units(interactive=not yes, dry_run=dry_run)
