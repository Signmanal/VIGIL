...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``vigil desktop`` / ``vigil gui``)
     Built inside the agent checkout under ``$VIGIL_HOME/vigil-agent/``:
...
def _agent_root(vigil_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return vigil_home / "vigil-agent"
...
def desktop_userdata_dir() -> Path:
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "VIGIL"
...
def source_built_gui_artifacts(vigil_home: Path) -> "list[Path]":
    ...
    Python agent runs from ``vigil-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(vigil_home)
    ...
    return [
        ...
        vigil_home / "desktop-build-stamp.json",
    ]
...
def agent_is_installed(vigil_home: Path) -> bool:
    ...
    if (agent_root / "vigil_cli").is_dir():
        return True
...
def gui_is_installed(vigil_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(vigil_home):
        if p.exists():
            return True
...
def gui_install_summary(vigil_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = vigil_home if vigil_home is not None else get_vigil_home()
...
def uninstall_gui(vigil_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``vigil-agent/vigil_cli`` (agent source), ``venv/``, or any
