...
Automates what a user otherwise does by hand: open the VIGIL Portal
...
This command:
  1. Resolves a fresh VIGIL Portal access token from the existing login
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    try:
        from vigil_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "VIGIL Portal rejected the access token (401). "
                "Try `vigil auth login nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
        ...
        raise RuntimeError(
            f"Could not reach VIGIL Portal at {portal_base_url}: {exc.reason}"
        ) from exc
...
def _print_post_register_hint(
    *,
    ...
    from vigil_cli.config import get_env_path
...
def cmd_dashboard_register(args) -> None:
    ...
    from vigil_cli.auth import AuthError, resolve_nous_access_token
    from vigil_cli.config import get_env_value, is_managed, save_env_value
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            print("✗ You're not logged into VIGIL Portal.")
            print("  Run `vigil setup` (or `vigil auth login nous`) first, then retry.")
        else:
            print(f"✗ Could not resolve a VIGIL Portal access token: {exc}")
        sys.exit(1)
    ...
    # the portal, e.g. https://vigil.example.com/auth/callback. At serve time
