...
(excluding the vigil-agent repo and transient files).
...
from vigil_constants import get_default_vigil_root, get_vigil_home, display_vigil_home
...
# ``vigil-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/vigil-agent/``
...
_EXCLUDED_DIRS = {
    "vigil-agent",     # the codebase repo — re-clone instead
...
#     stuck "starting"/"cooking", disconnecting it from the VIGIL Portal
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``vigil-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/vigil-agent/`` — must be preserved.
        if part == "vigil-agent" and part != parts[0]:
            continue
        return True
...
def run_backup(args) -> None:
    ...
    vigil_root = get_default_vigil_root()

    if not vigil_root.is_dir():
        print(f"Error: VIGIL home directory not found at {vigil_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"vigil-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"vigil-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_vigil_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(vigil_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(vigil_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``vigil-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "vigil-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(vigil_root)
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_vigil_home()}."
        )
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_vigil_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (vigil_root / "config.yaml").exists()
        has_env = (vigil_root / ".env").exists()
        ...
        for member in members:
            ...
            # VIGIL Portal. Matched by basename so both the root profile and
            # named profiles (profiles/<name>/gateway_state.json) are covered.
            if Path(rel).name in _IMPORT_SKIP_NAMES:
                skipped_runtime.append(rel)
                continue

            target = vigil_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(vigil_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_vigil_home()}."
            )
        ...
        profiles_dir = vigil_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # vigil_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: vigil profile list  (after installing vigil)")

        # Guidance
        print()
        if not (vigil_root / "vigil-agent").is_dir():
            print("Note: The vigil-agent codebase was not included in the backup.")
...
def _quick_snapshot_root(vigil_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(vigil_home)
    if not root.exists():
        return []
...
def restore_cron_jobs_if_emptied(
    ...
    Args:
        ...
        vigil_home: Override for the VIGIL home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(vigil_home), keep=keep)
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_vigil_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, vigil_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``vigil_root`` to ``out_path``.
...
def _pre_update_backup_dir(vigil_home: Optional[Path] = None) -> Path:
    home = vigil_home or get_vigil_home()
...
def create_pre_update_backup(
    ...
    """
    vigil_root = vigil_home or get_default_vigil_root()
    if not vigil_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(vigil_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, vigil_root)
    if result is None:
        return None
