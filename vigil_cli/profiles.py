...
_PROFILE_DIRS = [
    ...
    # See vigil_constants.get_subprocess_home().
    "home",
]
...
#
# Rationale per item:
#   vigil-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    is_default_source = source_resolved == _get_default_vigil_home().resolve()
...
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "vigil-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "vigil", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    """
    return _get_default_vigil_home() / "profiles"


def _get_default_vigil_home() -> Path:
    ...
    return get_default_vigil_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_vigil_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    ...
    if canon == "default":
        return _get_default_vigil_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, vigil subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            vigil_exe = shutil.which("vigil") or "vigil"
...
def _read_profile_config(profile_dir: Path) -> dict:
    """Read a profile's config.yaml as a dict, swallowing malformed legacy files."""
    config_path = profile_dir / "config.yaml"
...
def _count_enabled_toolsets(profile_dir: Path) -> int:
    """Count enabled configurable toolsets for the desktop/CLI profile."""
    try:
        from vigil_cli.tools_config import _get_platform_tools, enabled_mcp_server_names
        ...
        return len(set(enabled) - enabled_mcp_server_names(cfg))
    except Exception:
        return 0


def _count_enabled_mcp_servers(profile_dir: Path) -> int:
    """Count enabled MCP server configs in a profile."""
    servers = _read_profile_config(profile_dir).get("mcp_servers")
    if not isinstance(servers, dict):
        return 0
    return sum(
        1
        for server in servers.values()
        if not (isinstance(server, dict) and server.get("disabled") is True)
    )
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_vigil_home()
    if default_home.is_dir():
        ...
        profiles.append(ProfileInfo(
            ...
            tool_count=_count_enabled_toolsets(default_home),
            mcp_count=_count_enabled_mcp_servers(default_home),
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            ...
            profiles.append(ProfileInfo(
                ...
                tool_count=_count_enabled_toolsets(entry),
                mcp_count=_count_enabled_mcp_servers(entry),
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, vigil_home)`` pairs a gateway should serve.
    ...
    The returned ``vigil_home`` is the path to pass to
    ``set_vigil_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_vigil_home())]
...
def create_profile(
    ...
    if not soul_path.exists():
        try:
            from vigil_cli.default_soul import DEFAULT_SOUL_MD
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_vigil_home() / ".env"
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from vigil_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from vigil_cli.gateway import get_service_name, get_launchd_plist_path
...
def get_active_profile_name() -> str:
    ...
    resolved = vigil_home.resolve()

    default_resolved = _get_default_vigil_home().resolve()
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="vigil_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"vigil_{old_name}"
    legacy_old_host = f"vigil.{old_name}"
    new_host = f"vigil_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_vigil_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("vigil_"):
