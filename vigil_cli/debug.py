...
def _pending_file() -> Path:
    ...
    """
    return get_vigil_home() / "pastes" / "pending.json"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from vigil_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_vigil_home() / "logs" / filename) if filename else None
...
def run_debug_share(args):
    ...
    print(f"To delete now:  vigil debug delete <url>")
