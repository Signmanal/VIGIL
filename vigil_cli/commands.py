...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.vigil/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update VIGIL Agent to the latest version", "Info"),
    CommandDef("version", "Show VIGIL Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`vigil_cli.plugins.PluginContext.register_command`. They behave
...
# native slot, the alias spelling stays reachable via /vigil reset).
...
#   - credits: the billing/top-up surface; reached via /vigil credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /vigil billing.
#   - debug: the log/report upload surface; reached via /vigil debug on Slack.
_SLACK_VIA_VIGIL_ONLY = frozenset({"credits", "billing", "debug"})
...
def slack_app_manifest(request_url: str = "https://vigil-agent.local/slack/commands") -> dict[str, Any]:
