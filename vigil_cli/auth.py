...
Supports OAuth device code flows (VIGIL Portal, future: OpenAI Codex) and
...
from vigil_cli.config import get_vigil_home, get_config_path, read_raw_config
from vigil_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
# VIGIL Portal defaults
DEFAULT_NOUS_PORTAL_URL = "https://portal.nousresearch.com"
DEFAULT_NOUS_INFERENCE_URL = "https://inference-api.nousresearch.com/v1"
DEFAULT_NOUS_CLIENT_ID = "vigil-cli"
...
SPOTIFY_DOCS_URL = "https://vigil-agent.nousresearch.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://vigil-agent.nousresearch.com/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://vigil-agent.nousresearch.com/docs/guides/oauth-over-ssh"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from vigil_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.vigil/.env file
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    try:
        from vigil_cli.nous_account import (
        ...
        pass
    return f"{error} Check credits or billing in VIGIL Portal, then retry."
...
def _auth_file_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".vigil" / "auth.json").resolve(strict=False)
        try:
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_vigil_root()
    except Exception:
        return None
    profile_home = get_vigil_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``get_default_vigil_root()`` for a profile-shaped VIGIL_HOME can
...
# Allowlist of hosts the VIGIL Portal proxy is willing to forward inference
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    if reason is None:
        return
    raise AuthError(
        "VIGIL Portal access token is not a usable inference JWT "
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but VIGIL times out") can produce
            # actionable evidence without a code change.  Logged values are
            # fingerprints / booleans only; no actual code/state strings leak
            # into the log file.  Run with ``VIGIL_LOG_LEVEL=INFO`` (or check
            # ``~/.vigil/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>vigil auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    print("       App name:     anything (e.g. vigil-agent)")
...
# =============================================================================
# VIGIL Portal — token refresh and model discovery
# =============================================================================
...
# ``<vigil-root>/shared/nous_auth.json`` where ``<vigil-root>`` is what
# ``get_default_vigil_root()`` returns — ``~/.vigil`` on Linux/macOS,
...
# typically live under ``<vigil-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    ``<vigil-root>/shared/``, where ``<vigil-root>`` is what
    :func:`vigil_constants.get_default_vigil_root` returns — so
    ...
    from vigil_constants import get_default_vigil_root
    return get_default_vigil_root() / "shared"
...
def _nous_shared_store_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from vigil_constants import get_default_vigil_root
        real_home_shared = (
            get_default_vigil_root() / "shared" / NOUS_SHARED_STORE_FILENAME
...
def _refresh_access_token(
    *,
    ...
    # Detect the OAuth 2.1 "refresh token reuse" signal from the VIGIL Portal
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "VIGIL Portal detected refresh-token reuse and revoked this session.\n"
...
def resolve_nous_access_token(
    *,
    ...
    """Resolve a refresh-aware VIGIL Portal access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        ...
        if not state:
            raise AuthError(
                "VIGIL is not logged into VIGIL Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        with _nous_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for VIGIL Portal login.",
                    provider="nous",
                    relogin_required=True,
                )
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for VIGIL Portal.",
                    provider="nous",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("VIGIL is not logged into VIGIL Portal.",
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for VIGIL Portal login.",
                                provider="nous", relogin_required=True)
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from vigil_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from vigil_cli.config import clear_model_endpoint_credentials
...
def _prompt_model_selection(
    ...
    """
    from vigil_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from vigil_cli.config import save_config, load_config
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=vigil-agent` lets xAI attribute VIGIL-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in VIGIL — please report at "
            "https://github.com/NousResearch/vigil-agent/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize VIGIL with xAI:")
        print(authorize_url)
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    """VIGIL Portal device authorization flow."""
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                print("No curated models available for VIGIL Portal.")
        except Exception as exc:
        ...
        if not selected_model:
            ...
            print("  Run `vigil model` again to switch to VIGIL Portal.")
            return
