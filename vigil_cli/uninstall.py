...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing vigil-agent or vigil PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# VIGIL Agent" comment and following line
                if '# VIGIL Agent' in line or '# vigil-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('vigil' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing vigil
                if 'vigil' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    ...
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "vigil",
        Path("/usr/local/bin/vigil"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains vigil_cli reference)
                content = wrapper.read_text()
                if 'vigil_cli' in content or 'vigil-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(vigil_home: Path) -> list:
    ...
    """
    node_dir = (vigil_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    try:
        from vigil_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from vigil_cli.gateway import get_launchd_plist_path
...
def _vigil_path_markers(vigil_home: Path) -> list[str]:
    """Path-entry substrings that identify VIGIL-owned User-PATH entries."""
    root = str(vigil_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare vigil-agent install dir.
    markers = [root + "\\vigil-agent", root + "\\git", root + "\\node", root + "\\venv"]
    ...
    # unrelated entries like "cvigil-foo" or "ephermeral", so we look for
    # backslash-vigil as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(vigil_home: Path) -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _vigil_path_markers(vigil_home)
            kept: list[str] = []
...
def remove_vigil_env_vars_windows() -> list[str]:
...
def remove_portable_tooling_windows(vigil_home: Path) -> list[Path]:
...
def _is_default_vigil_home(vigil_home: Path) -> bool:
    """Return True when ``vigil_home`` points at the default (non-profile) root."""
    try:
        from vigil_constants import get_default_vigil_root
        return vigil_home.resolve() == get_default_vigil_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m vigil_cli.main` so we don't depend on a `vigil`
    #    wrapper that may be half-removed mid-uninstall.
    vigil_invocation = [_sys.executable, "-m", "vigil_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                vigil_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
...
def run_gui_uninstall(args):
    ...
    """
    from vigil_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(vigil_home)
    ...
    if not summary["gui_installed"]:
        ...
        print(f"  Checked: {vigil_home}, and the standard app locations for this OS.")
        return
    ...
    print()
    if agent_is_installed(vigil_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The VIGIL agent at {vigil_home / 'vigil-agent'}")
        print(f"  • Your config, sessions, and secrets under {vigil_home}")
        print()
...
def run_uninstall(args):
    ...
    is_default_profile = _is_default_vigil_home(vigil_home)
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ◆ VIGIL Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {vigil_home / 'config.yaml'}")
    print(f"  Secrets: {vigil_home / '.env'}")
    print(f"  Data:    {vigil_home / 'cron/'}, {vigil_home / 'sessions/'}, {vigil_home / 'logs/'}")
    print()
...
def _perform_uninstall(
    *,
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in vigil_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\vigil\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(vigil_home))))
        ...
        removed_env = remove_vigil_env_vars_windows()
        if removed_env:
    ...
    removed_node_links = remove_node_symlinks(vigil_home)
    ...
    #     step-5 rmtree(vigil_home) would sweep the in-tree artifacts anyway,
    ...
    try:
        from vigil_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(vigil_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.vigil/, just remove the vigil-agent subdir
            if vigil_home in project_root.parents or project_root.parent == vigil_home:
    ...
    #     the step-5 rmtree(vigil_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(vigil_home)
    ...
    if full_uninstall:
        ...
        try:
            if vigil_home.exists():
                shutil.rmtree(vigil_home)
                log_success(f"Removed {vigil_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {vigil_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {vigil_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://vigil-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://vigil-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using VIGIL Agent! ◆")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m vigil_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import vigil_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``vigil_constants`` + ``vigil_cli.colors``
    (and lazily ``vigil_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m vigil_cli.uninstall")
    parser.add_argument(
        "--mode",
