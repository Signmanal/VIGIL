import { forceRedraw, type MouseTrackingMode } from '@vigil/ink'
...
export const coreCommands: SlashCommand[] = [
  ...
  {
    help: 'update VIGIL Agent to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      if (DASHBOARD_TUI_MODE) {
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys(
            'clipboard copy failed — try VIGIL_TUI_FORCE_OSC52=1 to force the escape sequence'
          )
        }
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `VIGIL #${i + 1}`
