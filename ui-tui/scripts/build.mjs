...
await build({
  ...
  // Skip the prebuilt @vigil/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@vigil/ink': resolve(root, 'packages/vigil-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The vigil_cli launcher
