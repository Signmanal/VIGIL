#!/bin/sh
# shellcheck shell=sh
# /opt/vigil/bin/vigil — `docker exec` privilege-drop shim.
#
# Background
# ----------
...
# Fix
# ---
# This shim sits at /opt/vigil/bin/vigil and is placed earliest on PATH.
...
# (/opt/vigil/.venv/bin/vigil), so the second hop cannot re-enter this
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "vigil-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "vigil-shim: re-run with --user vigil or set VIGIL_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi
