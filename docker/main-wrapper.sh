...
# from the Dockerfile never reaches `vigil`. with-contenv repopulates
# the env from /run/s6/container_environment before exec'ing, which is
# what s6-supervised services use too (see main-vigil/run).
#
# Routing:
...
drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid vigil "$@"; exec "$@"; }
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u vigil)" ]; then
    cat >&2 <<EOF
[vigil] ERROR: container started with --user $cur_uid (an arbitrary, non-vigil UID) — not supported.
...
_vigil_orig_cwd="${VIGIL_ORIG_CWD:-$PWD}"
