...
#
# Wired into the image as /etc/cont-init.d/01-vigil-setup by the
...
as_hermes() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid vigil "$@"; }
...
# starts non-root. The baked install tree under /opt/vigil is intentionally
...
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u vigil)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-vigil UID).
...
and the baked /opt/vigil install tree is intentionally root-owned and
...
The image remaps the vigil user to that UID/GID at boot and chowns the data
...
if [ -n "${VIGIL_UID:-}" ] && validate_uid_gid "$VIGIL_UID" && [ "$VIGIL_UID" != "$(id -u vigil)" ]; then
    echo "[stage2] Changing vigil UID to $VIGIL_UID"
    usermod -u "$VIGIL_UID" vigil
fi
if [ -n "${VIGIL_GID:-}" ] && validate_uid_gid "$VIGIL_GID" && [ "$VIGIL_GID" != "$(id -g vigil)" ]; then
    echo "[stage2] Changing vigil GID to $VIGIL_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$VIGIL_GID" vigil 2>/dev/null || true
fi
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G vigil 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] vigil already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" vigil 2>/dev/null; then
        echo "[stage2] Added vigil to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group vigil failed; docker backend may fail with EACCES"
    fi
    break
done
...
#
# The canonical list of vigil-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_vigil_uid=$(id -u vigil)
needs_chown=false
if [ "$(stat -c %u "$VIGIL_HOME" 2>/dev/null)" != "$actual_vigil_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $VIGIL_HOME (targeted) to vigil ($actual_vigil_uid)"
    ...
    chown vigil:vigil "$VIGIL_HOME" 2>/dev/null || \
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$VIGIL_HOME/$sub" ]; then
            chown -R vigil:vigil "$VIGIL_HOME/$sub" 2>/dev/null || \
...
# VIGIL_DISABLE_LAZY_INSTALLS=1. Keeping /opt/vigil root-owned and
...
if [ -d "$VIGIL_HOME/profiles" ]; then
    chown -R vigil:vigil "$VIGIL_HOME/profiles" 2>/dev/null || true
fi
...
if [ -d "$VIGIL_HOME/cron" ]; then
    chown -R vigil:vigil "$VIGIL_HOME/cron" 2>/dev/null || true
fi

# Reset ownership of vigil-owned top-level state files on every boot.
# The targeted data-volume chown above only covers vigil-owned
...
# entries of vigil_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    if [ -e "$VIGIL_HOME/$f" ]; then
        chown vigil:vigil "$VIGIL_HOME/$f" 2>/dev/null || true
    fi
done
...
if [ -f "$VIGIL_HOME/config.yaml" ]; then
    chown vigil:vigil "$VIGIL_HOME/config.yaml" 2>/dev/null || true
...
# the baked /opt/vigil stamp, so deleting the data-dir copy is safe).
...
if [ -f "$VIGIL_HOME/.env" ]; then
    chown vigil:vigil "$VIGIL_HOME/.env" 2>/dev/null || true
...
if [ -f "$VIGIL_HOME/config.yaml" ]; then
    s6-setuidgid vigil "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$VIGIL_HOME/auth.json" ] && [ -n "${VIGIL_AUTH_JSON_BOOTSTRAP:-}" ]; then
    ...
    chown vigil:vigil "$VIGIL_HOME/auth.json" 2>/dev/null || true
...
if [ ! -f "$VIGIL_HOME/gateway_state.json" ] && \
    ...
    chown vigil:vigil "$VIGIL_HOME/gateway_state.json" 2>/dev/null || true
