...
For Windows, a complete end-user release includes:

- `XCLAW-<version>-win-<arch>.exe` for the NSIS installer.
- `XCLAW-<version>-win-<arch>.msi` for MSI-based installation.
- Any generated Windows `.blockmap` files.
- `latest.yml` when generated by electron-builder.

For Linux, a complete end-user release includes:

- `XCLAW-<version>-linux-<arch>.AppImage` for portable desktop usage.
- `XCLAW-<version>-linux-<arch>.deb` for Debian/Ubuntu-family systems.
- `XCLAW-<version>-linux-<arch>.rpm` for RPM-family systems.
- Any generated Linux `.blockmap` files and `latest-linux.yml` when generated
  by electron-builder.
...
### Desktop Release Update Mode

The `Desktop Release Installers` GitHub Actions workflow builds macOS, Windows,
and Linux desktop installers on their native GitHub-hosted runners, verifies the
macOS DMG contents, and uploads the complete assets to the matching
...
manually. Windows and Linux users follow the same free update UX: the app opens
the Release download page, and users install the installer or package that
