...
sidebar_position: 2
title: "Configuration"
description: "Configure VIGIL Agent — config.yaml, providers, models, API keys, and more"
---
...
Run `vigil setup --portal` — one OAuth gets you a model provider and all four Tool Gateway tools without hand-editing YAML. Portal subscribers also get 10% off token-billed providers. See [VIGIL Portal](/integrations/nous-portal).
:::
...
├── auth.json       # OAuth provider credentials (VIGIL Portal, etc.)
...
Leaving these unset keeps the legacy defaults (`VIGIL_API_TIMEOUT=1800`s, `VIGIL_API_CALL_STALE_TIMEOUT=90`s, native Anthropic 900s). The non-streaming stale detector is auto-disabled for local endpoints when left implicit and can scale upward for very large contexts. Not currently wired for AWS Bedrock (both `bedrock_converse` and AnthropicBedrock SDK paths use boto3 with its own timeout configuration). See the commented example in [`cli-config.yaml.example`](https://github.com/NousResearch/vigil-agent/blob/main/cli-config.yaml.example).
...
```yaml
updates:
  pre_update_backup: false       # Create a full VIGIL_HOME zip before every update
...
VIGIL supports six terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox (direct or via the Nous-managed gateway), a Daytona workspace, or a Singularity/Apptainer container.
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means VIGIL will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
vigil_home = Path(os.environ["VIGIL_HOME"])
...
Every VIGIL-managed container is tagged with three labels so subsequent processes (and the orphan reaper) can identify it:

- `vigil-agent=1` — marks it as VIGIL-managed
- `vigil-task-id=<sanitized task_id>` — keys the per-task reuse probe
- `vigil-profile=<sanitized profile name>` — scopes reuse and reaping to the active VIGIL profile

On startup, VIGIL runs `docker ps --filter label=vigil-task-id=<id> --filter label=vigil-profile=<profile>` and **attaches to the existing container** when it finds one. If the container is `exited` (e.g. after a Docker daemon restart), it's `docker start`'d and reused — filesystem state and any installed packages survive, but in-container background processes do not.
...
| Orphan reaper at next startup | Sweeps **Exited** vigil-labeled containers older than `2 × lifetime_seconds` (default 600s = 10 min), scoped to the current profile. **Running containers are never touched** — sibling-process safety. Set `docker_orphan_reaper: false` to disable. |
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `vigil-{task_id}`.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/vigil-agent` (HPC convention) → `~/.vigil/sandboxes/singularity`.
...
For the **SSH**, **Modal**, and **Daytona** backends (anywhere the agent's working tree lives on a different machine than the host running VIGIL), VIGIL tracks files the agent touched inside the remote sandbox and, on session teardown / sandbox cleanup, **syncs the modified files back to the host** under `~/.vigil/cache/remote-syncs/<session-id>/`.
...
- The remote sandbox may have been torn down by the time you go looking; the local `~/.vigil/cache/remote-syncs/…` copy is the authoritative record of what the agent changed.
...
terminal:
  ...
  docker_volumes:
    ...
    - "/home/user/.vigil/cache/documents:/output" # Gateway-visible exports
```
...
- Emit the **host path** in `MEDIA:`, for example:
  `MEDIA:/home/user/.vigil/cache/documents/report.txt`
...
Controls how much content VIGIL loads from each automatic context file before applying head/tail truncation. This applies to files injected into the system prompt such as `SOUL.md`, `.vigil.md`, `AGENTS.md`, `CLAUDE.md`, and `.cursorrules`. It does **not** affect the `read_file` tool.
...
| Stale stream detection | 180s | Auto-disabled | `VIGIL_STREAM_STALE_TIMEOUT` |
| Stale non-stream detection | 300s | Auto-disabled when left implicit | `providers.<id>.stale_timeout_seconds` or `VIGIL_API_CALL_STALE_TIMEOUT` |
...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="VIGIL Agent — Auxiliary Models Tutorial"
...
| `"nous"` | Force VIGIL Portal | `vigil auth` |
...
When the cap is reached, VIGIL returns a direct limit message for new sessions.
...
quick_commands:
  status:
    type: exec
    command: systemctl status vigil-agent
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.vigil/vigil-agent && git pull && pip install -e .
  gpu:
    type: exec
...
| `.vigil.md` / `VIGIL.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.vigil.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
