...
description: "Run VIGIL Agent natively on Windows 10 / 11 — install, feature matrix, UTF-8 console, Git Bash, gateway as a Scheduled Task, editor handling, PATH, uninstall, and common pitfalls"
...
If you prefer a real POSIX environment (for the dashboard's embedded terminal, `fork` semantics, Linux-style file watchers, etc.), see the **[Windows (WSL2) Guide](./windows-wsl-quickstart.md)**. Both coexist cleanly: native data lives under `%LOCALAPPDATA%\vigil`, WSL data lives under `~/.vigil`.
:::
...
```powershell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/NousResearch/vigil-agent/main/scripts/install.ps1))) -NoVenv -SkipSetup -Branch main
```
...
3. **Installs Node.js 22** (winget if available, else a portable Node tarball unpacked under `%LOCALAPPDATA%\vigil\node`). Used for the browser tool and the WhatsApp bridge.
4. **Installs portable Git** — if `git` is already on PATH the installer uses it; otherwise it downloads a trimmed, self-contained **PortableGit** (~45 MB, from the official `git-for-windows` release) to `%LOCALAPPDATA%\vigil\git`. No admin, no Windows installer registry, no interference with anything else on the box.
5. **Clones the repo** to `%LOCALAPPDATA%\vigil\vigil-agent` and creates a virtualenv inside it.
...
2. `%LOCALAPPDATA%\vigil\git\usr\bin\bash.exe` (installer-managed PortableGit).
3. `%LOCALAPPDATA%\vigil\git\bin\bash.exe` (older Git-for-Windows layout).
...
1. `schtasks /Create /SC ONLOGON /RL LIMITED /TN VIGILGateway` — registers a task that runs at your login with standard (non-elevated) permissions. No UAC prompt.
...
| `%LOCALAPPDATA%\vigil\` (root) | Your config, auth, skills, sessions, logs (`config.yaml`, `.env`, `skills\`, `sessions\`, `logs\`, …). **Survives reinstalls.** |

On native Windows the installer sets `VIGIL_HOME=%LOCALAPPDATA%\vigil`, so your data and the disposable install live under the **same** `%LOCALAPPDATA%\vigil` root: the install/runtime is the `vigil-agent\`, `git\`, `node\`, and `bin\` subdirectories, while your data files sit directly in `%LOCALAPPDATA%\vigil`. Reinstalling only replaces the `vigil-agent\` checkout, so your data survives — but because the two share a root, **don't** `Remove-Item -Recurse %LOCALAPPDATA%\vigil` if you want to keep your data; delete the `vigil-agent\` subdirectory instead. Your data directory is identical in shape to a Linux `~/.vigil`, so you can mirror it between machines.
...
The installer adds `%LOCALAPPDATA%\vigil\vigil-agent\venv\Scripts` to your **User PATH** via `[Environment]::SetEnvironmentVariable`. Existing terminals don't pick this up — open a new PowerShell window (or Windows Terminal tab) after installation. Close-and-reopen, don't `$env:PATH += …` by hand unless you know what you're doing.
...
VIGIL honors both `$env:X` (process-scope) and User environment variables (permanent, set in System Properties → Environment Variables). Setting API keys in `%LOCALAPPDATA%\vigil\.env` (your `VIGIL_HOME`) is the normal path — same as Linux:
