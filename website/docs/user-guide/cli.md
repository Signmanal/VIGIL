...
sidebar_position: 1
title: "CLI Interface"
description: "Master the VIGIL Agent terminal interface — commands, keybindings, personalities, and more"
---
...
The very first time you press Enter while VIGIL is working, VIGIL prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::
...
```
VIGIL Agent has been suspended. Run `fg` to bring VIGIL Agent back.
```
