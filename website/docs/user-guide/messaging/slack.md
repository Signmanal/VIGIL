...
sidebar_position: 4
title: "Slack"
description: "Set up VIGIL Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect VIGIL Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
...
### Option A: From a VIGIL-generated manifest (recommended)
...
4. Enter an app name (e.g., "VIGIL Agent") and select your workspace
5. Click **Create App**
...
| **Channels** | Bot **only responds when @mentioned** (e.g., `@VIGIL Agent what time is it?`). In channels, VIGIL replies in a thread attached to that message. |
...
Make sure the bot has been **invited to the channel** (`/invite @VIGIL Agent`).
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @VIGIL Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @VIGIL Agent` |
...
7. ✅ Bot was **invited** to the channel (`/invite @VIGIL Agent`)
