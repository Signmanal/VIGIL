...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to VIGIL Agent via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With VIGIL Agent's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
flowchart LR
    ...
    B["vigil-agent<br/>gateway API server<br/>port 8642"]
    A -->|POST /v1/chat/completions| B
    B -->|SSE streaming response| A
```

Open WebUI connects to VIGIL Agent's API server just like it would connect to OpenAI. VIGIL handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.
...
If you want VIGIL + Open WebUI wired together locally with a reusable launcher, run:

```bash
cd ~/.vigil/vigil-agent
...
- VIGIL API: `http://127.0.0.1:8642/v1`
- Open WebUI: `http://127.0.0.1:8080`
- model name advertised to Open WebUI: `VIGIL Agent`

Useful overrides:

```bash
OPEN_WEBUI_NAME='My VIGIL UI' \
OPEN_WEBUI_ENABLE_SIGNUP=true \
VIGIL_API_MODEL_NAME='My VIGIL Agent' \
bash scripts/setup_open_webui.sh
```
...
# {"object":"list","data":[{"id":"vigil-agent", ...}]}
```
...
Go to **http://localhost:3000**. Create your admin account (the first user becomes admin). You should see your agent in the model dropdown (named after your profile, or **vigil-agent** for the default profile). Start chatting!
...
Your agent model should now appear in the model dropdown (named after your profile, or **vigil-agent** for the default profile).
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and VIGIL Agent responds accordingly. Each request includes the full conversation history.
...
2. Edit your vigil-agent connection
3. Change **API Type** from "Chat Completions" to **"Responses (Experimental)"**
4. Save

With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and VIGIL Agent can preserve full tool call history across turns via `previous_response_id`. When `stream: true`, VIGIL also streams spec-native `function_call` and `function_call_output` items, which enables custom structured tool-call UI in clients that render Responses events.
...
2. VIGIL Agent creates a server-side `AIAgent` instance using the API server's profile, model/provider config, memory, skills, and configured API-server toolsets
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | VIGIL Agent's API URL (include `/v1`) |
...
- **Check model listing**: `curl -H "Authorization: Bearer your-secret-key" http://localhost:8642/v1/models` should return a list with `vigil-agent`
...
VIGIL Agent may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in VIGIL Agent.
