...
sidebar_position: 15
title: "Weixin (WeChat)"
description: "Connect VIGIL Agent to personal WeChat accounts via the iLink Bot API"
---
...
1. Pair VIGIL once with `hermes gateway setup` and note the connected iLink bot
   account.
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `cd ~/.vigil/vigil-agent && uv pip install -e ".[messaging]"`. Alternatively, open the URL printed above the QR |
