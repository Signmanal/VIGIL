...
The generated plist lives at `~/Library/LaunchAgents/ai.vigil.gateway.plist`. It includes three environment variables:
...
:::info Multiple installations
Like the Linux systemd service, each `VIGIL_HOME` directory gets its own launchd label. The default `~/.vigil` uses `ai.vigil.gateway`; other installations use `ai.vigil.gateway-<suffix>`.
:::
