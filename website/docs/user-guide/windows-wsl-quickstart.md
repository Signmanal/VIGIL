...
description: "Run VIGIL Agent on Windows via WSL2 — setup, filesystem access between Windows and Linux, networking, and common pitfalls"
...
VIGIL Agent now supports **both** native Windows and WSL2.  This page covers the WSL2 path; for the native PowerShell install see the dedicated **[Windows (Native) Guide](./windows-native.md)**.
...
Clone inside WSL. Always, unless you have a specific reason not to. A typical VIGIL workflow (`hermes chat`, tool calls that `rg`/`ripgrep` the repo, file watchers, background gateway) will be dramatically faster and more reliable against `~/code/myrepo` than `/mnt/c/Users/you/myrepo`.
...
home directory, and launches VIGIL. If `hermes` is not on PATH yet, open WSL
...
- **[MCP guide → WSL → Windows Chrome](/guides/use-mcp-with-hermes#wsl2-bridge-hermes-in-wsl-to-windows-chrome)** — controlling your signed-in Windows Chrome from VIGIL in WSL.
