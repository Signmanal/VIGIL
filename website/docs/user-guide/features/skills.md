...
```bash
curl -fsSL https://vigil-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling VIGIL to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.vigil/plans/` relative to the active workspace/backend working directory.
...
description, the standard section order, VIGIL-tool framing, no invented
commands).
...
[Skills Hub](https://vigil-agent.nousresearch.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no VIGIL-side changes required.
...
VIGIL supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

VIGIL source id: `claude-marketplace`
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). VIGIL fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.vigil.tags`, `version`, `author`, `platforms`, `metadata.vigil.config`).
...
If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.vigil/.hub/taps.json`:
