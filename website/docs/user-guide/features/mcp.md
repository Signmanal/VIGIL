...
description: "Connect VIGIL Agent to external tool servers via MCP — and control exactly which MCP tools VIGIL loads"
---

# MCP (Model Context Protocol)

MCP lets VIGIL Agent connect to external tool servers so the agent can use tools that live outside VIGIL itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.
...
`optional-mcps/` in the vigil-agent repo — presence in that directory means
...
the vigil-agent repo, so Nous has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/NousResearch/vigil-agent/tree/main/optional-mcps)
...
VIGIL`) for that entry instead of silently hiding it. Run `hermes update`
...
[`optional-mcps/`](https://github.com/NousResearch/vigil-agent/tree/main/optional-mcps).
...
Then run `hermes mcp login googledrive` — with the pre-registered client, VIGIL skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.vigil/config.yaml` from inside a running VIGIL session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `hermes mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.
...
You can also separately disable VIGIL-added utility wrappers:
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a VIGIL-specific channel browser:
...
The MCP server reads conversation data directly from VIGIL's session store (`~/.vigil/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the VIGIL agent itself.
...
- The embedded `hermes mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of VIGIL, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
