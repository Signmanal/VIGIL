...
(VIGIL will preserve your override on re-migration as long as it lives outside the `# managed by vigil-agent` markers.)
...
What's not migrated:
- VIGIL-specific keys like `sampling` (Codex's MCP client has no equivalent — these are dropped with a per-server warning).
...
This runtime is **opt-in beta**. Working as of VIGIL Agent 2026.5 + Codex CLI 0.130.0:
...
For implementation details, see [PR #24182](https://github.com/NousResearch/vigil-agent/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
