...
sidebar_position: 8
title: "Context Files"
description: "Project context files — .vigil.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every conversation"
---

# Context Files

VIGIL Agent automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the VIGIL instance and is loaded from `VIGIL_HOME` only.
...
Only **one** project context type is loaded per session (first match wins): `.vigil.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
:::
...
VIGIL is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.vigil.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.
...
1. **Scan working directory** — checks for `.vigil.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
