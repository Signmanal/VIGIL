...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [VIGIL Portal](/integrations/nous-portal) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
...
2. **`vigil_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`vigil_cli/runtime_provider.py`** — Pool-aware credential resolution
