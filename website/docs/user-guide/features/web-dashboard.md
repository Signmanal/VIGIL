...
|---------|-----------|--------|----------------|
| `VIGIL_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `hermes dashboard register` |
...
**1. Log in and register the dashboard.** `hermes dashboard register` uses your existing Nous login to provision an OAuth client and writes `VIGIL_DASHBOARD_OAUTH_CLIENT_ID` into `~/.vigil/.env` for you:
