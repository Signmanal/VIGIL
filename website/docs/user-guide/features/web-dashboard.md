...
The web dashboard is a browser-based UI for managing your VIGIL Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses VIGIL Portal OAuth; if you also want the dashboard to talk to a real backend, `vigil setup --portal` wires up the model and tool gateway too. See [VIGIL Portal](/integrations/nous-portal).
:::
...
The default `vigil-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.vigil/vigil-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.vigil/vigil-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.
...
- `ptyprocess` — installed by the `pty` extra (`cd ~/.vigil/vigil-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
To point [VIGIL Desktop](#connecting-vigil-desktop-to-a-remote-backend) at a dashboard running on another machine instead of its own bundled backend, see the remote-backend section below.
...
```ini
[Service]
EnvironmentFile=%h/.vigil/.env
ExecStart=/path/to/venv/bin/python -m vigil_cli.main dashboard \
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and VIGIL versions, CPU core count + utilization, memory, disk usage of the VIGIL home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The VIGIL version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `vigil update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **VIGIL Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `vigil portal`.
...
![System admin page — host stats and VIGIL Portal status](/img/dashboard/admin-system-top.png)
...
| `GET /api/vigil/update/check` | Report update availability (commits behind, install method) without applying. For git/pip installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
| `GET /api/curator` · `PUT .../paused` · `POST .../run` | Skill-curator status + pause/resume + run |
| `GET /api/portal` | VIGIL Portal auth + Tool Gateway routing (read-only) |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — VIGIL Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
- **[OAuth (VIGIL Portal)](#default-provider-nous-research)** — for hosted deployments and any dashboard reachable over the public internet, and the recommended path for a [remote VIGIL Desktop connection](#connecting-vigil-desktop-to-a-remote-backend). Every login is verified against your Nous account, so this is the provider suitable for internet-facing use.
- **[Self-hosted OIDC](#self-hosted-oidc-provider)** — for bringing your own identity provider via standard OpenID Connect (Keycloak, Auth0, Okta, Google, GitHub via an OIDC bridge, etc.). No VIGIL Portal involved; suitable for public-internet exposure when fronted by a conformant OIDC server.
...
Because every login is verified against VIGIL Portal and protected by your Nous account, **the Nous provider is the one suitable for exposing a dashboard to the public internet.**
...
Bundled providers reported these issues:
  • nous: VIGIL_DASHBOARD_OAUTH_CLIENT_ID is not set (and
    dashboard.oauth.client_id in config.yaml is empty). The VIGIL Portal
    provisions this env var (shape 'agent:{instance_id}') when it
    deploys a VIGIL Agent instance — set it to your provisioned
...
```bash
vigil setup            # if you're not already logged into VIGIL Portal
vigil dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes VIGIL_DASHBOARD_OAUTH_CLIENT_ID to ~/.vigil/.env
```
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://vigil.example.com/auth/callback` and set `VIGIL_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
If you run your own identity provider, the bundled `plugins/dashboard_auth/self_hosted` plugin authenticates the dashboard against it using **standard OpenID Connect** — no per-IDP code, no VIGIL Portal involved. It's verified against and works with any conformant OIDC server:
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-vigil.json` — it defines a `vigil` realm, a **public PKCE client** (`vigil-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "vigil-dashboard",
      "name": "VIGIL Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-vigil.json:/opt/keycloak/data/import/realm-vigil.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/vigil/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/vigil`). The admin console is at
...
```bash
export VIGIL_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/vigil"
export VIGIL_DASHBOARD_OIDC_CLIENT_ID="vigil-dashboard"
...
> admin console (Clients → vigil-dashboard → Settings). The same pattern works
...
```yaml
dashboard:
  public_url: "https://dashboard.example.com/vigil"
```
...
The provider implements the [VIGIL Portal OAuth contract v1](https://github.com/NousResearch/nous-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `vigil_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `vigil_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `vigil_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `vigil_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
from vigil_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (VIGIL Portal)** (register it with [`vigil dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Nous Research*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/vigil` are supported if you front it with a reverse proxy)
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `vigil_cli/web_dist/` which the FastAPI server serves as a static SPA.
...
| Theme | Character |
|-------|-----------|
| **VIGIL Signal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **VIGIL Signal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
