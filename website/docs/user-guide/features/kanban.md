...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/vigil-kanban-v1-spec.pdf` in the repository.
...
- **Storage** — files land under
  `<vigil-home>/kanban/attachments/<task_id>/` for the default board, or
  `<vigil-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
...
```json
{
  ...
  "verification": ["pytest tests/vigil_cli/test_kanban_db.py -q"],
...
Every `vigil kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `vigil chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `vigil_cli.kanban.run_slash()` entry point that reuses the `vigil kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `vigil kanban`. You don't have to leave the chat to drive the board.
...
For worked examples of each, see `docs/vigil-kanban-v1-spec.pdf`.
...
The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/vigil-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
