...
|-------|---------|------------|--------|
| **VIGIL Signal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **VIGIL Signal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except VIGIL Signal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
Backend routes run inside the dashboard process, so they can import from the vigil-agent codebase directly:
...
| 3 | `./.vigil/plugins/<name>/dashboard/` | `project` — only when `VIGIL_ENABLE_PROJECT_PLUGINS` is set |
