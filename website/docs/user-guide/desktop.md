...
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#vigil-cron).
...
The [manual update process](https://vigil-agent.nousresearch.com/docs/getting-started/updating) also works with the GUI.
...
| `--vigil-root PATH` | Override the VIGIL source root the app uses (sets `VIGIL_DESKTOP_VIGIL_ROOT`)          |
...
The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the VIGIL Agent runtime into `VIGIL_HOME` (`~/.vigil`, or `%LOCALAPPDATA%\vigil` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `VIGIL_DESKTOP_VIGIL_ROOT`, then a completed managed install, then a probed `vigil` on `PATH` (unless `--ignore-existing` / `VIGIL_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `VIGIL_DESKTOP_HERMES` command override for packagers such as Nix. The React renderer talks to a `vigil dashboard` backend over the `tui_gateway`/dashboard APIs and reuses the agent runtime rather than embedding `vigil --tui`. Install, backend-resolution, and self-update logic live in the Electron main process.
...
:::info The remote backend is a running `vigil dashboard` process
"Remote backend" means a **`vigil dashboard`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that dashboard is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `vigil dashboard` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the dashboard bound to a reachable address. The credentials live in `~/.vigil/.env` (the secrets file, mode 0600):
...
Keep that `vigil dashboard` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.
...
Running the dashboard as a systemd service? Give the unit `EnvironmentFile=%h/.vigil/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/vigil` work if you front it with a reverse proxy)
...
rm "$HOME/.vigil/vigil-agent/.vigil-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.vigil/vigil-agent/venv"

# Reset a stuck macOS microphone prompt
tccutil reset Microphone com.signmanal.vigil
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.vigil/vigil-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
