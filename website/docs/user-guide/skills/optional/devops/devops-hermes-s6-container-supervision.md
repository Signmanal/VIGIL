...
│   └── dashboard/run                  ← if VIGIL_DASHBOARD=1, runs `hermes dashboard`
│
...
- `hermes-tool-quirks`: Specific VIGIL-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with hermes built-in tools.
