...
description: "Frequently asked questions and solutions to common issues with VIGIL Agent"
---
...
VIGIL Agent works with any OpenAI-compatible API. Supported providers include:
...
Set your provider with `hermes model` or by editing `~/.vigil/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Yes, natively.** VIGIL supports native Windows via the PowerShell installer — no WSL required. Run in PowerShell:
...
WSL2 remains a fully supported alternative. To run VIGIL inside WSL2, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and use the standard install command:
...
Yes — VIGIL now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). VIGIL Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.vigil/`.
...
VIGIL Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same VIGIL Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
# Set a valid model
hermes config set VIGIL_MODEL anthropic/claude-opus-4.7
...
cd ~/.vigil/vigil-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
nohup hermes gateway run > ~/.vigil/logs/gateway.log 2>&1 &
```
...
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.vigil.gateway.plist
```
...
cd ~/.vigil/vigil-agent && uv pip install -e ".[mcp]"
...
- [Use MCP with VIGIL](/guides/use-mcp-with-hermes)
...
1. Install VIGIL Agent on the new machine:
   ```bash
   curl -fsSL https://vigil-agent.nousresearch.com/install.sh | bash
   ```
...
```bash
rsync -av --exclude='vigil-agent' ~/.vigil/ newmachine:~/.vigil/
```

:::tip
`hermes backup` produces a consistent snapshot even while VIGIL is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::
...
**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a VIGIL-specific issue — it's a shell config permissions problem.
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/vigil-agent/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), VIGIL version (`hermes --version`), and the full error message
