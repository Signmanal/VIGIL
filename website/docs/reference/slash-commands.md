...
VIGIL has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `vigil_cli/commands.py`:
...
Installed skills are also exposed as dynamic slash commands on both surfaces. That includes bundled skills like `/plan`, which opens plan mode and saves markdown plans under `.vigil/plans/` relative to the active workspace/backend working directory.
...
| `/version` | Show VIGIL Agent version, build, and environment info. |
...
| `/update` | Update VIGIL Agent to the latest version. |
...
**Inline skip:** append `now`, `--yes`, or `-y` to bypass the modal for a single invocation — e.g. `/reset now`, `/new --yes my-session`, `/clear -y`, `/undo -y`. Useful when the modal doesn't render correctly on your terminal (see [issue #30768](https://github.com/NousResearch/vigil-agent/issues/30768) for native Windows PowerShell) or when scripting against the CLI.
