...
| `--profile <name>`, `-p <name>` | Select which VIGIL profile to use for this invocation. Overrides the sticky default set by `hermes profile use`. |
...
| `hermes secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.vigil/.env`. |
...
| `hermes backup` | Back up VIGIL home directory to a zip file. |
| `hermes checkpoints` | Inspect / prune / clear `~/.vigil/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
| `hermes import` | Restore a VIGIL backup from a zip file. |
...
| `hermes acp` | Run VIGIL as an ACP server for editor integration. |
| `hermes mcp` | Manage MCP server configurations and run VIGIL as an MCP server. |
| `hermes plugins` | Manage VIGIL Agent plugins (install, enable, disable, remove). |
...
| `hermes profile` | Manage profiles — multiple isolated VIGIL instances. |
| `hermes completion` | Print shell completion scripts (bash/zsh/fish). |
| `hermes version` | Show version information. |
| `hermes update` | Pull latest code and reinstall dependencies. `--check` previews without installing; `--backup` takes a pre-pull `VIGIL_HOME` snapshot. |
| `hermes uninstall` | Remove VIGIL from the system. |
...
hermes chat --safe-mode -q "Is this bug mine or VIGIL'?"
```
...
# or:
VIGIL_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 hermes -z "…"
```
...
**`hermes model`** (run from your terminal, outside any VIGIL session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.
...
**If you need to add a new provider:** Exit your VIGIL session first (`Ctrl+C` or `/quit`), then run `hermes model` from your terminal prompt.
:::
...
| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `VIGIL_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `hermes update`. |
...
hermes slack manifest --write      # write to ~/.vigil/slack-manifest.json
...
> Not to be confused with `hermes claw migrate` (one-shot import of OpenClaw configuration into VIGIL) — `hermes migrate` is the top-level config-rewrite command.
...
hermes auth spotify                                      # Authenticate VIGIL with Spotify via PKCE
```
...
VIGIL uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `hermes logs list` subcommand shows all log files including rotated ones.
...
See [Plugins](../user-guide/features/plugins.md) and [Build a VIGIL Plugin](../guides/build-a-hermes-plugin.md).
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.vigil/` (by default a single restore-point archive is written to `~/.vigil/backups/pre-migration-*.zip` before apply; restorable with `hermes import`). |
...
`hermes update` pulls the configured update branch (default: `main`). If your checkout is on another branch, VIGIL may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
- **Gateway restart.** After a successful update, VIGIL attempts to restart all running gateway profiles automatically so they pick up the new code. Use `hermes gateway restart` when you want to restart a gateway without applying an update.
...
- **npm lockfile churn.** Before stashing or switching branches, VIGIL makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `hermes update`.
- **Pairing data snapshot.** Even when `--backup` is off, `hermes update` takes a lightweight snapshot of `~/.vigil/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `hermes backup restore --state pre-update` if a pull rewrites a file you were editing.
...
| `hermes postinstall` | Internal bootstrap. Runs once after the install script provisions VIGIL (or after `hermes update`) to install non-Python dependencies that pip cannot provide — Node.js runtime, headless browser, ripgrep, ffmpeg — and then trigger `hermes setup` if the profile has not been configured yet. Safe to re-run idempotently. |
| `hermes uninstall [--full] [--gui] [--yes]` | Remove VIGIL, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--yes` skips prompts. |
