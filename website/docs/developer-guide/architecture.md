...
sidebar_position: 1
title: "Architecture"
description: "VIGIL Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of VIGIL Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ vigil_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # VIGILCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── vigil_state.py           # SQLite session/state database with FTS5
├── vigil_constants.py       # VIGIL_HOME, profile-aware paths
...
│
├── vigil_cli/               # CLI subcommands and setup
...
```text
User input → VIGILCLI.process_input()
...
Three discovery sources: `~/.vigil/plugins/` (user), `.vigil/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `vigil plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-vigil-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
