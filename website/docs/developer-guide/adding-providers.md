...
description: "How to add a new inference provider to VIGIL Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---
...
1. `vigil_cli/auth.py` decides how credentials are found.
2. `vigil_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `vigil_cli/models.py` and `vigil_cli/main.py` make the provider show up in the CLI. (`vigil_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `vigil_cli/auth.py`
2. `vigil_cli/models.py`
3. `vigil_cli/runtime_provider.py`
4. `vigil_cli/main.py`
...
:::tip
`vigil_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `vigil setup`.
:::
...
- OAuth or token refresh (VIGIL Portal, Codex, Qwen Portal, Copilot)
...
- `PROVIDER_REGISTRY` in `vigil_cli/auth.py`
- `_PROVIDER_LABELS` in `vigil_cli/models.py`
- `_PROVIDER_ALIASES` in both `vigil_cli/auth.py` and `vigil_cli/models.py`
- CLI `--provider` choices in `vigil_cli/main.py`
...
## Step 2: Add auth metadata in `vigil_cli/auth.py`
...
## Step 3: Add model catalog and aliases in `vigil_cli/models.py`
...
## Step 4: Resolve runtime data in `vigil_cli/runtime_provider.py`
...
:::tip
`vigil_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `vigil model` and `vigil setup` automatically.
:::
...
- `tests/vigil_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/vigil_cli/test_model_switch_custom_providers.py` (and adjacent `tests/vigil_cli/test_model_switch_*.py`)
- `tests/vigil_cli/test_setup_model_provider.py`
...
python -m pytest tests/vigil_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/vigil_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
...
```bash
source venv/bin/activate
python -m vigil_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `vigil_cli/auth.py`
- [ ] aliases added in `vigil_cli/auth.py` and `vigil_cli/models.py`
- [ ] model catalog added in `vigil_cli/models.py`
- [ ] runtime branch added in `vigil_cli/runtime_provider.py`
- [ ] CLI wiring added in `vigil_cli/main.py` (setup.py inherits automatically)
