...
description: "How to build a memory provider plugin for VIGIL Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give VIGIL Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          vigil_home (str): Active VIGIL_HOME path. Use for storage.
        """
...
| `save_config(values, vigil_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
```python
def save_config(self, values: dict, vigil_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(vigil_home) / "my-provider.json"
...
All storage paths **must** use the `vigil_home` kwarg from `initialize()`, not hardcoded `~/.vigil`:

```python
# CORRECT — profile-scoped
from vigil_constants import get_vigil_home
data_dir = get_vigil_home() / "my-provider"
