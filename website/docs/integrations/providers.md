...
This page covers setting up inference providers for VIGIL Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use VIGIL.
...
**For full details:** see the dedicated [Nous Portal integration page](/integrations/nous-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run VIGIL Agent with Nous Portal guide](/guides/run-hermes-with-nous-portal).

**Client identification.** Every Portal request from VIGIL Agent carries a `client=hermes-client-v<version>` tag (e.g. `client=hermes-client-v0.13.0`) auto-aligned to your installed release. This is sent on all Portal pathways — main chat loop, auxiliary calls, compression summarizer, web extraction — and lets Portal-side telemetry distinguish VIGIL traffic from other clients. No config required; the tag updates automatically when you `hermes update`.
...
When you choose Anthropic OAuth through `hermes model`, VIGIL prefers Claude Code's own credential store over copying the token into `~/.vigil/.env`. That keeps refreshable Claude credentials refreshable.
...
xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.vigil/.env` and pick xAI in `hermes model`, or drop `grok` as a shortcut into `/model grok-4-fast-reasoning`.
...
hermes migrate xai --apply  # rewrite ~/.vigil/config.yaml in place
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `hermes model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and VIGIL auto-discovers the available models.
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `hermes model`, sign in through the browser, and VIGIL persists the refresh token.
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `hermes model`, sign in through the browser, and VIGIL persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
VIGIL Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point VIGIL at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
VIGIL Agent requires at least **64,000 tokens** of context for agent use with tools. Smaller windows are rejected at startup because the system prompt, tool schemas, and working conversation state need enough room for reliable multi-step workflows.
...
Supported parsers: `hermes` (Qwen 2.5, VIGIL 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
VIGIL will automatically load a LM Studio model with 64K context length
...
Since VIGIL Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Then configure VIGIL with `hermes model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure VIGIL with `hermes model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.vigil/config.yaml` and the matching API key goes in `~/.vigil/.env`.
