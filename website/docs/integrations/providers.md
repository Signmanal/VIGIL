...
| Provider | Setup |
|----------|-------|
| **VIGIL Portal** | `vigil model` (OAuth, subscription-based) |
...
**For full details:** see the dedicated [VIGIL Portal integration page](/integrations/nous-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run VIGIL Agent with VIGIL Portal guide](/guides/run-vigil-with-nous-portal).

**Client identification.** Every Portal request from VIGIL Agent carries a `client=vigil-client-v<version>` tag (e.g. `client=vigil-client-v0.13.0`) auto-aligned to your installed release. This is sent on all Portal pathways — main chat loop, auxiliary calls, compression summarizer, web extraction — and lets Portal-side telemetry distinguish VIGIL traffic from other clients. No config required; the tag updates automatically when you `vigil update`.
...
7. **VIGIL Portal** — suffix-matches Nous model IDs against OpenRouter metadata
...
custom_providers:
  ...
  - name: work
    ...
    api_mode: chat_completions   # set explicitly by `vigil model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
The `vigil model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.
...
|----------|-------------|
| **Just want it to work** | OpenRouter (default) or VIGIL Portal |
