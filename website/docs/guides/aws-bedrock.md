...
sidebar_position: 14
title: "AWS Bedrock"
description: "Use VIGIL Agent with Amazon Bedrock — native Converse API, IAM authentication, Guardrails, and cross-region inference"
---

# AWS Bedrock

VIGIL Agent supports Amazon Bedrock as a native provider using the **Converse API** — not the OpenAI-compatible endpoint. This gives you full access to the Bedrock ecosystem: IAM authentication, Guardrails, cross-region inference profiles, and all foundation models.
...
- **boto3** — install with `cd ~/.vigil/vigil-agent && uv pip install -e ".[bedrock]"`
...
cd ~/.vigil/vigil-agent && uv pip install -e ".[bedrock]"
...
After running `hermes model`, your `~/.vigil/config.yaml` will contain:
...
**[sample-vigil-agent-on-aws-with-bedrock](https://github.com/JiaDe-Wu/sample-vigil-agent-on-aws-with-bedrock)** — creates VPC, IAM role, EC2 instance, and configures Bedrock automatically. Deploy in any region with one click.
