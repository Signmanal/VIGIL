...
description: "How to complete browser-based OAuth (xAI, Spotify, MCP servers) when VIGIL runs on a remote machine, container, or behind a jump box"
---

# OAuth over SSH / Remote Hosts

Some VIGIL providers — **xAI Grok OAuth**, **Spotify**, and **remote MCP servers** (Linear, Sentry, Atlassian, Asana, Figma, …) — use a *loopback redirect* OAuth flow. The auth server redirects your browser to `http://127.0.0.1:<port>/callback` so a tiny HTTP listener started by VIGIL can grab the authorization code.
...
The fix is a one-line SSH local-forward — **or**, when you don't have a real SSH client (GCP Cloud Shell, GitHub Codespaces, EC2 Instance Connect, Gitpod, browser-based web IDEs), the new `--manual-paste` flag introduced in [#26923](https://github.com/NousResearch/vigil-agent/issues/26923).
...
Port `56121` is what xAI OAuth uses. For Spotify, replace it with `43827`. VIGIL prints the exact port it bound to on the `Waiting for callback on ...` line — copy it from there.

## Browser-only remote (Cloud Shell / Codespaces / EC2 Instance Connect)

If you don't have a regular SSH client — for example because you're running VIGIL inside GCP Cloud Shell, GitHub Codespaces, AWS EC2 Instance Connect, Gitpod, or another browser-based console — the SSH tunnel above isn't available. Use `--manual-paste` instead:
...
The same flag works on `hermes model --manual-paste` for the integrated model picker. VIGIL accepts three callback paste forms interchangeably: the full URL, a bare `?code=...&state=...` query fragment, or — when the upstream consent page renders the authorization code in-page instead of redirecting (xAI's current behavior on browser-based consoles) — just the bare code value on its own.

VIGIL uses the **same PKCE verifier, state and nonce** for both paths, so the upstream OAuth flow is byte-identical — `--manual-paste` is purely a transport change for the callback hop and is not a security downgrade.

## Which Providers Need This

| Provider | Loopback port | Tunnel needed? |
|----------|---------------|----------------|
| `xai-oauth` (Grok SuperGrok) | `56121` | Yes, when VIGIL is remote |
| Spotify | `43827` | Yes, when VIGIL is remote |
| MCP servers (`auth: oauth`) | auto-picked per server | Yes, when VIGIL is remote |
...
Remote MCP servers (Linear, Sentry, Atlassian, Asana, Figma, etc.) use the same loopback redirect flow. VIGIL auto-picks a free port per server and prints the authorize URL when the OAuth flow kicks off — either at startup (when a new server appears in `mcp_servers:`) or when you run `hermes mcp login <server>`.
...
**Option 2 — SSH port forward (same as xAI / Spotify).** VIGIL prints the exact port it bound to in the SSH-session hint. Open a separate terminal on your laptop:
...
The tokens are written under the Linux user that ran `hermes auth add ...`. If your gateway / systemd service runs as a different user (e.g. `root` or a dedicated `hermes` user), authenticate as **that** user so the tokens land in their `~/.vigil/auth.json`. `sudo -u hermes -i` or equivalent.
