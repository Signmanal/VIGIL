...
```bash
vigil auth add xai-oauth --manual-paste
# Or via the model picker:
vigil model --manual-paste
```
...
This is **not** a stale-token problem — re-running `vigil model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/NousResearch/vigil-agent/issues/26847)).
