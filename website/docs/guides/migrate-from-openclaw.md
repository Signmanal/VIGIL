...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to VIGIL Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`hermes claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into VIGIL. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.vigil/` (by default a single restore-point archive is written before apply, under `~/.vigil/backups/pre-migration-*.zip`; restorable with `hermes import`). |
