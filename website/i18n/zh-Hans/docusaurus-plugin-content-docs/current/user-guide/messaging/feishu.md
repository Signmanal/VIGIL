...
sidebar_position: 11
title: "飞书 / Lark"
description: "将 VIGIL Agent 配置为飞书或 Lark 机器人"
---

# 飞书 / Lark 配置

VIGIL Agent 可作为全功能机器人与飞书和 Lark 集成。连接后，你可以在私信或群聊中与 Agent 对话，在 home chat 中接收 cron job 结果，并通过标准 gateway 流程发送文本、图片、音频和文件附件。
...
Gateway 驱动的更新提示使用原生飞书 `Yes` / `No` 卡片，而非回退到纯文本回复。当 `hermes update --gateway` 需要确认时，适配器将所选答案记录到 VIGIL 的 `.update_response` 文件中，并将卡片内联替换为已解决状态。
