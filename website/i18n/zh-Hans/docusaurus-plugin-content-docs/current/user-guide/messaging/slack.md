...
sidebar_position: 4
title: "Slack"
description: "使用 Socket Mode 将 VIGIL Agent 设置为 Slack 机器人"
---

# Slack 设置

使用 Socket Mode 将 VIGIL Agent 作为机器人连接到 Slack。Socket Mode 使用 WebSocket 而非公开 HTTP 端点，因此你的 VIGIL 实例无需公开访问——它可以在防火墙后、笔记本电脑上或私有服务器上正常运行。
...
2. 点击 **Create New App**
3. 选择 **From scratch**
4. 输入应用名称（例如 "VIGIL Agent"）并选择你的工作区
5. 点击 **Create App**
...
底层实现：VIGIL 附带一个生成的 Slack 应用 manifest（见第一步，方式 A），它将 [`COMMAND_REGISTRY`](https://github.com/NousResearch/vigil-agent/blob/main/vigil_cli/commands.py) 中的每个命令声明为斜杠命令。在 Socket Mode 下，无论 manifest 的 `url` 字段如何，Slack 都会通过 WebSocket 路由命令事件。
...
为了向后兼容旧版 manifest，你仍然可以输入 `/vigil btw run the tests`——VIGIL 会以与 `/btw run the tests` 相同的方式路由它。自由形式的问题也有效：`/vigil what's the weather?` 会被当作普通消息处理。
...
|---------|----------|
| **私信** | 机器人响应每条消息——无需 @ 提及 |
| **频道** | 机器人**仅在被 @ 提及时响应**（例如 `@VIGIL Agent what time is it?`）。在频道中，VIGIL 在该消息附带的话题中回复。 |
...
确保机器人已被**邀请到该频道**（`/invite @VIGIL Agent`）。
...
| 机器人在私信中正常但在频道中不响应 | **最常见问题。** 将 `message.channels` 和 `message.groups` 添加到事件订阅，重新安装应用，并用 `/invite @VIGIL Agent` 邀请机器人加入频道 |
...
| 机器人响应但无法在频道中发帖 | 用 `/invite @VIGIL Agent` 邀请机器人加入频道 |
...
7. ✅ 已**邀请**机器人加入频道（`/invite @VIGIL Agent`）
