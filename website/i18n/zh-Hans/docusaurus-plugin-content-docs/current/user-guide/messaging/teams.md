...
description: "将 VIGIL Agent 设置为 Microsoft Teams 机器人"
---

# Microsoft Teams 设置

将 VIGIL Agent 作为机器人接入 Microsoft Teams。与 Slack 的 Socket Mode 不同，Teams 通过调用**公开 HTTPS webhook**（钩子）来投递消息，因此你的实例需要一个可公开访问的端点——本地开发时使用开发隧道，生产环境使用真实域名。
...
devtunnel port create vigil-bot -p 3978 --protocol https  # 如已修改 TEAMS_PORT，请替换 3978
...
| 隧道 URL 在重启后变更 | 使用命名隧道（`devtunnel create vigil-bot`）时，devtunnel URL 是持久的。ngrok 和 cloudflared 每次运行都会生成新 URL（除非你有付费计划）——URL 变更时请用 `teams app update` 更新机器人端点 |
