...
sidebar_position: 12
title: "Google Chat"
description: "使用 Cloud Pub/Sub 将 VIGIL Agent 设置为 Google Chat 机器人"
---

# Google Chat 设置

将 VIGIL Agent 作为机器人接入 Google Chat。该集成使用 Cloud Pub/Sub 拉取订阅接收入站事件，使用 Chat REST API 发送出站消息。与 Slack Socket Mode 或 Telegram 长轮询的使用体验相当：VIGIL 进程无需公网 URL、隧道或 TLS 证书。它直接连接、认证并监听订阅——就像 Telegram 机器人通过 token 监听一样。
...
- 主体：`vigil-chat-bot@<your-project>.iam.gserviceaccount.com`
...
- **Connection settings**：选择 **Cloud Pub/Sub**，输入 topic 名称 `projects/<your-project>/topics/vigil-chat-events`。
...
安装 Google Chat 适配器所需的依赖（目前没有发布 VIGIL extra，请直接安装）：
