...
| VIGIL Portal | `nous` | `vigil setup --portal`（全新安装）或 `vigil auth add nous`（OAuth） |
...
**以 VIGIL Portal 作为 OpenRouter 的备用：**
```yaml
model:
  provider: openrouter
...
```text
OpenRouter → VIGIL Portal → 自定义端点 → Codex OAuth →
...
```text
主提供商（若支持视觉）→ OpenRouter → VIGIL Portal →
...
| `"nous"` | 强制使用 VIGIL Portal | `vigil auth` |
