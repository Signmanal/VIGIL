...
VIGIL Agent 具备三层弹性机制，在提供商出现问题时保持会话正常运行：
...
`fallback_model`（单数）是旧版单备用键——VIGIL 仍支持以保持向后兼容。`fallback_providers`（复数，列表）支持按顺序尝试多个备用；`hermes fallback` 写入此键。当两者同时设置时，VIGIL 会合并它们，`fallback_providers` 优先。
:::
...
| Qwen Portal（OAuth） | `qwen-oauth` | `hermes model`（Qwen Portal OAuth；可选：`VIGIL_QWEN_BASE_URL`） |
