...
**→ [构建 VIGIL Plugin](/guides/build-a-vigil-plugin)** — 包含完整可运行示例的分步指南。
...
`./.vigil/plugins/` 下的项目本地插件默认禁用。仅对可信仓库启用，方法是在启动 VIGIL 前设置 `VIGIL_ENABLE_PROJECT_PLUGINS=true`。
...
| 通过 pip 分发 | `[project.entry-points."vigil_agent.plugins"]` |
...
| 项目 | `.vigil/plugins/` | 项目专属插件（需要 `VIGIL_ENABLE_PROJECT_PLUGINS=true`） |
| pip | `vigil_agent.plugins` entry_points | 分发包 |
| Nix | `services.vigil-agent.extraPlugins` / `extraPythonPackages` | NixOS 声明式安装 — 参见 [Nix Setup](/getting-started/nix-setup#plugins) |
...
| 想要添加… | 方式 | 编写指南 |
|---|---|---|
| LLM 可调用的**工具** | Python 插件 — `ctx.register_tool()` | [Build a VIGIL Plugin](/guides/build-a-vigil-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| **生命周期 hook**（LLM 前后、会话开始/结束、工具过滤） | Python 插件 — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a VIGIL Plugin](/guides/build-a-vigil-plugin) |
| CLI / gateway 的**斜杠命令** | Python 插件 — `ctx.register_command()` | [Build a VIGIL Plugin](/guides/build-a-vigil-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
...
完整的处理器约定、schema 格式、hook 行为、错误处理和常见错误请参见 **[完整指南](/guides/build-a-vigil-plugin)**。