...
请参阅 [插件](/user-guide/features/plugins) 页面了解通用插件系统，以及 [构建 VIGIL 插件](/guides/build-a-hermes-plugin) 了解如何编写自己的插件。
...
将 VIGIL 的轮次、LLM 调用和工具调用追踪到 [Langfuse](https://langfuse.com)——一个开源 LLM 可观测性平台。每轮一个 span，每次 API 调用一个 generation，每次工具调用一个 tool observation。用量总计、各类型 token 数量和成本估算来自 VIGIL 的标准 `agent.usage_pricing` 数据，因此 Langfuse 仪表盘看到的分类（input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`）与 `hermes logs` 中显示的一致。
...
或在交互式 `hermes plugins` UI 中勾选复选框。然后将凭据写入 `~/.vigil/.env`：
...
会话分组基于 VIGIL 会话 ID（或子 agent 的任务 ID），通过 `langfuse.propagate_attributes` 实现，因此单次 `hermes chat` 会话中的所有内容都归属于同一个 Langfuse session。

**验证：**

```bash
hermes plugins list                 # observability/langfuse 应显示 "enabled"
hermes chat -q "hello"              # 在 Langfuse UI 中检查是否有 "VIGIL turn" trace
```
...
**禁用：** `hermes plugins disable google_meet`。已缓存的转录和录音保留在 `~/.vigil/cache/google_meet/`，直到你手动删除。
...
内置插件的编写方式与其他 VIGIL 插件完全相同——参见 [构建 VIGIL 插件](/guides/build-a-hermes-plugin)。唯一的区别是：
