...
:::tip 快速开始
Tool Gateway 包含在付费 VIGIL Portal 订阅中。**[管理订阅 →](https://portal.nousresearch.com/manage-subscription)**
:::

**Tool Gateway** 让已付费的 [VIGIL Portal](https://portal.nousresearch.com) 用户通过同一份订阅，直接使用网页搜索、文生图、语音合成（TTS）与浏览器自动化，而**不必**再分别注册 Firecrawl、FAL、OpenAI、Browser Use 等服务的 API Key。
...
Tool Gateway 仅对 **[付费](https://portal.nousresearch.com/manage-subscription)** VIGIL Portal 订阅开放；免费档不可用——请 [升级订阅](https://portal.nousresearch.com/manage-subscription) 后解锁。
...
运行 `vigil model` 并选择 VIGIL Portal 作为提供商时，VIGIL 会主动询问是否启用 Tool Gateway：
...
网关使用 VIGIL Portal 凭据认证（在 `vigil model` 完成后写入 `~/.vigil/auth.json`）。
