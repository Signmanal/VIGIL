...
CSS 在主题应用时以单个带作用域的 `<style data-vigil-theme-css>` 标签注入，主题切换时清除。**每个主题上限为 32 KiB。**
...
| 主题 | 调色板 | 字体排版 | 布局 |
|-------|---------|------------|--------|
| **VIGIL Signal**（`default`） | 深青色 + 奶油色 | 系统字体栈，15px | 0.5rem 圆角，comfortable |
| **VIGIL Signal (Large)**（`default-large`） | 同 default | 系统字体栈，18px，行高 1.65 | 0.5rem 圆角，spacious |
...
引用 Google Fonts 的主题（除 VIGIL Signal 外均如此）会按需加载样式表——首次切换时会向 `<head>` 注入一个 `<link>` 标签。
...
参考插件（[`vigil-example-plugins`](https://github.com/NousResearch/vigil-example-plugins/tree/main/example-dashboard) 中的 `example-dashboard`）提供了一个向 `sessions:top` 注入横幅的实时演示——安装它可端到端了解该模式。
...
后端路由在 dashboard 进程内运行，因此可以直接从 vigil-agent 代码库导入：
...
| 3 | `./.vigil/plugins/<name>/dashboard/` | `project`——仅在设置 `VIGIL_ENABLE_PROJECT_PLUGINS` 时生效 |
...
[`strike-freedom-cockpit`](https://github.com/NousResearch/vigil-example-plugins/tree/main/strike-freedom-cockpit) 插件（伴随仓库 `vigil-example-plugins`）是一个完整的换肤演示。它将主题 YAML 与仅插槽插件配对，在不 fork dashboard 的情况下生成驾驶舱风格的 HUD。
...
打开 dashboard，从主题切换器中选择 **Strike Freedom**。驾驶舱侧边栏出现，徽标显示在顶栏，标语替换底栏。切换回 **VIGIL Signal**，插件仍然安装但不可见（`sidebar` 插槽仅在 `cockpit` 布局变体下渲染）。
