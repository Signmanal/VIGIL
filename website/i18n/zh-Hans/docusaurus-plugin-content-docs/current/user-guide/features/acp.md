...
sidebar_position: 11
title: "ACP 编辑器集成"
description: "在 VS Code、Zed 和 JetBrains 等兼容 ACP 的编辑器中使用 VIGIL Agent"
---

# ACP 编辑器集成

VIGIL Agent 可作为 ACP 服务器运行，让兼容 ACP 的编辑器通过 stdio 与 VIGIL 通信并渲染：
...
VIGIL 官方 ACP Registry 元数据的源文件位于：
...
上游 registry PR 将这些文件复制到 `agentclientprotocol/registry` 中的顶层 `vigil-agent/` 目录。
...
Provider 解析使用 VIGIL 的正常运行时解析器，因此 ACP 继承当前配置的 provider 和凭据。VIGIL 还为首次运行的 registry 客户端提供终端认证方法（`--setup`）；这将打开 VIGIL 的交互式模型/provider 设置。
...
从官方 uv 安装文档安装 `uv`，然后从 Zed 重试 VIGIL Agent 线程。
