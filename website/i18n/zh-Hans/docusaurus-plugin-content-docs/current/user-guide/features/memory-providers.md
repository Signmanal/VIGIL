...
VIGIL Agent 内置 8 个外部记忆提供者插件，为 Agent 提供跨会话的持久化知识，超越内置的 MEMORY.md 和 USER.md。同一时间只能激活**一个**外部提供者——内置记忆始终与其并行工作。
...
**配置：** `$VIGIL_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$VIGIL_HOME/honcho.json` > `~/.vigil/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/vigil-ai/vigil-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/vigil)。
...
`--clone` 在 `honcho.json` 中创建一个 `vigil.coder` host 块，包含 `aiPeer: "coder"`、共享的 `workspace`、继承的 `peerName`、`recallMode`、`writeFrequency`、`observation` 等。AI peer 会在 Honcho 中提前创建，确保在第一条消息之前就已存在。
...
参见[配置参考](https://github.com/vigil-ai/vigil-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/vigil)。
...
| `retain_context` | `conversation between VIGIL Agent and the User` | 保留记忆的上下文标签 |
...
完整配置参考参见[插件 README](https://github.com/NousResearch/vigil-agent/blob/main/plugins/memory/hindsight/README.md)。
...
**配置：** `plugins.vigil-memory-store` 下的 `config.yaml`
...
- **Profile 范围容器**——在 `container_tag` 中使用 `{identity}`（例如 `vigil-{identity}` → `vigil-coder`），按 VIGIL profile 隔离记忆
