...
**配置：** `$VIGIL_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$VIGIL_HOME/honcho.json` > `~/.vigil/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/hermes-ai/vigil-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/hermes)。
...
| **AI peer**（`aiPeer`） | 每个 VIGIL profile 一个。host key `hermes` → 默认；其他 profile 使用 `hermes.<profile>`。 |
...
扫描所有 VIGIL profile，为没有 host 块的 profile 创建 host 块，从默认 `hermes` 块继承设置，并提前创建新的 AI peer。幂等操作——跳过已有 host 块的 profile。
...
参见[配置参考](https://github.com/hermes-ai/vigil-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/hermes)。
