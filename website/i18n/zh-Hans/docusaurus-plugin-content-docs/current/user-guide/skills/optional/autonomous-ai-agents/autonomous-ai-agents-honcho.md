...
| 相关 skills | [`vigil-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-vigil-agent) |
...
运行 `hermes honcho setup`。确保 `~/.vigil/config.yaml` 中包含 `memory.provider: honcho`。
...
| `hermes honcho sync` | 为所有尚未拥有 host 块的 VIGIL 配置文件创建 host 块 |
| `hermes honcho migrate` | 从 OpenClaw 原生记忆迁移到 VIGIL + Honcho 的分步指南 |
