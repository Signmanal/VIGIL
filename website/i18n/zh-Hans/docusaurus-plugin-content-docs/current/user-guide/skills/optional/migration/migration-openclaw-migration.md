---
title: "Openclaw Migration — 将用户的 OpenClaw 自定义配置迁移到 VIGIL Agent"
sidebar_label: "Openclaw Migration"
description: "将用户的 OpenClaw 自定义配置迁移到 VIGIL Agent"
---
...
将用户的 OpenClaw 自定义配置迁移到 VIGIL Agent。从 `~/.openclaw` 导入 VIGIL 兼容的记忆、`SOUL.md`、命令白名单、用户技能及所选工作区资产，并精确报告无法迁移的内容及原因。
...
| 版本 | `1.0.0` |
| 作者 | VIGIL Agent (Nous Research) |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `Migration`, `OpenClaw`, `VIGIL`, `Memory`, `Persona`, `Import` |
| 相关 skill | [`vigil-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-vigil-agent) |
...
当用户希望以最少的手动清理将其 OpenClaw 配置迁移到 VIGIL Agent 时，使用此 skill。
