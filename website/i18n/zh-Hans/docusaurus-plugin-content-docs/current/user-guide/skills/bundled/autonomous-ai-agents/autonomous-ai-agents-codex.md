...
| 相关 skill | [`claude-code`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`vigil-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-vigil-agent) |
...
对于 VIGIL 本身，`model.provider: openai-codex` 会在执行 `hermes auth add openai-codex` 后使用 `~/.vigil/auth.json` 中 VIGIL 管理的 Codex OAuth。对于独立的 Codex CLI，有效的 CLI OAuth 会话可能存储在 `~/.codex/auth.json` 中；不要仅凭缺少 `OPENAI_API_KEY` 就认为 Codex 认证缺失。
