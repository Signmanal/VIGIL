...
VIGIL Agent 采用纵深防御安全模型。本页涵盖所有安全边界——从命令审批到容器隔离，再到消息平台上的用户授权。
...
| `pkill`/`killall` vigil/gateway | 防止自我终止 |
...
路径相对于 `~/.vigil/`。文件在容器内挂载到 `/root/.vigil/`。
...
VIGIL 内置了一个公告扫描器，用于标记活跃 venv 中与已知受损版本目录匹配的 Python 包（例如 2026 年 5 月的 `mistralai 2.4.6` 供应链投毒事件）。实现位于 `vigil_cli/security_advisories.py`。
...
许多功能（Mistral TTS、ElevenLabs、Honcho 记忆、Bedrock、Slack、Matrix 等）依赖并非每个用户都需要的 Python 包。VIGIL 在首次使用时**懒加载**安装这些包，而非在 `vigil-agent[all]` 下急切安装。实现位于 `tools/lazy_deps.py`。
