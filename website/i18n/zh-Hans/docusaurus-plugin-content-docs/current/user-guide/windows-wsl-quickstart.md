...
一个例外：**启动 Windows 二进制文件的 MCP bridge。** 如果你通过 `cmd.exe` 使用 `chrome-devtools-mcp`（参见 [MCP 指南：WSL → Windows Chrome](/guides/use-mcp-with-vigil#wsl2-bridge-vigil-in-wsl-to-windows-chrome)），当 VIGIL 的当前工作目录是 `~` 时，Windows 可能会报 `UNC` 警告。此时请从 `/mnt/c/` 下的某个目录启动 VIGIL，以便 Windows 进程拥有一个带盘符的工作目录。
...
- **[MCP 指南 → WSL → Windows Chrome](/guides/use-mcp-with-vigil#wsl2-bridge-vigil-in-wsl-to-windows-chrome)** —— 从 WSL 中的 VIGIL 控制你已登录的 Windows Chrome。
