...
:::note
你无法删除默认 profile（`~/.vigil`）。如需删除所有内容，请使用 `hermes uninstall`。
:::
...
profile 使用 `VIGIL_HOME` 环境变量。运行 `coder chat` 时，包装脚本在启动 hermes 前将 `VIGIL_HOME` 设置为 `~/.vigil/profiles/coder`。由于代码库中 119+ 个文件通过 `get_hermes_home()` 解析路径，VIGIL 状态会自动限定在 profile 目录范围内——包括配置、会话、记忆、技能、状态数据库、gateway PID、日志和 cron 任务。
