...
sidebar_position: 5
title: "内置技能目录"
description: "随 VIGIL Agent 附带的内置技能目录"
---
...
VIGIL 在执行 `hermes update` 时也会同步内置技能，但同步清单会尊重本地删除和用户编辑。如果此处列出的某个技能在你的 `~/.vigil/skills/` 目录树中缺失，它仍随 VIGIL 一同发布；可通过 `hermes skills reset <name> --restore` 恢复。
...
| [`vigil-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-vigil-agent) | 配置、扩展或贡献 VIGIL Agent。 | `autonomous-ai-agents/vigil-agent` |
...
| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`vigil-agent-skill-authoring`](/user-guide/skills/bundled/software-development/software-development-vigil-agent-skill-authoring) | 编写仓库内 SKILL.md：frontmatter、验证器、结构规范。 | `software-development/vigil-agent-skill-authoring` |
...
| [`plan`](/user-guide/skills/bundled/software-development/software-development-plan) | 计划模式：将 Markdown 计划写入 `.vigil/plans/`，不执行。 | `software-development/plan` |
