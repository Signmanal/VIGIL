...
| `vigil plugins` | 管理 VIGIL Agent plugin（安装、启用、禁用、删除）。 |
| `vigil portal` | VIGIL Portal 状态、订阅链接和 Tool Gateway 路由。参见 [Tool Gateway](../user-guide/features/tool-gateway.md)。 |
...
- 登录基于 OAuth 的 provider（Anthropic、Copilot、Codex、VIGIL Portal）
- 输入或更新 API 密钥
...
:::tip WSL 用户
使用 `vigil gateway run` 而非 `vigil gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s vigil 'vigil gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-vigil-gateway-start-fails)。
:::
...
| `--portal` | 一键 VIGIL Portal 设置：通过 OAuth 登录，将 Nous 设为推理 provider，并选择加入 [Tool Gateway](../user-guide/features/tool-gateway.md)。跳过向导其余部分。 |
...
检查 VIGIL Portal 认证、Tool Gateway 路由，并访问订阅页面。不带子命令时运行 `status`。
...
完整设计——与 Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise 的对比、八种协作模式、四个用户故事、并发正确性证明——请参阅仓库中的 `docs/vigil-kanban-v1-spec.pdf` 或 [Kanban 用户指南](/user-guide/features/kanban)。
...
| 选项 | 说明 |
|--------|-------------|
| `-o`, `--output <path>` | zip 文件的输出路径（默认：`~/vigil-backup-<timestamp>.zip`）。 |
...
```bash
vigil backup                           # 完整备份到 ~/vigil-backup-*.zip
vigil backup -o /tmp/vigil.zip        # 完整备份到指定路径
...
### 示例
```bash
vigil import ~/vigil-backup-20260423.zip           # 覆盖现有配置前提示确认
vigil import ~/vigil-backup-20260423.zip --force   # 不提示直接覆盖
```
...
vigil skills install https://sharethis.chat/SKILL.md                     # 直接 URL（单文件 SKILL.md）
...
参见 [MCP 配置参考](./mcp-config-reference.md)、[在 VIGIL 中使用 MCP](../guides/use-mcp-with-vigil.md) 和 [MCP 服务器模式](../user-guide/features/mcp.md#running-vigil-as-an-mcp-server)。
...
参见 [Plugins](../user-guide/features/plugins.md) 和 [构建 VIGIL Plugin](../guides/build-a-vigil-plugin.md)。
...
- **旧版 `vigil.service` 警告。** 如果 VIGIL 检测到预重命名的 `vigil.service` systemd 单元（而非当前的 `vigil-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
