...
|--------|-------------|
| `--version`, `-V` | 显示版本并退出。 |
| `--profile <name>`, `-p <name>` | 选择本次调用使用的 VIGIL profile（配置文件）。覆盖 `hermes profile use` 设置的粘性默认值。 |
...
| `hermes backup` | 将 VIGIL 主目录备份为 zip 文件。 |
| `hermes checkpoints` | 检查/修剪/清除 `~/.vigil/checkpoints/`（`/rollback` 使用的影子存储）。不带参数运行可查看状态概览。 |
| `hermes import` | 从 zip 文件恢复 VIGIL 备份。 |
...
| `hermes acp` | 将 VIGIL 作为 ACP 服务器运行，用于编辑器集成。 |
| `hermes mcp` | 管理 MCP 服务器配置，并将 VIGIL 作为 MCP 服务器运行。 |
| `hermes plugins` | 管理 VIGIL Agent plugin（安装、启用、禁用、删除）。 |
...
| `hermes profile` | 管理 profile——多个隔离的 VIGIL 实例。 |
| `hermes completion` | 打印 shell 补全脚本（bash/zsh/fish）。 |
| `hermes version` | 显示版本信息。 |
| `hermes update` | 拉取最新代码并重新安装依赖。`--check` 预览而不安装；`--backup` 在拉取前对 `VIGIL_HOME` 进行快照。 |
| `hermes uninstall` | 从系统中删除 VIGIL。 |
...
**`hermes model`**（从终端运行，在任何 VIGIL 会话外部）是**完整的 provider 设置向导**。它可以添加新 provider、运行 OAuth 流程、提示输入 API 密钥并配置端点。

**`/model`**（在活跃的 VIGIL 聊天会话中输入）只能**在已设置好的 provider 和模型之间切换**。它无法添加新 provider、运行 OAuth 或提示输入 API 密钥。

**如果需要添加新 provider：** 先退出 VIGIL 会话（`Ctrl+C` 或 `/quit`），然后从终端提示符运行 `hermes model`。
:::
...
| 选项 | 说明 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：对**每个 profile** 的 gateway 执行操作，而不仅限于活跃的 `VIGIL_HOME`。当你并行运行多个 profile 并希望在 `hermes update` 后全部重启时很有用。 |
...
hermes slack manifest --write      # 写入 ~/.vigil/slack-manifest.json
...
hermes auth spotify                                      # 通过 PKCE 将 VIGIL 与 Spotify 认证
```
...
创建 VIGIL 配置、skill、会话和数据的 zip 归档。备份不包含 vigil-agent 代码库本身。
...
- `vigil-agent` 代码本身（这是用户数据备份，不是仓库快照）。
...
VIGIL 使用 Python 的 `RotatingFileHandler`。旧日志会自动轮转——查找 `agent.log.1`、`agent.log.2` 等。`hermes logs list` 子命令显示所有日志文件，包括已轮转的。
...
参见 [Plugins](../user-guide/features/plugins.md) 和 [构建 VIGIL Plugin](../guides/build-a-hermes-plugin.md)。
...
`hermes update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你现在就想要的修复，而不想等待下次 VIGIL 更新时，使用此选项。
...
| `--no-backup` | 跳过迁移前对 `~/.vigil/` 的 zip 快照（默认情况下，在应用前会将单个还原点归档写入 `~/.vigil/backups/pre-migration-*.zip`；可用 `hermes import` 恢复）。 |
...
启动 Web 控制台——基于浏览器的界面，用于管理配置、API 密钥和监控会话。需要 `cd ~/.vigil/vigil-agent && uv pip install -e ".[web]"`（FastAPI + Uvicorn）。内嵌浏览器 Chat 标签页始终可用，但额外需要 `pty` extra（`cd ~/.vigil/vigil-agent && uv pip install -e ".[web,pty]"`）以及 POSIX PTY 环境（如 Linux、macOS 或 WSL2）。完整文档请参阅 [Web 控制台](/user-guide/features/web-dashboard)。
...
拉取最新的 `vigil-agent` 代码并在受管理的 venv 中重新安装依赖，然后重新运行安装后 hook（MCP 服务器、skill 同步、补全安装）。可在运行中的安装上安全执行。使用 `--check` 查看你的检出是否落后于 `origin/main`，而不安装。
...
- **配对数据快照。** 即使 `--backup` 关闭，`hermes update` 也会在 `git pull` 前对 `~/.vigil/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用 `hermes backup restore --state pre-update` 回滚。
...
| `hermes uninstall [--full] [--yes]` | 删除 VIGIL，可选择删除所有 config/数据。 |
