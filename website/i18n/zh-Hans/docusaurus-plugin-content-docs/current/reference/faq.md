...
sidebar_position: 3
title: "常见问题与故障排查"
description: "VIGIL Agent 常见问题解答及常见问题解决方案"
---
...
VIGIL Agent 可与任何兼容 OpenAI 的 API 配合使用。支持的提供商包括：

- **[OpenRouter](https://openrouter.ai/)** — 通过一个 API key 访问数百个模型（推荐，灵活性强）
- **VIGIL Portal** — Nous Research 自有推理端点
...
**原生不支持。** VIGIL Agent 需要类 Unix 环境。在 Windows 上，请安装 [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) 并在其中运行 VIGIL。标准安装命令在 WSL2 中可完美运行：
...
- [在 VIGIL 中使用 MCP](../guides/use-mcp-with-vigil.md#wsl2-bridge-vigil-in-wsl-to-windows-chrome)
...
API 调用**仅发送至您配置的 LLM 提供商**（例如 OpenRouter、您本地的 Ollama 实例）。VIGIL Agent 不收集遥测数据、使用数据或分析数据。您的对话、记忆和技能均存储在本地 `~/.vigil/` 目录中。
...
VIGIL Agent 本身**免费且开源**（MIT 许可证）。您只需为所选提供商的 LLM API 用量付费。本地模型完全免费运行。

### 多人可以使用同一个实例吗？

可以。[消息网关](../user-guide/messaging/index.md)允许多个用户通过 Telegram、Discord、Slack、WhatsApp 或 Home Assistant 与同一个 VIGIL Agent 实例交互。访问权限通过白名单（特定用户 ID）和私信配对（第一个发消息的用户获得访问权）来控制。
...
```bash
# 安装核心消息网关依赖项
cd ~/.vigil/vigil-agent && uv pip install -e ".[messaging]"  # Telegram、Discord、Slack 及共享网关依赖
...
Profiles 是构建在 `VIGIL_HOME` 之上的托管层。您*可以*在每次命令前手动设置 `VIGIL_HOME=/some/path`，但 profiles 会为您处理所有底层工作：创建目录结构、生成 shell 别名（`vigil-work`）、在 `~/.vigil/active_profile` 中跟踪活动 profile，以及自动跨所有 profiles 同步技能更新。它们还与 tab 补全集成，让您无需记忆路径。
...
2. 在**源机器**上创建完整备份：
   ...
   vigil backup
   ```
   这会将您整个 `~/.vigil/` 目录（配置、API key、记忆、技能、会话和 profiles）打包为 zip 文件，保存到主目录 `~/vigil-backup-<timestamp>.zip`。
...
1. **搜索现有 issue：** [GitHub Issues](https://github.com/NousResearch/vigil-agent/issues)
