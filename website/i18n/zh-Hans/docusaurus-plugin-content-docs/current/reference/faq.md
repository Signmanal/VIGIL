...
使用 `hermes model` 设置提供商，或直接编辑 `~/.vigil/.env`。所有提供商 key 请参阅[环境变量](./environment-variables.md)参考文档。
...
:::tip
`hermes backup` 即使在 VIGIL 正在运行时也能生成一致的快照。还原的归档文件不包含机器本地的运行时文件，如 `gateway.pid` 和 `cron.pid`。
:::
...
3. **提交 bug 报告：** 请包含您的操作系统、Python 版本（`python3 --version`）、VIGIL 版本（`hermes --version`）以及完整的错误信息