...
vigil acp / vigil-acp / python -m acp_adapter
  ...
  -> construct VIGILACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

Zed ACP Registry 路径通过 `uvx --from 'vigil-agent[acp]==<version>' vigil-acp` 启动同一适配器，指向 `vigil-agent` PyPI 发布包。
...
因此 ACP 通告并使用当前配置的 VIGIL provider/凭据。它还始终通告一个终端 setup 认证方法（`vigil-setup`，参数 `--setup`），以便首次运行的 registry 客户端在启动正常 ACP 会话前可以打开 VIGIL 的交互式模型/provider 配置。
...
- `tests/acp/` — ACP 测试套件
- `toolsets.py` — `vigil-acp` toolset 定义
- `vigil_cli/main.py` — `vigil acp` CLI 子命令
- `pyproject.toml` — `[acp]` 可选依赖 + `vigil-acp` 脚本