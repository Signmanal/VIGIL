...
sidebar_position: 8
title: "程序化集成"
description: "从外部程序驱动 vigil-agent 的三种协议：ACP、TUI gateway JSON-RPC 以及兼容 OpenAI 的 HTTP API"
---
...
Pi-mono RPC 规范（[issue #360](https://github.com/NousResearch/vigil-agent/issues/360)）中的每条命令均有对应的 TUI gateway 等价项：
...
内置 provider 感知解析（相同的模型名称会根据当前 provider 自动选择正确格式）。参见 `vigil_cli/model_switch.py`。
