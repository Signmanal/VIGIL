...
### Windows：另一个 `vigil.exe` 正在运行

在 Windows 上，如果 `vigil update` 检测到另一个 `vigil.exe` 进程持有 venv 入口点可执行文件的句柄，它将拒绝运行 — 最常见的情况是 VIGIL Desktop 应用启动的后端进程、另一个终端中打开的 `vigil` REPL，或正在运行的 gateway：
