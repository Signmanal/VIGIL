...
执行 `nix profile install` 后，`hermes`、`vigil-agent` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`hermes setup` 引导你完成提供商选择，`hermes gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.vigil/`。
...
设置 `addToSystemPackages = true` 有两个作用：将 `hermes` CLI 添加到系统 PATH，**并**在系统范围内设置 `VIGIL_HOME`，使交互式 CLI 与 gateway 服务共享状态（会话、技能、cron）。不设置此项时，在 shell 中运行 `hermes` 会创建独立的 `~/.vigil/` 目录。
:::
...
- 对于在 hermes 代码库上工作的开发者，设置 `VIGIL_DEV=1` 可绕过容器路由，直接运行本地检出版本
...
| 覆盖 hermes 包 | `package` | `inputs.vigil-agent.packages.${system}.default.override { ... }` |
...
2. **`.managed` 标记文件**，位于 `VIGIL_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it vigil-agent hermes config set ...` 也会被屏蔽）
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `hermes` 和 `vigil-agent` 二进制文件存在且 `hermes version` 可运行 |
...
| `managed-guard` | `VIGIL_MANAGED=true hermes config set ...` 打印 NixOS 错误 |
...
| `addToSystemPackages` | `bool` | `false` | 将 `hermes` CLI 添加到系统 PATH 并在系统范围内设置 `VIGIL_HOME` |
...
| `container.hostUsers` | `listOf str` | `[]` | 获得 `~/.vigil` 符号链接（指向服务 stateDir）的交互式用户，自动加入 `hermes` 组 |
...
```
/var/lib/vigil/                     # stateDir（归 hermes:hermes 所有，权限 0750）
├── .vigil/                         # VIGIL_HOME
...
| `hermes version` 显示旧版本 | 容器未重启 | `systemctl restart vigil-agent` |
