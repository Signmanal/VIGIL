...
| 提供商 | 配置方式 |
|----------|-------|
| **VIGIL Portal** | `vigil model`（OAuth，订阅制） |
...
**完整详情：** 参见专属的 [VIGIL Portal 集成页面](/integrations/nous-portal)（订阅内容、模型目录、故障排查）以及分步指南[使用 VIGIL Portal 运行 VIGIL Agent](/guides/run-vigil-with-nous-portal)。
...
:::warning
即使使用 VIGIL Portal、Codex 或自定义端点，某些工具（视觉、网页摘要、MoA）仍会使用单独的"辅助"模型。默认情况下（`auxiliary.*.provider: "auto"`），VIGIL 将这些任务路由到你的**主聊天模型**——即你在 `vigil model` 中选择的同一模型。你可以单独覆盖每个任务，将其路由到更便宜/更快的模型（例如 OpenRouter 上的 Gemini Flash）——参见[辅助模型](/user-guide/configuration#auxiliary-models)。
:::

:::tip Nous Tool Gateway
付费 VIGIL Portal 订阅者还可访问 **[Tool Gateway](/user-guide/features/tool-gateway)**——网页搜索、图像生成、TTS 和浏览器自动化，均通过你的订阅路由。无需额外 API key。全新安装时，`vigil setup --portal` 一条命令即可完成登录、设置 Nous 为提供商并开启网关。现有用户可通过 `vigil model` 或 `vigil tools` 按工具启用。随时使用 `vigil portal info` 查看路由状态。
:::
...
7. **VIGIL Portal** — 将 Nous 模型 ID 后缀匹配到 OpenRouter 元数据
...
| 使用场景 | 推荐方案 |
|----------|-------------|
| **只想让它工作** | OpenRouter（默认）或 VIGIL Portal |
| **本地模型，简单配置** | Ollama |
