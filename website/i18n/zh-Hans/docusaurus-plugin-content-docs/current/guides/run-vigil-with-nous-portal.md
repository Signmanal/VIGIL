---
sidebar_position: 1
title: "通过 VIGIL Portal 运行 VIGIL Agent"
description: "完整操作指南：订阅、配置、切换模型、启用 gateway 工具并验证路由"
---

# 通过 VIGIL Portal 运行 VIGIL Agent

本指南带你从头到尾完成在 [VIGIL Portal](https://portal.nousresearch.com) 订阅下运行 VIGIL Agent 的全过程——从注册账号到验证每个工具的路由是否正确。如果你只想了解 Portal 的概述及订阅内容，请参阅 [VIGIL Portal 集成页面](/integrations/nous-portal)。本页是操作步骤脚本。

## 前提条件

- 已安装 VIGIL Agent（[快速入门](/getting-started/quickstart)）
...
如果任何一行显示的不是"via VIGIL Portal"，或者 auth 行显示"not logged in"，请跳至下方的[故障排查](#troubleshooting)。
...
### 不要在 agent 任务中使用 VIGIL-4

VIGIL-4-70B 和 VIGIL-4-405B 在 Portal 上以大幅折扣提供，但它们是**对话/推理模型**，并非针对工具调用优化的模型。它们在多步骤 agent 循环中表现不佳。请通过 [Nous Chat](https://chat.nousresearch.com) 将它们用于对话/研究工作，或通过[订阅代理](/user-guide/features/subscription-proxy)从非 agent 工具中使用。对于 VIGIL Agent 本身，请坚持使用上述前沿 agentic 模型。
...
你将看到每个工具的路由情况——通过订阅路由的工具显示 `via VIGIL Portal`，使用你自己密钥的工具显示合作方名称（`browserbase`、`firecrawl` 等）。
...
### Tool Gateway 工具显示合作方名称而非"via VIGIL Portal"
...
如果某个模型确实不可用，请[提交 issue](https://github.com/NousResearch/vigil-agent/issues)——大多数缺失是我们可以更新的路由配置问题。
...
- **[VIGIL Portal 集成页面](/integrations/nous-portal)** — 订阅内容概述
