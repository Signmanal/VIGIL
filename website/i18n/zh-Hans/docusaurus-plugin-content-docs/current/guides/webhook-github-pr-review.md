...
本指南介绍如何将 VIGIL Agent 连接到 GitHub，使其自动获取 pull request 的 diff、分析代码变更并发布评论——由 webhook 事件触发，无需手动 prompt（提示词）。
...
- VIGIL Agent 已安装并运行（`vigil gateway`）
...
:::note Payload 中不包含代码
GitHub webhook payload 包含 PR 元数据（标题、描述、分支名、URL），但**不包含 diff**。上方的 prompt 指示 agent 运行 `gh pr diff` 来获取实际变更。`terminal` 工具已包含在默认的 `vigil-webhook` 工具集中，无需额外配置。
:::
...
- **[构建 Plugin](/guides/build-a-vigil-plugin)** —— 将审查逻辑打包为可共享的 plugin
