...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  VIGIL Agent  ──▶  GitHub API  ──▶  Review     │
...
- **已安装 VIGIL Agent** — 参见[安装指南](/getting-started/installation)
...
```
Run: gh pr list --repo NousResearch/vigil-agent --state open --limit 3
```
...
Run: gh pr diff 3888 --repo NousResearch/vigil-agent
```
...
- **[构建 Plugin](/guides/build-a-vigil-plugin)** — 将审查逻辑封装为可共享的 plugin
