...
创建 `/etc/systemd/system/vigil-teams-pipeline-maintain.service`：
...
以及 `/etc/systemd/system/vigil-teams-pipeline-maintain.timer`：
...
```cron
0 */12 * * * /usr/local/bin/vigil teams-pipeline maintain-subscriptions >> /var/log/vigil/teams-pipeline-maintain.log 2>&1
```
