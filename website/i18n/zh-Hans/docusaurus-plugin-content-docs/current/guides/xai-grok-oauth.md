...
description: "使用 SuperGrok 或 X Premium+ 订阅登录，在 VIGIL Agent 中使用 Grok 模型——无需 API 密钥"
---

# xAI Grok OAuth（SuperGrok / X Premium+）

VIGIL Agent 通过基于浏览器的 OAuth 登录流程支持 xAI Grok，认证服务器为 [accounts.x.ai](https://accounts.x.ai)，支持 **SuperGrok 订阅**（[grok.com](https://x.ai/grok)）或 **X Premium+ 订阅**（已关联的 X 账号）。无需 `XAI_API_KEY`——登录一次后，VIGIL 会在后台自动刷新会话。
...
xAI 的后端对 OAuth API 接口维护自己的白名单，已有记录显示即使应用内订阅处于激活状态，标准 SuperGrok 订阅者也会收到 `HTTP 403`（见 issue [#26847](https://github.com/NousResearch/vigil-agent/issues/26847)）。如果浏览器中 OAuth 登录成功但推理返回 403，请设置 `XAI_API_KEY` 并切换到 API 密钥路径（`provider: xai`）——该接口目前不受相同限制。
:::
...
在没有浏览器的服务器、容器或 SSH 会话中，VIGIL 会检测到远程环境并打印授权 URL，而不是打开浏览器。
...
如果你没有常规 SSH 客户端（例如在 GCP Cloud Shell、GitHub Codespaces、AWS EC2 Instance Connect、Gitpod 或其他基于浏览器的控制台中运行 VIGIL），上述 `ssh -L` 方案不可用。请改用 `--manual-paste`——VIGIL 跳过回环监听器，让你直接从浏览器粘贴失败的回调 URL：
...
完整操作说明请参阅 [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect)。此为 [#26923](https://github.com/NousResearch/vigil-agent/issues/26923) 的回归修复。

## 登录流程说明

1. VIGIL 在浏览器中打开 `accounts.x.ai`。
2. 你登录（或确认现有会话）并批准访问。
3. xAI 重定向回 VIGIL，token 保存到 `~/.vigil/auth.json`。
4. 此后，VIGIL 在后台刷新 access token——你将保持登录状态，直到执行 `hermes auth logout xai-oauth` 或在 xAI 账号设置中撤销访问。
...
回环监听器有有限的过期窗口（默认 180 秒）。如果你未在时限内批准登录，VIGIL 会抛出超时错误。

**修复方法：** 重新运行 `hermes auth add xai-oauth`（或 `hermes model`）。流程重新开始。

### State 不匹配（可能的 CSRF）

VIGIL 检测到授权服务器返回的 `state` 值与发送的不匹配。

**修复方法：** 重新运行登录。如果问题持续，检查是否有代理或重定向在修改 OAuth 响应。

### 从远程服务器登录

在 SSH 或容器会话中，VIGIL 打印授权 URL 而不是打开浏览器。回环回调监听器仍绑定在远程主机的 `127.0.0.1:56121`——你笔记本上的浏览器无法访问它，除非进行 SSH 本地端口转发：
...
这**不是** token 过期问题——重新运行 `hermes model` 不会改变结果。xAI 的后端已被观察到将 OAuth API 访问限制在特定 SuperGrok 套餐，即使应用内订阅处于激活状态（issue [#26847](https://github.com/NousResearch/vigil-agent/issues/26847)）。
