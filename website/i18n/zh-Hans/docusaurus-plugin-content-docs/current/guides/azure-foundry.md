...
description: "将 VIGIL Agent 与 Microsoft Foundry 配合使用——OpenAI 风格与 Anthropic 风格端点、传输协议与已部署模型的自动检测"
---

# Microsoft Foundry

VIGIL Agent 的 `azure-foundry` provider 支持 Microsoft Foundry（原 Azure AI Foundry）和 Azure OpenAI。单个 Foundry 资源可以托管两种不同传输格式的模型：
...
Azure SDK 直接读取 `AZURE_*` 环境变量。VIGIL 除在 `hermes doctor` 输出中报告哪些来源存在外，不会检查这些变量。
