...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - vigil_home (str): The active VIGIL_HOME directory path. Use this
    ...
    def save_config(self, values: Dict[str, Any], vigil_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'vigil memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``vigil_home`` is the active VIGIL_HOME directory path.
