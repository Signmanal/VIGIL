...
appropriate.  All installs go to a VIGIL-owned bin staging dir,
...
def vigil_lsp_bin_dir() -> Path:
    """Return the VIGIL-owned bin staging dir for LSP servers."""
    home = os.environ.get("VIGIL_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".vigil")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(vigil_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = vigil_lsp_bin_dir().parent  # <VIGIL_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a vigil-owned target dir.
    ...
    console script.
    """
    pip_target = vigil_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = vigil_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
