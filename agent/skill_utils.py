...
from vigil_constants import get_config_path, get_skills_dir, is_termux
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``vigil_cli.config`` on the
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    vigil = metadata.get("vigil") or {}
    if not isinstance(vigil, dict):
        vigil = {}
    return {
        "fallback_for_toolsets": vigil.get("fallback_for_toolsets", []),
        "requires_toolsets": vigil.get("requires_toolsets", []),
        "fallback_for_tools": vigil.get("fallback_for_tools", []),
        "requires_tools": vigil.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    vigil = metadata.get("vigil")
    if not isinstance(vigil, dict):
        return []
    raw = vigil.get("config")
    if not raw:
        return []
