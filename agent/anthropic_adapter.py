...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      ...
      4. Anthropic credential_pool OAuth entry (~/.vigil/auth.json)
    ...
    # 1. VIGIL-managed OAuth/setup token env var
    ...
    # 4. VIGIL credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration VIGIL configs.
...
def read_vigil_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if _VIGIL_OAUTH_FILE.exists():
        try:
            data = json.loads(_VIGIL_OAUTH_FILE.read_text(encoding="utf-8"))
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See vigil-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for m in result:
        if m["role"] == "assistant" and isinstance(m["content"], list):
            ...
            # replaying it verbatim.  See vigil-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See vigil-agent#13848 (Kimi) and
    vigil-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        #      a) bare VIGIL-native tools (``read_file``)  -> ``mcp__read_file``
