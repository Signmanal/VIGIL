...
from vigil_logging import set_session_context
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            if agent.provider == "nous":
                try:
                    ...
                    if _nous_remaining is not None and _nous_remaining > 0:
                        _nous_msg = (
                            f"VIGIL Portal rate limit active — "
            ...
            try:
                ...
                try:
                    from vigil_cli.middleware import apply_llm_request_middleware
                ...
                from vigil_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a VIGIL/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if (
                    ...
                    from vigil_constants import display_vigil_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for VIGIL-managed OAuth/setup tokens")
                ...
                # ── VIGIL Portal: record rate limit & skip retries ─────
                ...
                #
                # IMPORTANT: VIGIL Portal multiplexes multiple upstream
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}   💡 VIGIL Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: vigil portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.nousresearch.com", force=True)
                                # ``:free`` is OpenRouter slug syntax; VIGIL Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         VIGIL Portal won't recognize that model name. Either switch to a", force=True)
