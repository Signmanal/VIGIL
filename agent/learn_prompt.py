...
that instructs the live agent to:
  ...
  2. Author a single ``SKILL.md`` via ``skill_manage`` that follows the VIGIL
     ...
     order, VIGIL-tool framing, no invented commands).
...
# (HARDLINE)" and the vigil-agent-dev new-skill salvage reference. Embedded in
...
- version: 0.1.0
- author: the human you are authoring this for, first; "VIGIL Agent" second.
...
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.vigil.tags: a few Capitalized, Relevant, Tags.
...
VIGIL-tool framing (this is what makes it a skill, not shell docs):
