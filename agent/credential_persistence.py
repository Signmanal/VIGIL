...
``auth.json``.  They intentionally have no dependency on ``vigil_cli.auth`` so
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and VIGIL-owned OAuth/device-code state)
