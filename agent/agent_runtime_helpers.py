...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    # VIGIL Portal proxies to OpenRouter behind the scenes — identical
    ...
    if (is_openrouter or is_nous_portal) and is_claude:
        return True, False
    # VIGIL Portal Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from vigil_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
