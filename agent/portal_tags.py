"""Centralized VIGIL Portal request tags.

Every VIGIL request that hits the VIGIL Portal — main agent loop, auxiliary
...
Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):
    ...
    [
        "product=vigil-agent",
        "client=vigil-client-v<__version__>",
    ]

The version is sourced live from ``vigil_cli.__version__`` so it auto-aligns
...
``vigil_cli.__version__`` is the canonical source of truth.
"""
...
def _vigil_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``vigil_cli`` cannot be imported (should
...
def vigil_client_tag() -> str:
    """Return the ``client=...`` tag for VIGIL Portal requests.

    Format: ``client=vigil-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=vigil-client-v{_vigil_version()}"


def nous_portal_tags() -> List[str]:
    """Return the canonical list of VIGIL Portal product tags.

    Always returns a fresh list so callers can mutate it freely
    (e.g. ``merged_extra.setdefault("tags", []).extend(nous_portal_tags())``).
    """
    return ["product=vigil-agent", vigil_client_tag()]
