...
Resolution order for text tasks (auto mode):
  ...
  3. VIGIL Portal (~/.vigil/auth.json active provider)
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://vigil-agent.nousresearch.com",
    "X-Title": "VIGIL Agent",
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "VIGILAgent",
}
...
# VIGIL Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by VIGIL Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with vigil_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    """Return a fresh VIGIL Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``vigil_cli.__version__`` is
...
# Set at resolve time — True if the auxiliary client points to VIGIL Portal
...
_AUTH_JSON_PATH = get_vigil_home() / "auth.json"
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``vigil_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _nous_api_key(provider: dict) -> str:
    ...
    from vigil_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from vigil_cli.auth import _agent_key_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    Falls back to ``vigil_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from vigil_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from vigil_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from vigil_cli.auth import is_provider_explicitly_configured
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        ...
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping VIGIL Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
    ...
    logger.debug("Auxiliary client: VIGIL Portal")
    ...
    model = _NOUS_MODEL
    try:
        from vigil_cli.models import get_nous_recommended_aux_model
...
def _refresh_nous_recommended_model(
    *, vision: bool, stale_model: Optional[str]
    ...
    """Re-fetch the VIGIL Portal's recommended model after a stale-model 404.
...
def _try_azure_foundry(
    *,
    ...
    :func:`vigil_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from vigil_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but VIGIL Portal not configured (run: vigil auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
    ...
    try:
        from vigil_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("vigil_cli.auth not available for provider %s", provider)
        return None, None
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #   3. VIGIL Portal (vision-capable aggregator fallback)
        #   4. Stop
        main_provider = _read_main_provider()
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes VIGIL Portal product tags when the auxiliary client is backed
    by VIGIL Portal. Returns empty dict otherwise.
    """
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`vigil_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def call_llm(
    ...
    *,
    ...
    try:
        ...
        # ── Stale-model self-heal (VIGIL Portal recommendation drift) ───
