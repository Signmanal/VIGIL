...
VIGIL seeds its credential pool from many places:
    ...
    vigil_pkce   — ~/.vigil/.anthropic_oauth.json
...
qwen-cli, nous device_code (partial), vigil_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    """
    from vigil_cli.config import get_env_path, remove_env_value
...
def _remove_vigil_pkce(provider: str, removed) -> RemovalResult:
    """~/.vigil/.anthropic_oauth.json is ours — delete it outright."""
    from vigil_constants import get_vigil_home

    result = RemovalResult()
    oauth_file = get_vigil_home() / ".anthropic_oauth.json"
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Belt-and-braces against the manual entry path: ``vigil auth add
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="vigil_pkce",
        remove_fn=_remove_vigil_pkce,
