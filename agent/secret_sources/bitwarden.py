...
* The ``bws`` binary is auto-installed into ``<vigil_home>/bin/bws`` on
...
# In-process cache so repeated load_vigil_dotenv() calls (CLI startup,
...
# <vigil_home>/cache/bws_cache.json. The file holds only the secret VALUES,
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under vigil_home/cache/.

    `home_path` is what `load_vigil_dotenv()` already resolved; falling back
    to `$VIGIL_HOME` / `~/.vigil` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("VIGIL_HOME", Path.home() / ".vigil"))
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<vigil_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _vigil_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="vigil-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "vigil-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<vigil_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from vigil_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_vigil_dotenv()`` calls after the .env
