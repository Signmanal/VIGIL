...
def _handle_create(args: dict, **kw) -> str:
    ...
    session_id = args.get("session_id") or os.environ.get("VIGIL_SESSION_ID")
    priority = args.get("priority")
    # Resolve workspace. If the caller passed one explicitly, honor it.
    # Otherwise, a dispatcher-spawned worker (VIGIL_KANBAN_TASK set)
    ...
    # (kanban toolset, no VIGIL_KANBAN_TASK) and CLI/dashboard callers
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    - **Gateway** (telegram/discord/slack/etc): ``VIGIL_SESSION_PLATFORM``
      and ``VIGIL_SESSION_CHAT_ID`` are set in ContextVars by the
