...
def register_credential_file(
    ...
    *relative_path* is relative to ``VIGIL_HOME`` (e.g. ``google_token.json``).
    ...
    """
    vigil_home = _resolve_vigil_home()
    ...
    host_path = vigil_home / relative_path
    ...
    containment_error = validate_within_dir(host_path, vigil_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.vigil/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="vigil-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching vigil_constants.get_vigil_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_vigil_dir()`` for backward compatibility with old directory layouts.
    """
    from vigil_constants import get_vigil_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_vigil_dir(new_subpath, old_name)
        if host_dir.is_dir():
