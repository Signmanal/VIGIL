...
def has_xai_credentials() -> bool:
    ...
    try:
        ...
        auth_path = get_vigil_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    ...
    Wraps :func:`vigil_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from vigil_cli.config import get_env_value as _vigil_get_env_value

        value = _vigil_get_env_value(name)
...
def vigil_xai_user_agent() -> str:
    """Return a stable VIGIL-specific User-Agent for xAI HTTP calls."""
    try:
        from vigil_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"VIGIL/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers VIGIL-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``vigil_cli.config.get_env_value`` so keys
