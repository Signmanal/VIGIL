...
`mcp_tool.py`, and `vigil_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# VIGILMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_vigil_provider_class() -> Optional[type]:
    ...
    class VIGILMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._vigil_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`VIGILTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, VIGILTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._vigil_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import VIGILTokenStorage
                        if isinstance(storage, VIGILTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._vigil_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import VIGILTokenStorage
            if not isinstance(storage, VIGILTokenStorage):
                return
    ...
    return VIGILMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_VIGIL_PROVIDER_CLS: Optional[type] = _make_vigil_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`VIGILMCPOAuthProvider` directly using the helpers
        ...
        storage = VIGILTokenStorage(server_name)
