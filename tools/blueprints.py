...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.vigil.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.vigil.blueprint`` is a mapping containing
    ...
    vigil = meta.get("vigil") if isinstance(meta, dict) else None
    blueprint = vigil.get("blueprint") if isinstance(vigil, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.vigil.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.vigil.blueprint`` block) they can hand
