...
DANGEROUS_PATTERNS = [
    ...
    # In-place edit of a VIGIL-managed security file (~/.vigil/config.yaml or
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.vigil/ form so the VIGIL config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/vigil/.vigil/config.yaml`) rather than
...
def _rewrite_resolved_vigil_home(command: str) -> str:
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate VIGIL_HOME (e.g. "/" or "") rewriting
        ...
        # /home/vigil/.vigil or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.vigil/")
    return command
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("VIGIL_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set VIGIL_INTERACTIVE or VIGIL_GATEWAY_SESSION to require approval.",
