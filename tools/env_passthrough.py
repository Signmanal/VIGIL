...
def _is_vigil_provider_credential(name: str) -> bool:
    """True if ``name`` is a VIGIL-managed provider credential (API key,
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are VIGIL-managed provider credentials (from
    ``_VIGIL_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a VIGIL-managed
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_vigil_provider_credential(name):
            logger.warning(
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # VIGIL-managed provider credentials must not be passed
