...
Architecture (two transports):
  ...
  **Local backend (UDS):**
  1. Parent generates a `vigil_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `vigil_tools.py` with file-based RPC stubs
...
# are non-secret runtime-location flags (the same set vigil_cli treats as the
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-VIGIL-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-VIGIL-profile soft guard."""',
...
def generate_vigil_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the vigil_tools.py stub module.
...
_RPC_DIR = os.environ.get("VIGIL_RPC_DIR") or os.path.join(tempfile.gettempdir(), "vigil_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated vigil_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/vigil_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_vigil_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/vigil_tools.py", tools_src)
...
def execute_code(
    ...
    # --- Set up temp directory with vigil_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="vigil_sandbox_")
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"vigil_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated vigil_tools module.
        ...
        tools_src = generate_vigil_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "vigil_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        # Ensure the vigil-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from vigil_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _vigil_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _vigil_root]
        if _existing_pp:
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (vigil-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    # temp-dir staging and vigil-agent's own python.
    if mode == "strict":
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call VIGIL tools programmatically. "
        ...
        f"Available via `from vigil_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into vigil_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from vigil_tools import {import_str}` "
