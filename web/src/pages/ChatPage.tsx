...
import { VIGIL_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
function buildWsUrl(
  ...
  // Profile-scoped chat: the PTY child gets VIGIL_HOME pointed at the
  // selected profile, so the conversation runs with that profile's model,
  // skills, memory, and sessions (see web_server._resolve_chat_argv).
  if (profile) qs.set("profile", profile);
  return `${proto}//${window.location.host}${VIGIL_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  useEffect(() => {
    ...
    const gated = !!window.__VIGIL_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner VIGIL TUI has enabled xterm mouse-events
    ...
    // WebSocket. In gated mode (``window.__VIGIL_AUTH_REQUIRED__``) this
    ...
    // forwarding them into VIGIL. Keyboard input, paste, and resize still
    // behave normally.
