...
// (e.g. https://mission-control.tilos.com/vigil/). The Python backend
// injects ``window.__VIGIL_BASE_PATH__`` into index.html based on the
...
export const VIGIL_BASE_PATH = readBasePath();
const BASE = VIGIL_BASE_PATH;
...
declare global {
  interface Window {
    ...
    __VIGIL_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-VIGIL-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__VIGIL_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    if (!window.__VIGIL_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__VIGIL_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the VIGIL dashboard server");
}
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__VIGIL_AUTH_REQUIRED__) {
 ...
 * ``window.__VIGIL_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateVIGIL: () =>
    fetchJSON<ActionResponse>("/api/vigil/update", { method: "POST" }),
  checkVIGILUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/vigil/update/check${force ? "?force=true" : ""}`,
    ),
