...
#   2. ~/.vigil/node/ from a prior VIGIL-managed install
...
# Redirect a VIGIL-managed Node's `npm install -g` to the command link dir
...
ensure_node() {
    ...
    if [ -x "$VIGIL_HOME/node/bin/node" ]; then
        export PATH="$VIGIL_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (VIGIL-managed)"
            VIGIL_NODE_AVAILABLE=true
            return 0
        fi
    fi
