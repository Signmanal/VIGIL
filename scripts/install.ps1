...
$RepoUrlSsh = "git@github.com:NousResearch/vigil-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/vigil-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * VIGIL Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $VIGILHome)) {
        New-Item -ItemType Directory -Force -Path $VIGILHome | Out-Null
    }
    $envFile = Join-Path $VIGILHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $VIGILHome "node"
...
function Install-Uv {
    ...
    Write-Info "Installing managed uv into $VIGILHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $VIGILHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $VIGILHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $VIGILHome "bin"
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under VIGIL-Setup.exe
...
function Install-Git {
    ...
    which VIGIL uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\vigil\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    VIGIL needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the VIGIL copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\vigil\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``VIGIL_GIT_BASH_PATH`` (User scope) so VIGIL can find it in a fresh
    ...
    # Download PortableGit into $VIGILHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $VIGILHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent VIGIL features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  VIGIL needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``VIGIL_GIT_BASH_PATH`` (User env scope) so VIGIL can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $VIGILHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $VIGILHome\git\usr\bin\bash.exe
    $candidates += "$VIGILHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$VIGILHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    Write-Warn "Could not locate bash.exe -- VIGIL may not find Git Bash."
...
function Test-Node {
    ...
    # Prefer a VIGIL-managed Node from a previous run over a too-old system one.
    $managedNode = "$VIGILHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$VIGILHome\node;$env:Path"
        Write-Success "Node.js $version found (VIGIL-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing VIGIL-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $VIGILHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $VIGILHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                if (Test-Path "$VIGILHome\node") { Remove-Item -Recurse -Force "$VIGILHome\node" }
                Move-Item $extractedDir.FullName "$VIGILHome\node"
                ...
                $version = & "$VIGILHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $VIGILHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if VIGIL behaves unexpectedly."
                        } else {
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/vigil-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/vigil-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/vigil-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\vigil-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\vigil-agent-extract"
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under VIGIL-Setup.exe
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install vigil-agent package even with no extras. Inspect the uv pip install output above."
    }
...
function Set-PathVariable {
    ...
    # Only needed on Windows where we install to %LOCALAPPDATA%\vigil instead
    # of the Unix default ~/.vigil
    $currentVIGILHome = [Environment]::GetEnvironmentVariable("VIGIL_HOME", "User")
    if (-not $currentVIGILHome -or $currentVIGILHome -ne $VIGILHome) {
        [Environment]::SetEnvironmentVariable("VIGIL_HOME", $VIGILHome, "User")
        Write-Success "Set VIGIL_HOME=$VIGILHome"
    }
    $env:VIGIL_HOME = $VIGILHome
...
function Write-BootstrapMarker {
    # Writes $InstallDir\.vigil-bootstrap-complete which tells the VIGIL
    ...
    $markerPath = Join-Path $InstallDir ".vigil-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the VIGIL_HOME directory structure ($VIGILHome, default %LOCALAPPDATA%\vigil)
    New-Item -ItemType Directory -Force -Path "$VIGILHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$VIGILHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$VIGILHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$VIGILHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$VIGILHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$VIGILHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$VIGILHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$VIGILHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$VIGILHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$VIGILHome\.env"
    ...
    # Create config.yaml
    $configPath = "$VIGILHome\config.yaml"
...
"@
    ...
    Write-Success "Configuration directory ready: $VIGILHome"
    
    # Seed bundled skills into $VIGILHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $VIGILHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $VIGILHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$VIGILHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $VIGILHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (VIGIL-Setup.exe runs each -Stage NAME
...
function Install-Desktop {
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the VIGIL-managed
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'VIGIL.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'VIGIL.lnk')
        )
...
function Start-GatewayIfConfigured {
    ...
    $whatsappSession = "$VIGILHome\whatsapp\session\creds.json"
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$VIGILHome\logs\gateway.log"
            Start-Process -FilePath $hermesCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$VIGILHome\logs\gateway-error.log" `
                -WindowStyle Hidden
...
function Write-Completion {
    ...
    Write-Host "$VIGILHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$VIGILHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$VIGILHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$VIGILHome\vigil-agent\"
    Write-Host ""
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning VIGIL repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (VIGIL-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding VIGIL to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://vigil-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
