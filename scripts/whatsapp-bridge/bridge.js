...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.vigil', 'whatsapp', 'session'));
...
const IMAGE_CACHE_DIR = process.env.VIGIL_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.vigil', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.VIGIL_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.vigil', 'document_cache');
const AUDIO_CACHE_DIR = process.env.VIGIL_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.vigil', 'audio_cache');
...
const DEFAULT_REPLY_PREFIX = '⚕ *VIGIL Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['VIGIL Agent', 'Chrome', '120.0'],
