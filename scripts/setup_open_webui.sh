...
# Bootstrap Open WebUI against VIGIL Agent's OpenAI-compatible API server.
#
...
#   VIGIL_API_PORT=8642
#   VIGIL_API_HOST=127.0.0.1
#   VIGIL_API_MODEL_NAME='VIGIL Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-VIGIL Agent WebUI}"
...
VIGIL_ENV_FILE="${VIGIL_ENV_FILE:-$HOME/.vigil/.env}"
VIGIL_API_PORT="${VIGIL_API_PORT:-8642}"
VIGIL_API_HOST="${VIGIL_API_HOST:-127.0.0.1}"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$VIGIL_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$VIGIL_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.vigil.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.vigil</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.vigil"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.vigil"
}
...
install_systemd_user_service() {
  ...
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to VIGIL Agent
...
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.vigil/logs/openwebui.log
StandardError=append:%h/.vigil/logs/openwebui.error.log
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$VIGIL_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring VIGIL API server is configured...'
  upsert_env API_SERVER_ENABLED true "$VIGIL_ENV_FILE"
  upsert_env API_SERVER_HOST "$VIGIL_API_HOST" "$VIGIL_ENV_FILE"
  upsert_env API_SERVER_PORT "$VIGIL_API_PORT" "$VIGIL_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$VIGIL_API_MODEL_NAME" "$VIGIL_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$VIGIL_ENV_FILE"
  ensure_env_permissions
  ...
  log "VIGIL API endpoint: ${VIGIL_API_BASE_URL}"
