...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/vigil-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/vigil-agent.git"
...
#   code at /usr/local/lib/vigil-agent, command at /usr/local/bin/vigil,
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, VIGIL installs the code under"
            echo "  /usr/local/lib/vigil-agent and links the command into"
            echo "  /usr/local/bin/vigil (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$VIGIL_HOME"
            echo "  (default /root/.vigil).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$VIGIL_HOME/vigil-agent are preserved in-place."
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (VIGIL-Setup) passes it so
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $VIGIL_HOME/vigil-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $VIGIL_HOME/vigil-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/vigil-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $VIGIL_HOME/vigil-agent — then preserve it)
#
...
resolve_install_layout() {
    ...
    if is_termux; then
        INSTALL_DIR="$VIGIL_HOME/vigil-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$VIGIL_HOME/vigil-agent/.git" ]; then
            INSTALL_DIR="$VIGIL_HOME/vigil-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/vigil-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/vigil-agent"
        ...
        # /usr/local/bin/vigil wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/vigil"
...
# Point a VIGIL-managed Node's `npm install -g` at a directory that is on
...
# Idempotent and a no-op when there is no VIGIL-managed npm, so calling it on
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://vigil-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
# string clears that floor; anything below it is replaced with the VIGIL-
...
check_node() {
    ...
    # Repair pre-existing VIGIL-managed installs where `npm install -g` lands
    ...
    if [ -x "$VIGIL_HOME/node/bin/node" ] && node_satisfies_build "$("$VIGIL_HOME/node/bin/node" --version)"; then
        ...
        log_success "Node.js $("$VIGIL_HOME/node/bin/node" --version) found (VIGIL-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing VIGIL-managed Node $NODE_VERSION LTS..."
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. VIGIL install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "VIGIL Agent itself does not require or retain root access."
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if VIGIL behaves unexpectedly."
                    else
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# VIGIL Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# VIGIL Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "VIGIL will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# VIGIL Agent browser tools — explicit browser override."
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for VIGIL to send/receive messages."
...
ensure_browser() {
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$VIGIL_HOME/node" --silent --ignore-scripts \
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up VIGIL for pip install"
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the VIGIL-managed Node to PATH, so
    ...
    # trips Gatekeeper's tamper check ("VIGIL is damaged and can't be opened").
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the VIGIL-managed Node
