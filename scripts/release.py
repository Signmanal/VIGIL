...
AUTHOR_MAP = {
    ...
    "274096618+vigil-agent-dhabibi@users.noreply.github.com": "dhabibi",
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"vigil-agent[acp]=={semver}"
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/vigil-agent",
...
def main():
    ...
    print(f"{'='*60}")
    print(f"  VIGIL Agent Release Preview")
    print(f"{'='*60}")
