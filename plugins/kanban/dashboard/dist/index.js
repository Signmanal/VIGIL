...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "vigil-kanban-dot-triage",
    todo: "vigil-kanban-dot-todo",
    ready: "vigil-kanban-dot-ready",
    running: "vigil-kanban-dot-running",
    blocked: "vigil-kanban-dot-blocked",
    done: "vigil-kanban-dot-done",
    archived: "vigil-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-vigil-task";
  ...
  const DOCS_URL = "https://vigil-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://vigil-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "vigil.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="vigil-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])vigil-kanban-md-code\1/i.test(attrs)) {
      return ' class="vigil-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "vigil-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "vigil-kanban-md",
  ...
  // standard `drop` event and our `vigil-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("vigil-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("vigil-kanban-column--drop");
          if (target) target.classList.add("vigil-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("vigil-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("vigil-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("vigil-kanban:drop", {
  ...
  function KanbanPage() {
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "vigil-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "vigil-kanban-attention",
        "vigil-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "vigil-kanban-attention-bar" },
        h("span", { className: "vigil-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "vigil-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "vigil-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "vigil-kanban-attention-row",
                  "vigil-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "vigil-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "vigil-kanban-attention-row-id" }, task.id),
                h("span", { className: "vigil-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "vigil-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "vigil-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "vigil-kanban-diag-action-btn",
      action.suggested ? "vigil-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " vigil-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".vigil-kanban-drawer-comment-row input, .vigil-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "vigil-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("vigil-kanban-diag", sevClass) },
      h("div", { className: "vigil-kanban-diag-header" },
        h("span", { className: "vigil-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "vigil-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "vigil-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "vigil-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "vigil-kanban-diag-data-row" },
                  h("span", { className: "vigil-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "vigil-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "vigil-kanban-diag-data-row" },
                h("span", { className: "vigil-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "vigil-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "vigil-kanban-diag-reassign-row" },
            h("span", { className: "vigil-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "vigil-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "vigil-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "vigil-kanban-diag-msg",
              msg.ok ? "vigil-kanban-diag-msg--ok" : "vigil-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "vigil-kanban-section" },
      h("div", { className: "vigil-kanban-section-head-row" },
        h("span", { className: "vigil-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "vigil-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "vigil-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "vigil-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "vigil-kanban-docs-link",
  ...
  function OrchestrationPanel() {
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "vigil-kanban-msg-ok" : "vigil-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "vigil-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "vigil-kanban-boardswitcher" },
      h("div", { className: "vigil-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "vigil-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "vigil-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "vigil-kanban-dialog-title" },
        ...
        h("div", { className: "vigil-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "vigil-kanban-bulk" },
      h("span", { className: "vigil-kanban-bulk-count" },
      ...
      h("div", { className: "vigil-kanban-bulk-priority",
      ...
      h("div", { className: "vigil-kanban-bulk-reassign",
      ...
      h("label", { className: "vigil-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("vigil-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("vigil-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "vigil-kanban-trash",
        dragOver ? "vigil-kanban-trash--drop" : "",
        props.draggingTaskId ? "vigil-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "vigil-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "vigil-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".vigil-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "vigil-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("vigil-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("vigil-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "vigil-kanban-column",
        dragOver ? "vigil-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "vigil-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "vigil-kanban-col-check",
        ...
        h("span", { className: cn("vigil-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "vigil-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "vigil-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "vigil-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "vigil-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "vigil-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "vigil-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "vigil-kanban-lane" },
                  h("div", { className: "vigil-kanban-lane-head" },
                    h("span", { className: "vigil-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "vigil-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "vigil-kanban-card--stale-red";
    if (age >= tier.amber) return "vigil-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".vigil-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "vigil-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "vigil-kanban-card",
        props.selected ? "vigil-kanban-card--selected" : "",
        props.failed ? "vigil-kanban-card--failed" : "",
        props.draggingSource ? "vigil-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "vigil-kanban-card-content" },
          h("div", { className: "vigil-kanban-card-row" },
            h("label", {
              className: "vigil-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "vigil-kanban-card-check",
            ...
            h("span", { className: "vigil-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or vigil kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "vigil-kanban-warning-badge",
                    "vigil-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "vigil-kanban-needs-assignee",
          ...
          h("div", { className: "vigil-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "vigil-kanban-card-row vigil-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "vigil-kanban-assignee",
                            title: `Assigned to VIGIL profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "vigil-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "vigil-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "vigil-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "vigil-kanban-inline-create" },
      h("textarea", {
        value: title,
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "vigil-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "vigil-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "vigil-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "vigil-kanban-drawer-close",
        ...
        data ? h("div", { className: "vigil-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "vigil-kanban-section" },
      h("div", { className: "vigil-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "vigil-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "vigil-kanban-drawer-body" },
      h("div", { className: "vigil-kanban-drawer-title" },
        h("span", { className: cn("vigil-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "vigil-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "vigil-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "vigil-kanban-section" },
        h("div", { className: "vigil-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "vigil-kanban-section" },
        h("div", { className: "vigil-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "vigil-kanban-comment" },
                h("div", { className: "vigil-kanban-comment-head" },
                  h("span", { className: "vigil-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "vigil-kanban-comment-ago" },
      ...
      h("div", { className: "vigil-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "vigil-kanban-event",
              isDiag ? "vigil-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "vigil-kanban-event-header" },
                  h("span", { className: "vigil-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "vigil-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "vigil-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "vigil-kanban-event-header-plain" },
                  h("span", { className: "vigil-kanban-event-kind" }, e.kind),
                  h("span", { className: "vigil-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "vigil-kanban-event-phantom-row" },
                  h("span", { className: "vigil-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "vigil-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "vigil-kanban-event-payload" },
    ...
    return h("div", { className: "vigil-kanban-section" },
      h("div", { className: "vigil-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "vigil-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `vigil-kanban-run--${r.outcome || r.status || "ended"}`
          : "vigil-kanban-run--active";
        return h("div", { key: r.id, className: cn("vigil-kanban-run", outcomeClass) },
          h("div", { className: "vigil-kanban-run-head" },
            h("span", { className: "vigil-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "vigil-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "vigil-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "vigil-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "vigil-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "vigil-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "vigil-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "vigil-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "vigil-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "vigil-kanban-pre vigil-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "vigil-kanban-meta-row" },
      h("span", { className: "vigil-kanban-meta-label" }, props.label),
      h("span", { className: "vigil-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "vigil-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "vigil-kanban-meta-row" },
        h("span", { className: "vigil-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "vigil-kanban-meta-value vigil-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "vigil-kanban-meta-row" },
        h("span", { className: "vigil-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "vigil-kanban-meta-value vigil-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "vigil-kanban-section" },
      h("div", { className: "vigil-kanban-section-head-row" },
        h("span", { className: "vigil-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "vigil-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "vigil-kanban-section" },
      h("div", { className: "vigil-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "vigil-kanban-deps-row" },
        h("span", { className: "vigil-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "vigil-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "vigil-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "vigil-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "vigil-kanban-dep-chip-x",
      ...
      h("div", { className: "vigil-kanban-deps-row" },
        h("span", { className: "vigil-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "vigil-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "vigil-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "vigil-kanban-section" },
      ...
      h("div", { className: "vigil-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "vigil-kanban-home-sub vigil-kanban-home-sub--on"
              : "vigil-kanban-home-sub",
          }, label);
        })
      )
    );
