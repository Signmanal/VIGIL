...
# ``vigil_plugins.platforms__google_chat.adapter`` once the plugin
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            ...
            _vigil_home = _get_vigil_home()
        except (ModuleNotFoundError, ImportError):
            _vigil_home = _Path.home() / ".vigil"
        self._thread_count_store = _ThreadCountStore(
            _vigil_home / "google_chat_thread_counts.json"
        )
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        ...
        body: Dict[str, Any] = {"text": "VIGIL is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
...
def interactive_setup() -> None:
    ...
    The setup wizard at ``vigil_cli/gateway.py`` calls this for plugin
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. vigil-chat-events) and a Pull subscription.")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'vigil-agent[google_chat]'",
