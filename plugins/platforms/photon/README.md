...
This plugin connects VIGIL Agent to iMessage (and other Spectrum
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-VIGIL-Sidecar-Token`.
...
2. **Find or create** the `VIGIL Agent` project on the Photon dashboard.
...
5. Verify end-to-end: `vigil photon status`, a DM and a group roundtrip,
