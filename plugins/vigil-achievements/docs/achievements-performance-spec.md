...
Owner: vigil-achievements plugin
...
### `GET /api/plugins/vigil-achievements/achievements`
...
### `POST /api/plugins/vigil-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/vigil-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/vigil-achievements/overview`
...
- Persist plugin data under:
  - `~/.vigil/plugins/vigil-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.vigil/plugins/vigil-achievements/scan_checkpoint.json`
...
- `~/.vigil/plugins/vigil-achievements/`
