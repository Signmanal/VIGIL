(function () {
  "use strict";
  // vigil-achievements dashboard plugin
  // Originally authored by @PCinkusz — https://github.com/PCinkusz/vigil-achievements (MIT).
  // Bundled into vigil-agent. Upstream repo remains the staging ground for new
  ...
  function api(path, options) {
    ...
    // gated OAuth (vigil_session_at cookie via credentials:'include').
    ...
    const url = "/api/plugins/vigil-achievements" + path;
  ...
  async function buildShareImage(achievement) {
    ...
    ctx.fillText("VIGIL AGENT  ·  vigil-agent.nousresearch.com", 70, H - 40);
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    function download() {
      ...
      a.href = url;
      a.download = "vigil-achievement-" + (achievement.id || "badge") + ".png";
    ...
    function tweetText() {
      ...
      const tmpl = tx(t, "share.tweet_text", "Just unlocked {tier_part}\"{name}\" in VIGIL Agent ☤", {
        tier_part: tierPart,
        name: achievement.name,
      });
      return tmpl + "\n\n@NousResearch · https://vigil-agent.nousresearch.com";
    }
  ...
  window.__VIGIL_PLUGINS__.register("vigil-achievements", AchievementsPage);
})();
