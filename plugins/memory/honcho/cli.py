...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "vigil" else ""
...
def _local_config_path() -> Path:
    ...
    """
    return get_vigil_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        vigil_host.get("pinUserPeer"),
        vigil_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in vigil_host:
        aliases_src = vigil_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in vigil_host:
        prefix_src = vigil_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(vigil_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom vigil_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    vigil_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        vigil_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(vigil_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = vigil_host.get("userPeerAliases")
    prefix = vigil_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(vigil_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(vigil_host)
    vigil_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        vigil_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        vigil_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    if is_local:
        ...
        current_host_key = vigil_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            vigil_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(vigil_host)
        ...
        if use_oauth:
            ...
            try:
                ...
                return
            vigil_host["apiKey"] = cred.access_token
            vigil_host["oauth"] = cred.oauth_block()
            # Default the peer prompt to the name entered at consent.
            if cred.consent_peer_name:
                vigil_host["peerName"] = cred.consent_peer_name
    ...
    # --- 3. Identity ---
    current_peer = vigil_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        vigil_host["peerName"] = new_peer

    current_ai = vigil_host.get("aiPeer") or cfg.get("aiPeer", "vigil")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        vigil_host["aiPeer"] = new_ai

    current_workspace = vigil_host.get("workspace") or cfg.get("workspace", "vigil")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        vigil_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, vigil_host)
    ...
    if run_mapping:
        peer_target = vigil_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(vigil_host)
            vigil_host["pinUserPeer"] = False
            if prior_aliases:
                vigil_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                vigil_host, current_prefix, prefix_from_root,
            ...
            if merged:
                vigil_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                vigil_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                vigil_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = vigil_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        vigil_host["observationMode"] = new_obs
    else:
        vigil_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(vigil_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        vigil_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        vigil_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = vigil_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        vigil_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = vigil_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        vigil_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                vigil_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(vigil_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            vigil_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        vigil_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        vigil_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        vigil_host["dialecticReasoningLevel"] = new_reasoning
    else:
        vigil_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = vigil_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        vigil_host["sessionStrategy"] = new_strat

    vigil_host["enabled"] = True
    vigil_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        vigil_config = load_config()
        vigil_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(vigil_config)
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        vigil = hosts.get(_host_key(), {})
        user = vigil.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = vigil.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = vigil.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = vigil.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = vigil.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = vigil.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = vigil.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
...
def cmd_migrate(args) -> None:
    ...
    print("\nHoncho migration: OpenClaw native memory → VIGIL\n" + "─" * 50)
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        from vigil_cli.memory_setup import cmd_setup_provider
