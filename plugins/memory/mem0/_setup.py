...
def _save_mem0_json(vigil_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(vigil_home) / "mem0.json"
    existing = {}
...
def _setup_platform(vigil_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "vigil-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "vigil"},
    ...
    provider.save_config(provider_config, vigil_home)

    if env_writes:
        _write_env(Path(vigil_home) / ".env", env_writes)
...
def _setup_oss(vigil_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(vigil_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "vigil-user")
    ...
    if env_writes:
        _write_env(Path(vigil_home) / ".env", env_writes)
    _save_mem0_json(vigil_home, {"mode": "oss", "user_id": user_id, "agent_id": "vigil", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, vigil_home: str) -> str:
    """Prompt for API key, showing masked existing value if found."""
    existing = os.environ.get(env_var, "")
    if not existing:
        env_path = Path(vigil_home) / ".env"
        if env_path.exists():
...
_PGVECTOR_CONTAINER = "vigil-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "vigil"
...
def _setup_oss_interactive(vigil_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], vigil_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], vigil_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'vigil-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "vigil-user")

    agent_id = input("  Agent ID [vigil]: ").strip()
    agent_id = agent_id or "vigil"
    ...
    if env_writes:
        _write_env(Path(vigil_home) / ".env", env_writes)
    _save_mem0_json(vigil_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(vigil_home: str, config: dict) -> None:
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(vigil_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(vigil_home, config, flags)
        return
