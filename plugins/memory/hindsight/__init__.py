...
def _load_config() -> dict:
    ...
    profile_path = get_vigil_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "vigil": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "vigil"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    ...
    profile = config.get("profile", "vigil")
    return str(profile or "vigil")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    collapsed — e.g. ``vigil-{user}`` with no user becomes ``vigil``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, vigil_home):
        """Write config to $VIGIL_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(vigil_home) / "hindsight"
    ...
    def post_setup(self, vigil_home: str, config: dict) -> None:
        ...
        from vigil_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "vigil")
        ...
        self.save_config(provider_config, vigil_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(vigil_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(vigil_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "vigil"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: vigil-{profile}", "default": ""},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "vigil"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "vigil"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "vigil", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "vigil")
