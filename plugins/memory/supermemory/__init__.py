...
def _load_supermemory_config(vigil_home: str) -> dict:
    config = _default_config()
    config_path = Path(vigil_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, vigil_home: str) -> None:
    config_path = Path(vigil_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "vigil"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "vigil", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, vigil_home):
        ...
        _save_supermemory_config(sanitized, vigil_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from vigil_constants import get_vigil_home
        self._vigil_home = kwargs.get("vigil_home") or str(get_vigil_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._vigil_home)
